...
test("PR-5 strict: entry.dependencies -> installable with manual-install note", async () => {
  // arrange
  const context = resolveContext(marketplaceRoot, { [pathUnderMarketplace("./local")]: "dir" });
...
test("repeated strict resolution is observationally identical", async () => {
  // arrange
  const context = resolveContext(marketplaceRoot, {
    [pathUnderMarketplace("./repeat")]: "dir",
  });
  const entry = pluginEntry({ name: "repeat", source: "./repeat" });

  // act
  const firstResolution = await resolveStrict(entry, context);
  const secondResolution = await resolveStrict(entry, context);
