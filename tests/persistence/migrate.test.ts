...
// SPLIT-01 / D-13: a non-existent path threaded as `configJsonPath` keeps the
// D-13 autoupdate scrub GATE-CLOSED, preserving prior behavior for fixtures
// that do not carry an `autoupdate` field.
const NO_CONFIG = path.join(tmpdir(), "no-such-config-for-migrate-tests.json");
...
test("Pitfall 9 migrate on null returns empty marketplaces (no mutation flag)", () => {
  const result = migrateLegacyMarketplaceRecords(null, "/ext-root", NO_CONFIG);
  assert.deepEqual(result.marketplaces, {});
  assert.equal(result.mutated, false);
});

test("Pitfall 9 migrate on top-level array returns empty marketplaces", () => {
  const result = migrateLegacyMarketplaceRecords([1, 2, 3], "/ext-root", NO_CONFIG);
  assert.deepEqual(result.marketplaces, {});
  assert.equal(result.mutated, false);
});

test("migrate on marketplaces:[] (array, not object) resets to {} with mutated=true", () => {
  const result = migrateLegacyMarketplaceRecords({ marketplaces: [] }, "/ext-root", NO_CONFIG);
  assert.deepEqual(result.marketplaces, {});
  assert.equal(result.mutated, true);
});

test("migrate on marketplaces missing entirely returns {} with mutated=false", () => {
  const result = migrateLegacyMarketplaceRecords({ schemaVersion: 1 }, "/ext-root", NO_CONFIG);
  assert.deepEqual(result.marketplaces, {});
  assert.equal(result.mutated, false);
});

// ===================================================================
// SPLIT-01 / D-12 / D-13 -- autoupdate scrub gated on configJsonPath
// ===================================================================

test("D-13 GATE CLOSED: scrub does NOT fire when configJsonPath does not exist; autoupdate preserved", async () => {
  const fixture = JSON.parse(
    await readFile(path.join(FIXTURES, "state-with-autoupdate.json"), "utf8"),
  ) as unknown;
  // Gate closed: configJsonPath points to a path that does NOT exist on disk.
  const missingConfig = path.join(tmpdir(), "definitely-not-a-real-config.json");
  const { marketplaces } = migrateLegacyMarketplaceRecords(fixture, "/ext-root", missingConfig);
...
test("D-13 GATE OPEN: scrub fires when configJsonPath exists; autoupdate removed and mutated=true", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "pi-cm-d13-gate-open-"));
  try {
    const configJsonPath = path.join(dir, "claude-plugins.json");
    // Gate open: materialize the config file so existsSync returns true.
    await writeFile(configJsonPath, "{}", "utf8");
    const fixture = JSON.parse(
      await readFile(path.join(FIXTURES, "state-with-autoupdate.json"), "utf8"),
    ) as unknown;
    const { marketplaces, mutated } = migrateLegacyMarketplaceRecords(
...
test("D-13 idempotency: second migrate on already-scrubbed input returns mutated=false (gate open)", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "pi-cm-d13-idempotent-"));
  try {
    const configJsonPath = path.join(dir, "claude-plugins.json");
    await writeFile(configJsonPath, "{}", "utf8");
    const fixture = JSON.parse(
    ...
    const first = migrateLegacyMarketplaceRecords(fixture, "/ext-root", configJsonPath);
