...
} from "../../extensions/pi-claude-marketplace/shared/path-containment.ts";

import type { PathSafetyInspector } from "../../extensions/pi-claude-marketplace/shared/path-containment.ts";
...
test("accepts a parent as its own child boundary", async (t) => {
  ...
  // act
  try {
    await createPathSafetyGuard({
      lstat: fs.lstat,
      readlink: (target) => fs.readlink(target),
    }).assertPathInside(directory, directory, "equal boundary");
  } catch (error) {
    actualError = error;
  }
...
test("accepts an existing direct child", async (t) => {
  ...
  try {
    await createPathSafetyGuard({
      lstat: fs.lstat,
      readlink: (target) => fs.readlink(target),
    }).assertPathInside(directory, child, "direct child");
  } catch (error) {
    actualError = error;
  }
  ...
  test(`rejects raw lexical traversal spelled with ${JSON.stringify(separator)} separators`, async (t) => {
    ...
    try {
      await createPathSafetyGuard({
        lstat: fs.lstat,
        readlink: (target) => fs.readlink(target),
      }).assertPathInside(scopeRoot, rawChild, "plugin source");
    } catch (error) {
...
test("accepts a contained absolute child with redundant dot segments", async (t) => {
  ...
  try {
    await createPathSafetyGuard({
      lstat: fs.lstat,
      readlink: (target) => fs.readlink(target),
    }).assertPathInside(directory, redundantChild, "absolute child");
  } catch (error) {
...
test("rejects the direct parent as a one-step escape", async (t) => {
  ...
  try {
    await createPathSafetyGuard({
      lstat: fs.lstat,
      readlink: (target) => fs.readlink(target),
    }).assertPathInside(directory, child, "path target");
  } catch (error) {
    containmentError = error;
  }
...
test("accepts a missing intermediate segment before a write", async (t) => {
  ...
  await createPathSafetyGuard({
    lstat: fs.lstat,
    readlink: (target) => fs.readlink(target),
  }).assertPathInside(directory, child, "future component");
...
test("rejects a deeper path outside the parent boundary", async (t) => {
  ...
  try {
    await createPathSafetyGuard({
      lstat: fs.lstat,
      readlink: (target) => fs.readlink(target),
    }).assertPathInside(directory, child, "deep target");
  } catch (error) {
    containmentError = error;
  }
  ...
  test(name, async (t) => {
    ...
    try {
      await createPathSafetyGuard({
        lstat: fs.lstat,
        readlink: (target) => fs.readlink(target),
      }).assertPathInside(directory, child, "plugin component");
    } catch (error) {
      symlinkError = error;
    }
...
test("rejects lexical traversal before the required inspector runs", async (t) => {
  ...
  assert.deepStrictEqual(
    ...
    {
      name: "LexicalTraversalError",
      message: `plugin source contains forbidden lexical traversal (parent: ${path.resolve(directory)}, target: ${path.resolve(rawChild)}).`,
