// Plan 06-04 Task 1 + Phase 13 Plan 13-02d-01 migration:
//
// `listPlugins` orchestrator emits a success notification with the rendered
// plugin list, or the bare `(no plugins)` EmptyToken (CMC-10 / MSG-ER-1) for
// the empty case. We verify the shim reaches the orchestrator by observing
...
test("shim :: bare /list calls listPlugins with no marketplace, no scope, no filter flags", async () => {
  await withHermeticHome(async ({ cwd }) => {
    ...
    await handler("", ctx);
    // CMC-10 / MSG-ER-1: empty state -> bare `(no plugins)` EmptyToken.
