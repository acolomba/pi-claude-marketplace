...
for (const scenario of ["prefixed", "coexistence", "occupied retry"] as const) {
  test(`AGENT-01: update migrates legacy agents for ${scenario}`, async (t) => {
    await withHermeticHome(async () => {
      ...
      const newTarget = path.join(
        locations.agentsDir,
        "pi-claude-marketplace-hello-hello-reviewer.md",
      );
