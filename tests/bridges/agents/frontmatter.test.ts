...
test("AG-5 emitGeneratedAgentFile generator frontmatter value carries GENERATED_AGENT_MARKER substring", () => {
  const out = emitGeneratedAgentFile({
    frontmatter: {
      name: "pi-claude-marketplace-acme-bot",
      description: "A.",
...
test("T-d8i-02 emitGeneratedAgentFile keeps a newline-bearing sourcePath on one frontmatter line, blocking key injection", () => {
  const out = emitGeneratedAgentFile({
    frontmatter: {
      name: "pi-claude-marketplace-acme-bot",
      description: "A.",
  ...
  // The newline is collapsed to a space -- the whole crafted value round-trips
  // on the sourcePath key, and no bogus `injectedKey` was parsed out.
  assert.equal(raw.sourcePath, "/path/evil.md injectedKey: injectedValue");
...
test("T-d8i-01 provenance keys are appended immediately after inheritSkills, with warnings last before the closing ---", () => {
  const base = makeLegendEmitInput();
  const out = emitGeneratedAgentFile({
    ...base,
    frontmatter: { ...base.frontmatter, inheritSkills: true },
...
// prompt), and rides the frontmatter `generator:` key instead.
// ---------------------------------------------------------------------------

test("T-d8i-01 parseFrontmatter(emitGeneratedAgentFile(...)).body excludes all provenance text", () => {
  const out = emitGeneratedAgentFile({
    ...makeLegendEmitInput(),
    provenance: {
      pluginName: "acme",
  ...
  const { raw, body } = parseFrontmatter(out);
  assert.doesNotMatch(body, /generated by pi-claude-marketplace/);
  ...
  assert.equal(raw.generator, GENERATED_AGENT_MARKER);
});
