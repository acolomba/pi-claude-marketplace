...
test("T-d8i-02 emitGeneratedAgentFile keeps a newline-bearing sourcePath on one provenance item, blocking key injection", () => {
  const out = emitGeneratedAgentFile({
    frontmatter: {
      name: "pi-claude-marketplace-acme-bot",
      description: "A.",
  ...
  // provenance list item -- no bogus `injectedKey` was parsed out.
...
test("T-d8i-01 provenance block is appended immediately after inheritSkills, with the warnings item last before the closing ---", () => {
  const base = makeLegendEmitInput();
  const out = emitGeneratedAgentFile({
    ...base,
    frontmatter: { ...base.frontmatter, inheritSkills: true },
...
// prompt), and rides the frontmatter `provenance` block-list instead.
// ---------------------------------------------------------------------------
...
test("T-d8i-01 parseFrontmatter(emitGeneratedAgentFile(...)).body excludes all provenance text while the marker survives in the file", () => {
  ...
  // it (as the first provenance item) for isOwnedAgentFile.
  assert.ok(out.includes(GENERATED_AGENT_MARKER));
});
