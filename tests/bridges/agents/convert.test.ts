...
test("AGSK-02 / T-d8i-01 crafted cross-plugin token stays confined to frontmatter, never reaching the body", () => {
  // The warning echoes the token verbatim into the `warnings:` frontmatter
  // value; T-d8i-01 requires it stay out of the body (the child subagent's
  // system prompt), with no injected frontmatter key.
  ...
  assert.match((raw.warnings as string | undefined) ?? "", /evil-->qual:skill/);
