...
test("AGSK-02 / T-d8i-01 crafted cross-plugin token stays confined to piSubagentMetadata, never reaching the body", () => {
  // The warning echoes the token verbatim into the piSubagentMetadata block;
  // T-d8i-01 requires it stay out of the body (the child subagent's system
  // prompt), with no injected frontmatter key.
  ...
  const { raw, body } = parseFrontmatter(out.fileContent);
  assert.doesNotMatch(body, /evil-->qual:skill/);
  // The token round-trips inside the folded piSubagentMetadata value, and the
  // crafted `qual:` segment never became a top-level frontmatter key.
  assert.match((raw.piSubagentMetadata as string | undefined) ?? "", /evil-->qual:skill/);
  assert.equal("qual" in raw, false);
});
