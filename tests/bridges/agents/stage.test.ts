...
test("AG-5 staged file carries the GENERATED_AGENT_MARKER substring", async () => {
  await withTmpScope(async ({ locations }) => {
    const pluginRoot = path.join(FIXTURES, "test-plugin");
    const resolved = makeResolved("acme", pluginRoot);
    ...
    for (const name of prepared.result.stagedNames) {
      ...
      assert.ok(content.includes(GENERATED_AGENT_MARKER));
    }
  });
});
...
test("commit re-stage path: rm previous targets first, then rename staged", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    assert.ok(firstContent.includes(GENERATED_AGENT_MARKER));
    ...
    assert.ok(secondContent.includes(GENERATED_AGENT_MARKER));
  });
});
...
test("TR-06 replacePreparedAgents tolerates owned orphan file from prior partial install", async () => {
  ...
  await withTmpScope(async ({ locations }) => {
    ...
    // The new agent file must carry the generated marker (this is
    // the regenerated content, not the orphan).
    assert.ok(
      replacedBody.includes(GENERATED_AGENT_MARKER),
      "replacement file must carry the generated marker",
    );
  });
});
