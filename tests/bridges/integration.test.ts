...
describe("integration: full-plugin staging", () => {
  ...
  test("AG-1/AG-2/AG-3/AG-5: agents bridge stages with marker discipline, partitions index by (mp, plugin)", async () => {
    ...
    // AG-5 marker discipline: every staged file carries the verbatim
    // generatedBy marker emitted by frontmatter.ts.
    for (const name of agentsDirContents) {
      const fileContent = await readFile(path.join(locations.agentsDir, name), "utf8");
      assert.ok(
        fileContent.includes(GENERATED_AGENT_MARKER),
        `AG-5: agent file "${name}" carries the generated marker`,
      );
    }
