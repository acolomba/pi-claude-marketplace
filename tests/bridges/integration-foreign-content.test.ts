...
//
// Foreign-content preservation + CommitResult.failed[] surfacing across the
// agents bridge end-to-end.
//
// AG-5 foreign content (a previously-indexed target file whose body has lost
// the verbatim marker, OR whose basename no longer matches
// `pi-claude-marketplace-*`) is preserved BYTE-IDENTICAL on commit and
// surfaced via the bridge's `result.failed[]` array (D-06 corollary:
// prepare-time foreign content does NOT throw, it surfaces softly so the
// install can still proceed for the agents that ARE owned).
//
...
describe("integration: foreign content preservation", () => {
  ...
  test("AG-5: foreign target with valid basename but missing marker is preserved byte-identical and surfaced via failed[]", async () => {
    ...
    // Sanity-check the fixture really lacks BOTH the current and legacy
    // markers -- if the no-marker.md fixture ever changes, this fails loudly.
    assert.ok(
      !beforeBytes.includes(GENERATED_AGENT_MARKER) &&
        !beforeBytes.includes(GENERATED_AGENT_MARKER_LEGACY),
      "AG-5 fixture invariant: no-marker.md must NOT contain any generated-marker substring",
    );
