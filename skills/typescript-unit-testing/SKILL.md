...
description: Unit testing rules for TypeScript -- pairing, coverage, case structure, naming, assertions, test doubles, test data, hermeticity, and production design for tests. Apply when writing or changing tests/**/*.ts or the extension modules they pair with.
---
...
- Seeds reused across cases live in the concern's seed module (`tests/orders/order-seeds.ts`) as functions that return fresh values, never shared mutable constants.
...
Keep fakes, seeds, contracts, and fixtures next to the tests of the concern they serve: `tests/orders/create-fake-order-store.ts`, `tests/orders/order-seeds.ts`, `tests/orders/order-store-contract.ts`. Do not create `tests/helpers/`, `tests/utils/`, or `tests/mocks/`. Use a `fixtures/` directory only when a concern has enough static files to need one.
