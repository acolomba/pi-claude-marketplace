...
- **User-visible concurrent-operation marker updated:** Concurrent mutating operations now fail fast with `Another pi-claude-marketplace operation is in progress for <scope>; retry the command.` when the per-scope state lock is already held. Retry remains the recovery action. This supersedes the older V1 concurrent-install path that could surface `was installed concurrently` after reaching state-guard commit; Phase 7 D-08 prevents that race before mutation starts.
