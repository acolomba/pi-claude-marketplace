...
- Private git sources authenticate on demand through a provider-auth registry: a private GitHub-hosted source triggers the GitHub Device Flow, and bulk install/update/reinstall sweeps prompt at most once per host. Credentials are never persisted, logged, or rendered.
