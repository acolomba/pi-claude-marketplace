...
- The `/claude:plugin` command now opens an interactive browser when run in interactive mode, matching Claude Code's `/plugin` picker. Running bare `/claude:plugin` or `/claude:plugin browse` provides a four-screen browser to inspect marketplaces, browse plugins, pick actions (install, enable, disable, info, uninstall), and choose install scopes. Running in non-interactive mode falls back to listing plugins as before. Thanks to @rakesh-vs (#152).
- An agent that does not declare `tools:` now installs without a tool allowlist, so pi-subagents grants its default tools -- matching Claude Code, which grants every tool available to subagents when `tools:` is omitted. The old `read,bash,edit` default locked converted agents out of the rest of their toolset. `disallowedTools` on such an agent now emits `excludeTools` (pi-subagents 0.62.0 or newer; older versions ignore the field, and the conversion warns about the floor), a disallowed name with no Pi mapping warns instead of vanishing, and `Skill` counts as granted unless disallowed. Thanks to @kevinkirkup (#179).
- Dropped agent fields `allowed-tools`, `mcpServers`, `permissionMode`, and `hooks` now each warn with an explanation instead of only a bare `dropped fields:` note. Claude Code ignores all four on plugin agents, so the drop matches upstream; the `mcpServers` warning names the pi-subagents agent-overrides settings key that grants the agent MCP tools.
- Plugins that keep agents in more than one directory now install every agent. Install, update, and reinstall used to read one directory and silently drop the rest.
- A failure partway through a command no longer leaves half-applied state behind or hides the original error. This covers an ambiguous marketplace alias, a malformed `mcpServers` entry, cleanup after a failed update, and a discovery failure in one scope.
- A path that escapes its allowed root through normalization or a symlink is now rejected before any read or write. A hook `if` rule no longer matches inherited keys such as `__proto__`.
- `PreCompact` and `PostCompact` hooks now receive `trigger: "manual"` for a manual compaction and `"auto"` for threshold or overflow. Every compaction used to report the same value.
- Notifications now say "1 plugin" or "3 plugins" based on what you asked for, not on how many results came back. A bulk command that matched nothing reports zero instead of staying silent.
