...
- The read-only "next reload diff" command (introduced in unreleased 0.5.0 as `/claude:plugin preview`) ships as `/claude:plugin pending`. Output rows are unchanged; the empty-steady-state advisory reads `Pending: next reload will apply 0 actions.`. No user-facing migration — npm 0.4.3 never carried the prior name.
