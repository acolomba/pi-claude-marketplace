...
| `Stop` matcher (none upstream)                                                                                                   | ✓           | ✗   | non-empty matcher is a reported `no-matcher-support` drop (UserPromptSubmit precedent)                                                                                                                                                                                           |
| `StopFailure` error-type matcher                                                                                                 | ✓           | ✓   | closed 10-value set: `rate_limit`, `overloaded`, `authentication_failed`, `oauth_org_not_allowed`, `billing_error`, `invalid_request`, `model_not_found`, `server_error`, `max_output_tokens`, `unknown`; exact-match charset (letters, digits, `_`, `\|`), no pipe-OR splitting |
