...
type PluginNotificationMessage =
  ...
  | PluginUnsupportedMessage // status: "unsupported";  reasons (required); NO scope (SNM-11); ⊖ (USTAT-01)
...
- `reasons: readonly Reason[]` REQUIRED only on `unsupported | unavailable | upgradable | skipped | failed | manual recovery` (D-15-01). The other variants omit the field so `(installed) {up-to-date}` is a compile error.
...
- `scope?: Scope` on every variant EXCEPT `available | unsupported | unavailable` (SNM-11 -- MSG-PL-6 carve-out preserved structurally; the list surface does not emit `[<scope>]` brackets for those rows).
...
Alongside the status-derived ladder, `computeSeverity` MAX-reduces any caller-stamped `row.severity` field (SEV-02): a row may carry an explicit `severity: "error"` that elevates the notification even when its status is not itself in the ladder's error set. This is how install-failure rows route to error. A no-`--force` install failure stamps `severity: "error"` on the row for BOTH arms (D-70-02): the force-degradable arm renders the resolver-state-driven `(unsupported)` token (`PluginUnsupportedMessage`, XSURF-01 -- consistent with how `list` / `info` describe the same plugin) and carries the `--force` hint trailer (force can degrade-install the supported components), while the structural arm renders `(unavailable)` (`PluginUnavailableMessage`) and carries NO hint (force cannot degrade-install a structural defect). The SAME `PluginUnsupportedMessage` / `PluginUnavailableMessage` variants on the list / info surfaces omit `severity` (and `forceHint`) and render **info** byte-frozen -- the per-row caller-stamped severity is the discriminator between the install-failure surface and the inventory surface, not the status token. The `--force` hint trailer byte form is FROZEN (D-70-01): `Re-run with --force to install the supported components.` -- this exact string is the locked DOC contract; the renderer literal in `shared/notify.ts` (`FORCE_INSTALL_HINT_TRAILER`) and the catalog-UAT gate assert against it byte-for-byte. The update-decline surface (XSURF-03) carries the update-worded analog, also FROZEN: `Re-run with --force to update with the supported components.` (`FORCE_UPDATE_HINT_TRAILER`), gated on `PluginForceUpgradableMessage.forceHint` so the list-inventory `force-upgradable` row stays byte-frozen.
