...
- `dependencies: readonly Dependency[]` REQUIRED on `installed | updated | reinstalled` (D-15-02 + SNM-06) and OPTIONAL on `partially-installed` (WR-03), which carries it on the install / update / enable success rows and omits it on the list / info inventory rows so those emit no marker. The other 15 of the 19 `PLUGIN_STATUSES` members omit the field entirely and cannot emit a soft-dep marker structurally; only those 4 switch arms reach the per-dependency probe path.
