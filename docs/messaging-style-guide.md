...
- [`extensions/pi-claude-marketplace/shared/notification-types.ts`](../extensions/pi-claude-marketplace/shared/notification-types.ts) -- the closed-set tuples and discriminated notification unions, including `REASONS` (the closed-set tuple consumed by the 15 reason-bearing plugin variants), `PLUGIN_STATUSES`, and `MARKETPLACE_STATUSES`.
