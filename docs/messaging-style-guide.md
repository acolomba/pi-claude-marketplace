...
Alongside the status-derived ladder, `computeSeverity` MAX-reduces any caller-stamped `row.severity` field (SEV-02): a row may carry an explicit `severity: "error"` that elevates the notification even when its status is not itself in the ladder's error set. This is how install-failure `unavailable` rows route to error. A no-`--force` install of an `unavailable` plugin stamps `severity: "error"` on the row for BOTH arms (D-70-02): the force-degradable `unsupported` arm ALSO carries the `--force` hint trailer (force can degrade-install the supported components), while the structural arm carries NO hint (force cannot degrade-install a structural defect). The SAME `PluginUnavailableMessage` variant on the list surface omits `severity` and renders **info** -- the per-row caller-stamped severity is the discriminator between the install-failure surface and the list surface, not the status token. The `--force` hint trailer byte form is FROZEN (D-70-01): `Re-run with --force to install the supported components.` -- this exact string is the locked DOC contract; the renderer literal in `shared/notify.ts` (`FORCE_INSTALL_HINT_TRAILER`) and the catalog-UAT gate assert against it byte-for-byte.
