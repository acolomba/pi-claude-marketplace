...
export type NotificationMessage; // cascade or standalone discriminated union
export type MarketplaceNotificationMessage; // { name; scope; status?; details?; plugins }
export type PluginNotificationMessage; // discriminated union on `status`
export type PluginStatus; // literal strings derived from PLUGIN_STATUSES tuple
...
The discriminated `PluginNotificationMessage` union pins each variant's `status` to its literal string for TypeScript narrowing (read `extensions/pi-claude-marketplace/shared/notification-types.ts` for the canonical membership; do not re-enumerate a count in prose):
...
type PluginNotificationMessage =
  ...
  | PluginUninstalledMessage // status: "uninstalled"; reasons? for prune and data disposition
  ...
  | PluginWillUninstallMessage // status: "will uninstall"; pending-tense (DIFF-02); NO version; reasons?
...
- `REASONS` -- the closed set of reason tokens used inside `{<reason>}` braces, defined in `extensions/pi-claude-marketplace/shared/notification-types.ts::REASONS`. Standalone prune reuses `dependency pruned`; it adds no token. `authentication required` (D-76-08) is the failure-class token for an HTTP auth challenge (401/403) on a marketplace clone: **error** severity, with the HTTP detail in the cause chain. Reused by `PROV-04`'s fail-clean provider-auth case.

The discriminated union and the per-variant field carve-outs are the binding compile-time contract. Adding or removing a variant, or shifting a field's required/optional discipline, is enforced by the renderer's exhaustive `switch` with no `default` arm (SNM-17; `@typescript-eslint/switch-exhaustiveness-check` rejects a missing arm and a redundant `default`), by the per-command `satisfies CommandContext` checks in the `*.messaging.ts` modules, and by the `_UncoveredReason` / `_ExtraReason` closed-set membership proof in `extensions/pi-claude-marketplace/shared/notify-reasons.ts`; the closed-set tuple lengths are tripwired by `tests/architecture/notify-closed-set-locks.test.ts`. A drift from one of the tuples or the per-variant discipline becomes a compile error or a failing length lock:

- `reasons: readonly ContentReason[]` is REQUIRED on eight variants: `partially-available | unavailable | upgradable | skipped | failed | manual recovery | partially-installed | partially-upgradable`. It is OPTIONAL on eight: `installed | updated | reinstalled | uninstalled | disabled | available | remote | will uninstall`. The three remaining pending variants -- `will install | will enable | will disable` -- have no reason field. A standalone prune preview supplies `dependency pruned` to `will uninstall`, yielding `(will uninstall) {dependency pruned}`. Ordinary pending reconciliation and marketplace removal omit the field, yielding bare `(will uninstall)`; both are versionless. A completed prune supplies the same reason to `uninstalled`, yielding `(uninstalled) {dependency pruned}` with the recorded version.
...
- **Marketplace-header cascade form.** Every cascade output begins with a marketplace header at column 0; plugin rows are indented two spaces beneath. A single-plugin install uses the same form. Standalone informational messages, including the scoped `Nothing to prune` result, render their own body without a marketplace header.
...
- **Standalone prune preview and empty result.** A `prune --dry-run` row is versionless `○ <name> (will uninstall) {dependency pruned}`. It has no reload trailer because no removal has happened. A completed prune uses `○ <name> v<version> (uninstalled) {dependency pruned}` and carries the reload trailer when a member was removed. If either command finds no orphaned dependency installs, it emits `Nothing to prune in <scope> scope: no orphaned dependency installs were found.` as an informational standalone message, with no marketplace row or reload trailer (D-12-01 / D-12-02). The preview reserves nothing; concurrent changes or a member failure can make a later actual result differ.
...
- **Cross-marketplace cause.** The existing `{cross-marketplace}` reason marks the blocked dependency's failed row. Its `cause:` names that dependency, its immediate declarer, the target marketplace, and the marketplace whose policy refused the edge. For a direct install, the named policy root is the requested plugin's marketplace, even across an intermediary. For a reload's original missing edge, the cause uses the first eligible original declarer as a stable example; any eligible declarer can grant permission. If the missing dependency declares another foreign dependency, its own marketplace becomes that cascade's policy root. The cause gives two remedies: install the blocked dependency by name, or edit `allowCrossMarketplaceDependenciesOn` in the named policy root's `marketplace.json`.
...
- **No caller-supplied top-level free text.** `NotificationMessage` has no field for free-text preambles, anchors, or diagnostic augmentations. Standalone kinds such as `prune-empty` select fixed renderer-owned sentences. The `(no marketplaces)` sentinel represents an empty cascade `marketplaces: []` (per D-16-17); an empty per-marketplace `plugins: []` renders the bare header alone (per D-15-08).
...
- [`extensions/pi-claude-marketplace/shared/notification-types.ts`](../extensions/pi-claude-marketplace/shared/notification-types.ts) -- the closed-set tuples and discriminated notification unions, including `REASONS`, `PLUGIN_STATUSES`, and `MARKETPLACE_STATUSES`.
