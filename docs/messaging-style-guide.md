...
- `Dependency` -- the closed set of 3 soft-dependency probe targets, declared as a literal union in `extensions/pi-claude-marketplace/shared/concerns/soft-dep.ts`. Nothing iterates the members at runtime, so the union is the sole declaration site and carries no companion tuple. Drives the render-time probe path; `agents` → `pi-subagents`, `mcp` → `pi-mcp-adapter`, `workflows` → `pi-dynamic-workflows`. The marker for the third target is spelled `requires pi-dynamic-workflows` and never `requires pi-workflows`, which is the npm name of a different package.
