...
| `timeout`       | number (seconds)                                           | optional | Defaults vary by `type`: command/http/mcp_tool = 600s; prompt = 30s; agent = 60s. Per-event reduction: UserPromptSubmit lowers default to 30s; MessageDisplay to 10s; SessionEnd hooks share a 1.5s budget, raised to match a longer per-hook `timeout` up to 60s (re-checked 2026-08-19). | Wall-clock timeout for the handler.                                                                                                                                                                                                                                   | **IMPLEMENT** -- EXEC-02 already covers this. NOTE: REQ EXEC-02 currently specifies a 60s bridge default; upstream default for `command` is 600s. Decision required at implementation time -- see § 9.                                                                                                                                                                                                                                                                                                                                                          |
