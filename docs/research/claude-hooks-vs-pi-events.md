...
- **Naive 1:1 mapping is misleading.** Direct correspondence: 10 exact, 4 partial, 16 with no Pi analog. The 16 ○ events conflate four different situations -- see [Perfect-fidelity feasibility](#perfect-fidelity-feasibility).
...
| `SubagentStop`        | --                                                   | ○        | Same.                                                                                                                                                                                                                                                                                                                                        |
| `TaskCreated`         | --                                                   | ○        | No canonical Pi task primitive. **Bucket H** (semantically inapplicable, silently dropped). rpiv-todo and pi-crew are competing extension takes; Claude's TaskCreate semantics don't map.                                                                                                                                                    |
| `TaskCompleted`       | --                                                   | ○        | Same. **Bucket H** (semantically inapplicable, silently dropped).                                                                                                                                                                                                                                                                            |
| `Stop`                | `agent_settled`                                      | ●        | Dispatched off `agent_settled` with full decision control: block re-entry via `sendMessage(..., { deliverAs: "followUp", triggerTurn: true })`, exit-2, `continue: false` precedence, and an 8-block cap. The only divergence is a non-hook-observable turn-boundary timing shift. See [issue-103](issue-103-stop-stopfailure-promotion.md). |
| `StopFailure`         | `agent_settled` (`stopReason` gate)                  | ●        | `error`/`length` endings surface as the final assistant message's `stopReason`; the error type is classified from `errorMessage` into the closed 10-value vocabulary. Observation-only (no decision control). See [issue-103](issue-103-stop-stopfailure-promotion.md).                                                                      |
...
| ◐ Partial / lossy mapping        | 4     | UserPromptExpansion, PostToolBatch, MessageDisplay, FileChanged                                                                                                                                                                               |
...
Note: earlier drafts split `Stop` across the ● and ◐ rows because the two plugin-author intents Claude wraps in the same event -- "observe that the agent stopped" and "veto the stop and feed back into the conversation" -- looked separable, with only the observe half mapping to the then-current `agent_end`. Both intents now ship: `Stop` is dispatched off `agent_settled` with full block-to-continue decision control, so it sits in the ● row (see [issue-103](issue-103-stop-stopfailure-promotion.md)). `StopFailure` -- earlier grouped in the ◐ row as an `after_provider_response` synthesis -- also now ships off `agent_settled`, classifying error/length endings from the settle-time `stopReason`, so it joins the ● row alongside `Stop`.
...
| `after_provider_response` | No analog -- Claude controls the provider transport internally. (`StopFailure` classifies error endings from the settle-time `stopReason` / `errorMessage`, not from this event.) |
...
| ---------------------------------------------------------- | ----- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **A. Direct 1:1 mapping**                                  | 10    | none                | SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, PostToolUseFailure, PreCompact, PostCompact, SessionEnd, Stop, StopFailure |
| **B. Synthesize from Pi primitives -- stable**             | 1     | none                | FileChanged                                                                                                                         |
| **C. Bridge ships the underlying feature**                 | 0     | none                | (empty in v1.13 scope -- all candidates moved to G or H by the "would-anybody-naturally-invoke-this" filter)                        |
| **D. Synthesize -- lossy or future-fragile**               | 3     | none                | CwdChanged, PostToolBatch, UserPromptExpansion                                                                                      |
...
| **H. Semantically inapplicable to Pi** -- silently dropped | 5     | n/a                 | ConfigChange, Setup, InstructionsLoaded, TaskCreated, TaskCompleted                                                                 |

**Totals (after the soft-dep audit moved `SubagentStart` / `SubagentStop` from F to A/B-conditional; after `WorktreeCreate` / `WorktreeRemove` moved from C to G; and after `TaskCreated` / `TaskCompleted` moved from C to H):**

- **14 events shippable today via direct map or synthesis** (A + B + D), plus **2 more conditional on `pi-subagents` being installed** (the soft-dep entries from `SubagentStart` and `SubagentStop`) → **16 supported in v1.13**.
...
- **5 events are semantically inapplicable** (H): no upstream PR would unlock them because Pi doesn't share the runtime concept they're named for. Silently dropped at install time with a one-line debug log; no per-hook warning at install (would be noise).
- **Bucket C ends up empty.** The framework still describes a real category (events that fire from a bridge-shipped feature), but no v1.13 event passes the "would-anybody-naturally-invoke-this" filter: worktrees need a soft-dep with an upstream PR (bucket G); tasks have no canonical Pi concept (bucket H). Bucket C remains a valid category for future events that might land there.

### How each bucket B/C/D synthesizes

The B/D divide is defined by **future-stability**: bucket B is reserved for syntheses that cannot be broken by future Pi runtime additions; bucket D covers syntheses that work under current Pi assumptions but could quietly drift if Pi grows new tools, transform stages, or execution patterns. This makes "bucket B" mean something stronger than "bucket D" -- a reader can trust B without revisiting it as Pi evolves.

**B -- Synthesize from Pi primitives (stable):**
...
`fs.watch` semantics are runtime-agnostic; the synthesis is structurally complete and won't break as Pi adds new tools or features.

**C -- Bridge ships the underlying feature (empty in v1.13):**

No events land here in the v1.13 scope. The structural candidates (`TaskCreated` / `TaskCompleted` and `WorktreeCreate` / `WorktreeRemove`) all failed the "would-anybody-naturally-invoke-this" filter:

- Worktree pair → bucket G (use `pi-worktrees` as the soft dep + upstream PR to publish events, instead of the bridge shipping circular `/worktree create` commands nobody would invoke).
- Task pair → bucket H (Pi has no canonical task primitive -- rpiv-todo and pi-crew represent competing extension takes; Claude's TaskCreate semantics don't map to either).

Bucket C remains a valid theoretical category for future events that might land there.

**D -- Synthesize, lossy or future-fragile:**

| Claude event               | Synthesis approach                                                                                                                                                                                                                                                                                                                                                                                                          | Loss mode                                                                                                                                                                                                                                                           |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CwdChanged`               | Watch `tool_result` for `bash` tool calls matching `cd` patterns; compare resulting `ctx.cwd` against last known. Today bash is the only cwd-changing tool in Pi.                                                                                                                                                                                                                                                           | **Future-fragile + parse-fragile.** False negatives if Pi grows non-bash cwd-changing tools (silent miss). Also fails for `cd` chained in compound shell commands the bridge doesn't parse. Plugin author would see hooks silently not firing for some cwd changes. |
| `PostToolBatch`            | Count `tool_execution_end` events against the assistant message's tool-call count; fire when the count is reached and before the next LLM call (catch with `turn_start` or `before_provider_request`). Fire from `turn_end` as a safety net.                                                                                                                                                                                | **Timing approximate + future-fragile.** Races if a tool errors out before emitting `tool_execution_end` (the count never reaches expected; safety-net fire covers it). Future parallel-execution patterns could break the count.                                   |
| `UserPromptExpansion`      | Capture `input` event's `text` field; compare against `before_agent_start.prompt`; if they differ AND `input.text` started with `/`, an expansion happened. Fire the synthesized event between the two.                                                                                                                                                                                                                     | **False positive + future-fragile.** Fires for ANY prompt rewrite (e.g., another extension's `input` transform), not just slash-command expansion. Future input-transform stages could mask real expansions.                                                        |
| `Stop` (block-to-continue) | **Promoted to bucket A -- shipped, not synthesized.** Dispatched off `agent_settled`: a `{"decision": "block", "reason": ...}` (or exit-2) re-enters via `sendMessage(..., { deliverAs: "followUp", triggerTurn: true })` with the reason as content; `continue: false` takes precedence; an 8-block cap bounds the loop. See [issue-103](issue-103-stop-stopfailure-promotion.md).                                         | **Timing shift only (not hook-observable).** Upstream folds the continuation into the same turn; under Pi re-entry starts a new turn. Same payload, flag cadence, and cap; the transcript shows one extra turn boundary.                                            |
| `StopFailure`              | **Promoted to bucket A -- shipped, not synthesized.** Rides the same `agent_settled` subscriber, gated on the final assistant message's `stopReason` (`error`/`length`); the error type is classified from `errorMessage` into the closed 10-value vocabulary (`length` → `max_output_tokens`), not from `after_provider_response` HTTP status. Observation-only. See [issue-103](issue-103-stop-stopfailure-promotion.md). | **None.** No decision control to lose; the `stopReason` contract is a documented provider protocol, not a fragile synthesis.                                                                                                                                        |
...
### H -- Semantically inapplicable to Pi (5)
...
| `TaskCreated` / `TaskCompleted` | Claude's LLM-driven Task tool firing when a sub-task context is created/completed. Includes Claude-specific isolation, subagent linkage, and lifecycle.                      | Pi has no canonical task primitive. Two extensions (`@juicesharp/rpiv-todo`, `pi-crew`) implement task-like surfaces with different semantics (kanban-style work tracking vs multi-agent orchestration); neither maps to Claude's Task model. Picking one as "the" Pi task surface would be a favoritism call; using the bridge to ship its own would be circular (the only invoker would be the bridge). Drop honestly rather than fire with mismatched semantics. |
...
### Synthesis caveats (test coverage)

These are the same failure modes recorded in the bucket-D table above; this section is the operational view -- what test coverage and documentation the bridge needs to ship to make the loss visible rather than silent.

| Path                                                                                | Failure mode + required mitigation                                                                                                                                                                                                                                                                          |
| ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Counting `tool_execution_end` to derive `PostToolBatch`                             | Races if a tool errors out before emitting `tool_execution_end` (the count never reaches expected). Bridge needs a timeout + safety-net fire from `turn_end`. **Tests need cancellation-mid-batch coverage.**                                                                                               |
| Diffing `input.text` vs `before_agent_start.prompt` to derive `UserPromptExpansion` | Fires for *any* prompt rewrite between `input` and `before_agent_start`, including transforms from other extensions. Document as best-effort. **Tests should cover both real expansion and inter-extension-transform paths to make the false-positive visible.**                                            |
| Watching `bash` tool results to derive `CwdChanged`                                 | Misses cwd changes via non-bash tools (none in Pi today, but a future addition would silently break the hook). Also misses `cd` chained in compound commands. **Document scope clearly. Tests should pin "bash-only" as the contract so a future tool addition surfaces as a test gap, not a silent miss.** |

### Path forward

Assuming we don't want to wait on Pi upstream:

1. **V1 bridge ships buckets A + B + D + soft-dep `SubagentStart` / `SubagentStop`.** That's **16 of 30 Claude events** at perfect or near-perfect fidelity. (Bucket C is empty.) The empirical marketplace audit (next section) shows the first-party catalog uses only 5 distinct events -- all in A or D -- so even a minimum-viable V1 limited to `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop` covers 100% of Anthropic's plugins. Shipping all of A + B + D + soft-dep buys forward-compatibility for third-party plugins at modest extra cost.
2. **`Stop` ships as bucket A with full decision control.** Three first-party plugins hook it and depend on the block-to-continue JSON contract; the shipped `agent_settled` dispatcher round-trips `{"decision": "block", "reason": "..."}` (with exit-2, `continue: false` precedence, and the 8-block cap). `ralph-wiggum` is the canary -- if it works, the contract holds. See [issue-103](issue-103-stop-stopfailure-promotion.md).
3. **5 bucket-H events are silently dropped** (`ConfigChange`, `Setup`, `InstructionsLoaded`, `TaskCreated`, `TaskCompleted`). Debug-log only at parse time; no install warning. Don't burn implementation effort here; document the rationale in the bridge README.
...
- **Buckets:** Stop → A
- **Verdict:** FULL SUPPORT
- **Notes:** Uses `Stop`'s block-to-continue semantics to drive a self-referential loop where Claude re-runs the same task until completion. The shipped `Stop` dispatcher surfaces the `{"decision": "block", "reason": "..."}` JSON contract off `agent_settled`; `ralph-wiggum` is the canary for that contract.
...
- **Buckets:** all → A
- **Verdict:** FULL SUPPORT
- **Notes:** No matchers -- handlers run on every event of each kind and self-filter inside the Python handler. Same Stop block-to-continue dependency as `ralph-wiggum`, now shipped.
...
| ralph-wiggum             | 1          | A            | FULL    | none (timing shift)           |
| hookify                  | 4          | A            | FULL    | none (timing shift)           |
| security-guidance        | 4          | A            | PARTIAL | asyncRewake payload extension |

### Findings

- **The first-party catalog exercises only 5 distinct hook events:** SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop. All five are in bucket A -- i.e. supportable with no Pi runtime changes.
- **None of the 9 upstream-fixable blocker events are used, and none of the 5 H-bucket silently-dropped events are used either.** Notification, PermissionRequest, PermissionDenied, MessageDisplay, TeammateIdle, Elicitation, ElicitationResult, WorktreeCreate, WorktreeRemove (E + F + G); ConfigChange, Setup, InstructionsLoaded, TaskCreated, TaskCompleted (H): all zero usage in the first-party marketplace. The unsupported events exist but the official catalog doesn't surface any of them as user-visible breakage.
- **`Stop` ships as bucket A with full decision control.** 3 of 5 hook-using plugins (`ralph-wiggum`, `hookify`, `security-guidance`) hook Stop; the `agent_settled` dispatcher honors the block-to-continue contract, so it is no longer a lossy synthesis. See [issue-103](issue-103-stop-stopfailure-promotion.md).
...
- Buckets A + B + D + soft-dep: **16 of 30 events** supported in v1.13. Bucket C is empty.
...
- Semantically-inapplicable (H): **5 events** silently dropped. No upstream PR would unlock them -- Pi's runtime model doesn't share the semantics they're named for.
...
The current answer to "perfect fidelity?" is: **16/30 today** (conditional on the already-installed soft dep) plus 4 small Pi runtime PRs, 3 soft-dep extension PRs (2 to pi-mcp-adapter + 1 to pi-worktrees), and 1 large Pi feature addition (`TeammateIdle` / agent teams) for everything except the 5 H-bucket events, which are permanently out of scope.
