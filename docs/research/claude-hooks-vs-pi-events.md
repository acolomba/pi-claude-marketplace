...
- **What the bridge can actually ship today.** With synthesis inside the bridge plus the project's existing soft-dep extensions (`pi-subagents`, `pi-mcp-adapter`), **20 of 30 events become supportable at perfect or near-perfect fidelity with zero pi-coding-agent runtime changes**. Buckets A + B + C + D cover 18 events directly; `SubagentStart` and `SubagentStop` lift from bucket F to A/B when `pi-subagents` is installed (see [Soft-dep extension event surfaces](#soft-dep-extension-event-surfaces)).
- **Five upstream-fixable blockers** would need PRs to `pi-coding-agent` or `pi-mcp-adapter` to unlock: 4 events need pi-coding-agent to expose internal state (`Notification`, `PermissionRequest`, `PermissionDenied`, `MessageDisplay`); 1 event (`TeammateIdle`) needs Pi to add an agent-team primitive; 2 more events (`Elicitation`, `ElicitationResult`) are blocked on two specific PRs to `pi-mcp-adapter`.
...
- **Empirical check against the official Anthropic marketplace: 5/5 hook-using plugins are supportable** under the bucket A + B + C + D scope (4 fully, 1 partially). **None of the 10 unsupported events are exercised by any first-party plugin** -- neither the 7 upstream-fixable blockers nor the 3 H-bucket inapplicable events. The blockers are real but not market-driven for the first-party catalog.
...
| **G. pi-mcp-adapter PRs required** -- upstream-fixable     | 2     | needed              | Elicitation, ElicitationResult (handler registration + `pi.events` publication)                                  |
| **H. Semantically inapplicable to Pi** -- silently dropped | 3     | n/a                 | ConfigChange, Setup, InstructionsLoaded                                                                          |

**Totals (after the soft-dep audit moved `SubagentStart` / `SubagentStop` from F to A/B-conditional):**

- **18 events shippable today via direct map or synthesis** (A + B + C + D), plus **2 more conditional on `pi-subagents` being installed** (the soft-dep entries from `SubagentStart` and `SubagentStop`) → **20 supported in v1.13**.
- **7 events are upstream-fixable** blockers (E + F + G): 4 need `pi-coding-agent` runtime exposures, 1 needs a new Pi agent-team primitive, 2 need specific `pi-mcp-adapter` PRs.
...
**G -- `pi-mcp-adapter` PRs required (2):**

| Claude event                        | What pi-mcp-adapter has today                                                                                                                                                                | What's missing                                                                                                                                                                 | Likely change                                                                                                                                                                                                                | Size                                                |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `Elicitation` / `ElicitationResult` | Registers only `CreateMessageRequestSchema` (sampling) at `sampling-handler.ts:26`; never imports `ElicitRequestSchema`; zero `pi.events.emit` / `pi.events.on` anywhere in the source tree. | An `ElicitRequestSchema` handler in `server-manager.ts` (mirroring the sampling handler) plus event publication on the shared `pi.events` bus so other extensions can observe. | (a) Register handler + bridge through `ctx.ui` (small, sampling-handler is the template). (b) Publish `pi.events.emit("mcp:elicit-request" / "mcp:elicit-result", ...)` from that handler (trivial once the handler exists). | Small + trivial -- two cleanly-scoped additive PRs. |

Confirmed by the [Soft-dep extension event surfaces](#soft-dep-extension-event-surfaces) audit. Until those PRs land, no bridge code can observe MCP elicitation under Pi.
...
4. **7 upstream-fixable events are documented as known limitations** in the bridge README. Plugin authors are told: if your plugin hooks `Notification` / `PermissionRequest` / `PermissionDenied` / `MessageDisplay` / `TeammateIdle` / `Elicitation` / `ElicitationResult`, that hook will not fire under Pi today. The marketplace audit shows zero first-party plugins use any of these, so the documentation matters more than the runtime impact in the short term.
5. **Open 4 issues upstream in `pi-coding-agent`** for the E-bucket runtime exposures. Each is a small-to-medium patch (Notification looks smallest). If accepted, the bridge moves them out of the blocker list version-by-version, gated on the peer-dep floor. Urgency is low until a third-party plugin actually exercises one.
...
7. **G-bucket: open 2 PRs against `pi-mcp-adapter`** to register the `ElicitRequestSchema` handler and publish `mcp:elicit-request` / `mcp:elicit-result` on `pi.events`. Both are additive and mechanically small. Out of scope for v1.13 itself; tracked separately.
...
- **None of the 7 upstream-fixable blocker events are used, and none of the 3 H-bucket silently-dropped events are used either.** Notification, PermissionRequest, PermissionDenied, MessageDisplay, TeammateIdle, Elicitation, ElicitationResult (E + F + G); ConfigChange, Setup, InstructionsLoaded (H): all zero usage in the first-party marketplace. The unsupported events exist but the official catalog doesn't surface any of them as user-visible breakage.
...
- Buckets A + B + C + D + soft-dep: **20 of 30 events** supported in v1.13.
- Upstream-fixable blockers (E + F + G): **7 events** -- 4 pi-coding-agent runtime exposures, 1 Pi product feature addition (`TeammateIdle`), 2 specific pi-mcp-adapter PRs.
...
The current answer to "perfect fidelity?" is: **20/30 today** (conditional on the already-installed soft dep) plus 4 small Pi runtime PRs, 2 pi-mcp-adapter PRs, and 1 large Pi feature addition (`TeammateIdle` / agent teams) for everything except the 3 H-bucket events, which are permanently out of scope.
