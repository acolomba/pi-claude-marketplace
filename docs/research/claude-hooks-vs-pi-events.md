...
| -------------------------------- | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ● Exact or near-exact mapping    | 9     | SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, PostToolUseFailure, PreCompact, PostCompact, SessionEnd, Stop                                                                                                                        |
...
Note: earlier drafts split `Stop` across the ● and ◐ rows because the two plugin-author intents Claude wraps in the same event -- "observe that the agent stopped" and "veto the stop and feed back into the conversation" -- looked separable, with only the observe half mapping to the then-current `agent_end`. Both intents now ship: `Stop` is dispatched off `agent_settled` with full block-to-continue decision control, so it sits in the ● row alone (see [issue-103](issue-103-stop-stopfailure-promotion.md)).
