...
| `(uninstalled)`          | ○    | Plugin row -- uninstall single-plugin, marketplace-remove partial success rows. It admits exactly one token, the data-disposition marker `{data kept}`, stamped by `uninstall --keep-data` (DATA-01 / WR-06); a bare row means the plugin's data directory went with it. `marketplace remove` has no such opt-out, so its rows are always bare.                                                                                                                                                                                             |
