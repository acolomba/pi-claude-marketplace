...
- `{<reasons>}` -- single brace block, comma-space separated, emitted only on the 6 reason-bearing variants (`partially-available | unavailable | upgradable | skipped | failed | manual recovery`) and only when the composed reasons list is non-empty.
...
Triggered when `resolveStrict` returns `installable: false` for the plugin entry. USTAT-01 / D-64-01: the not-installed info row now de-collapses by resolver STATE, byte-consistent with the list surface. A STRUCTURALLY-malformed plugin (a `hooks/hooks.json` that is invalid JSON, or a schema failure such as a `type:"command"` handler missing its `command`; an unreadable / non-path source) resolves `unavailable` and renders the `⊘` glyph with `(unavailable)`. A partially-available plugin (a path-resolvable manifest declaring an unsupported component such as `lspServers`, or a parseable-but-unsupportable `hooks.json` -- a non-bucket-A event or unsupported matcher group) resolves `partially-available` and renders the `⊖` glyph with `(partially-available)` (D-71-03 / PHOOK-03); once installed it derives `partially-installed`. Each row is followed by a closed-set REASON brace (`{unsupported hooks}` / `{lsp}` / `{unsupported source}`; the structural arm sources reasons via `narrowResolverNotes`, the partially-available arm via `narrowUnsupportedKinds`). The example below is the STRUCTURAL malformed-`hooks.json` case, so it keeps `⊘ (unavailable)`. The renderer's `componentsResolved: false` switch arm fires for the not-installable arms, emitting the `components: not resolved` marker line in place of per-kind component lists -- the plugin is not installable so its component layout is moot. Severity `info` (neither `partially-available` nor `unavailable` is a failure on the info surface; only `failed` routes to error).
