...
- [`docs/prd/pi-claude-marketplace-prd.md`](prd/pi-claude-marketplace-prd.md) §6.12 ES-5 -- the stable user-contract strings origin; the 5 ES-5 markers were superseded by the v1.3 style guide and no renderer arm emits one. No dedicated gate blocks their reintroduction: the suite that pinned the literals was retired with the V1 wrappers and nothing replaced it, so a reintroduction is caught only where `tests/architecture/catalog-uat.test.ts` already records the row byte-for-byte.
