...
Claude Code reports this same fallback as a warning. This extension reports it at the same severity, because the requesting plugin installed against a dependency at an unverified version; the [load-time check](#the-load-time-check) is what later decides whether anything is actually wrong. That check runs on every reload afterward: it disables the dependent if the fallback copy really is out of range, and lifts the disable again once it is not (D-07-03).
