...
`pi-claude-marketplace` is a Pi extension that gives Pi users access to Claude plugin marketplaces using a command surface intentionally aligned with Claude Code's upstream `/plugin`. The extension translates Claude plugin artifacts (skills, commands, agents, MCP servers) into the equivalent Pi-native artifacts (Pi skills, Pi prompt templates, pi-subagents agents, pi-mcp-adapter MCP entries) and manages the lifecycle of those translations (install, update, uninstall, remove, list).
...
- **Atomic, recoverable operations** -- install, update, uninstall, and marketplace removal must never leave on-disk artifacts and persisted state in irrecoverable disagreement; failures must surface a recovery path.
...
| **Project lead**            | Curate a per-project marketplace set so the team gets the same skills/commands | Add a marketplace to project scope; commit `.pi/` artifacts; document a `pi install` flow                                                        |
...
- **Target scope** -- the scope where a mutating plugin operation writes or changes the plugin install record and generated Pi artifacts.
...
- **Unavailable** -- the resolver's structural-defect arm (`unavailable`): the plugin cannot be installed even with `--partial` (e.g., `unsupported source kind: github`, a missing source directory, a malformed manifest, an NFR-10 containment violation). On the `list` surface it is the shorthand for these structurally non-installable plugins; the row surfaces the structural reason as a note (RSTATE-02).
- **Partially-available** -- the resolver's partially-available arm (`partially-available`): the plugin is structurally sound but declares one or more component kinds Pi does not support (e.g., `contains hooks`, `lsp`). Without `--partial` it blocks install/update at `error`; with `--partial` the supported components install and the unsupported ones are skipped (RSTATE-01 / FORCE-01..03). The `partially-available` arm carries `pluginRoot`; the `unavailable` arm exposes it to no consumer (RSTATE-03, NFR-7).
...
The `--partial` flag is a per-invocation opt-in (absent by default) that lets a _partially_-supported plugin install anyway: it stages the supported components and skips the unsupported ones, never blocking. It applies identically on `install` and `update` (§5.2.3) and is the ONLY opt-in to component degradation -- without it, a `partially-available` plugin still fails.

| ID            | Requirement                                                                                                                                                                                                                                                                                           |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **FORCE-01**  | `install --partial` on a `partially-available` plugin installs the supported components and skips the unsupported ones; `--partial` on a fully-supported plugin is a no-op and installs normally as `(installed)`.                                                                                    |
| **FORCE-04**  | No `Warning:` summary is emitted on any `--partial` path -- the explicit `--partial` is the opt-in; the dropped-component detail lives in `info`. A direct partial degrade renders at `info` severity (SEV-01).                                                                                       |
| **FORCE-05**  | `--partial` never bypasses hard failures: a structurally `unavailable` plugin, an NFR-10 path-containment violation, a missing marketplace, or an unresolvable source fail/block regardless of `--partial`.                                                                                           |
| **FSTAT-01a** | A partially-installed plugin's state is DERIVED -- recorded-installed AND currently re-resolving `partially-available` -- with no persisted flag and no migration; it renders with the `partially-installed` status and the `◉` glyph, distinct from `(installed)`'s `●` (FSTAT-01/02).               |
| **FSTAT-03a** | A partially-installed plugin whose newer version is fully supported returns to `(installed)` automatically after upgrade -- no lingering partial state (FSTAT-03).                                                                                                                                    |
| **SEV-02a**   | Installing a `partially-available` plugin WITHOUT `--partial` renders at `error` with the frozen hint `Re-run with --partial to install the supported components.` (SEV-02 / D-70-01); installing a structurally `unavailable` plugin renders at `error` with NO `--partial` hint (SEV-02 / D-70-02). |
...
| **PU-4** | Post-commit data-dir cleanup leaks are intentionally NOT surfaced. The cleanup runs AFTER the durable atomic state save (PU-2), so a leak cannot corrupt state; surfacing it was dropped to reduce notification noise. The `rm` still runs; only the user-facing warning is suppressed. |
...
The `--partial` flag (see §5.2.1) applies to `update` exactly as on `install`: `update --partial` on a plugin whose newer version became `partially-available` updates it by degrading the now-unsupported components instead of failing (FORCE-02). Without `--partial`, a targeted `update` of a `partially-upgradable` plugin declines the degrading upgrade and renders at `warning`; a bulk/untargeted `update` that skips it renders at `info` (SEV-04). The marketplace autoupdate cascade does NOT take a `--partial` flag (see §5.4).
...
| **PUP-3** | If the resolved version equals the recorded version, the plugin MUST be reported `unchanged` (no I/O on disk artifacts).                                                                                                                                                                                                                                 |
...
| **PL-4** | Each plugin entry MUST show its icon (one of `●` / `○` / `⊘` / `⊖` / `◉` / `◌` / `◍` -- the seven `ICON_*` constants `shared/notification-grammar.ts` exports), name, optional `(<version>)`, and a status marker drawn from the closed set `(installed)` / `(partially-installed)` / `(upgradable)` / `(partially-upgradable)` / `(available)` / `(unavailable)` / `(uninstalled)` / `(updated)` / `(reinstalled)` / `(skipped)` / `(failed)` / `(manual recovery)` / `(disabled)` / `(remote)`, optionally followed by a `{reasons}` brace on the reason-bearing variants (including `partially-installed` and `partially-upgradable`, SEV-05). Glyph roles: `●` installed or pending-positive transition (also a clean `partially-upgradable` row, which is clean today); `○` not-installed with no error or pending-removal; `⊘` error/blocked (unavailable, failed, skipped, manual recovery); `⊖` partially-available -- a not-installed plugin whose components would be dropped under `--partial` (USTAT-01); `◉` partially-installed (recorded-installed but currently re-resolving `partially-available`); `◌` a not-installed git-source plugin whose clone is not yet materialized locally (RSTA-01 / D-80-01); `◍` the deliberate, user-requested disabled state, which D-80-01 moved off `◌` when `◌` took the remote state. A description (when present) appears on a second indented line, truncated at column 66. |
| **PL-5** | A plugin is `upgradable` iff the manifest version differs (string compare) from the install record.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **PL-6** | If a marketplace's manifest fails to load (malformed JSON, missing file, permission denied), that marketplace MUST render as the bare `⊘ <name> [<scope>] (failed)` header alone, with no child rows -- neither its own installed records nor the rows folded in from the other scope. BOUND-01 settles this output and the current tests pin it. The block claims nothing about membership, because a manifest that did not load cannot settle whether it declares a plugin; the failed header is the honest statement, and hiding a read failure behind rows the manifest never confirmed is the failure mode BOUND-01 forbids. A marketplace whose manifest DOES load renders its installed records regardless of whether the manifest still declares them (INV-01) -- that, and not a fallback on the failure path, is what keeps installed plugins from silently disappearing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **PL-7** | Per-marketplace headers in the nested tree MUST include the `[autoupdate]` tag when the flag is on.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **PL-8** | `list` gains a `--partial` filter; `--installed` spans BOTH `installed` and `partially-installed`. No `--upgradable` filter is added (LIST-01).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **PL-9** | A recorded-installed plugin that currently re-resolves `partially-available` renders as `partially-installed` with the `◉` glyph (FSTAT-02). An installed CLEAN plugin whose newer no-network cache candidate would NEWLY degrade it renders as `partially-upgradable` with the clean `●` glyph; a partially-installed plugin is never partially-upgradable (FSTAT-04). The candidate version driving `upgradable` / `partially-upgradable` is resolved without network access, from cache (FSTAT-05).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
flowchart TD
  ...
  C --> D[per marketplace: read the manifest<br/>its OWN record names]
  D -- read failed --> F["bare (failed) header, no child rows<br/>BOUND-01"]
  D -- read ok --> L[per installed record:<br/>manifestLookupFor]
  L --> M{ManifestLookup}
  M -- declared --> N["entry drives the PL-5 version compare<br/>and the PL-4 description"]
  M -- absent --> O["(installed) {not in manifest}<br/>INV-01"]
  M -- unverified --> P["bare (installed), no brace<br/>BOUND-03"]
  D -- read ok --> Q[manifest entries with no record:<br/>available / partially-available /<br/>unavailable / remote rows]
  N --> G[render nested tree]
  O --> G
  P --> G
  Q --> G
  F --> G
...
The `unverified` arm is reachable only on the cross-scope fold: the header marketplace read its manifest, but a folded record from the other scope could not read the manifest ITS own record names. A same-scope read failure takes the `BOUND-01` arm instead, so the two failures never produce the same output.
...
**Partially-upgradable behavior in the cascade (SEV-03).** When the per-marketplace `autoupdate` flag is on, the cascade takes a `partially-upgradable` plugin's degrading upgrade automatically -- it is NOT reported as `(skipped) {no longer installable}` for the unsupported-component case. The cascade renders at `warning` only when the upgrade NEWLY degrades a previously-clean plugin, and at `info` when the plugin was already degraded.

**Soft-dependency companion warning scope (WR-01 / D-70-03).** The missing-soft-dep-companion warning (a declared `pi-subagents` / `pi-mcp-adapter` companion that is unloaded -- SEV-01) is raised on `install` and manual `update` success ONLY. It is deliberately NOT added to the marketplace autoupdate cascade: autoupdate severity is governed by SEV-03, and the cascade's actionable signal is the new-degradation warning described above. This scoping is intentional and shipped, not a gap.
...
| **SK-2** | The generated skill name MUST be `<plugin>-<skill>`, with a matching `<plugin>-`, `<plugin>:`, or `<plugin>.` prefix stripped from the source name when present. A source skill name equal to the plugin name becomes `<plugin>`. Skill names MUST satisfy Pi's validator: lowercase `a-z`, `0-9`, and single interior hyphens only, with a 64-character limit. Other source characters become hyphens; a segment with no letters or digits and an overlong name receive deterministic hash suffixes. |
...
| **SK-6** | Loaded marketplace skills MUST offer `/plugin:skill` completion and invocation in interactive Pi sessions after `/reload`. The native `/skill:name` form remains available. An existing command or prompt template with the qualified name takes priority.                                                                                                                                                                                                                                            |
| **SK-7** | The skills, commands, agents, and copied skill Markdown files of an installed plugin MUST rewrite known same-plugin references to Pi invocations. Skills use `/skill:<generated name>`; commands use their generated slash name, with dots on Windows. For an ambiguous source name, a command wins unless the source explicitly uses `/skill:`. Rewrite fenced examples and references without an initial slash. Leave unknown and foreign-plugin names unchanged.                                   |

### 5.6 Commands Bridge (Pi prompt templates)

| ID       | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CM-1** | Commands MUST be staged as `<scope>/pi-claude-marketplace/resources/prompts/<generated name>.md`, where the generated name is the `<plugin>:<command>` form CM-2 defines. A nested source carries one colon per path segment, so `commands/build/web.md` stages as `acme:build:web.md`.                                                                                                                                                                                                                                                                                                                                                                    |
| **CM-2** | Generated command name MUST be `<plugin>:<command>`, with the `<plugin>-` prefix stripped from the head of the source name when present. The head is the first path segment when the source is a nested path, so `acme-tools/lint.md` in plugin `acme` becomes `acme:tools:lint` (D-141-01) where Claude Code registers `acme:acme-tools:lint`; Claude Code performs no elision at all, which is the same divergence CM-2 already carried for flat files. The strip MUST NOT fire when it would leave the head empty, so `commands/acme-.md` becomes `acme:acme-` and `commands/acme-/lint.md` becomes `acme:acme-:lint`, matching Claude Code (D-141-02). |
| **CM-3** | Variable substitution (`${CLAUDE_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_DATA}`) MUST apply to command bodies.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **CM-4** | Discovery MUST enumerate `*.md` files under each `commands/` path **recursively**. A nested file (e.g. `commands/build/web.md`) is discovered with source name `build/web` and generated name `<plugin>:build:web` -- the path separator becomes a colon. Recursion and colon-joining are exact parity to arbitrary depth, measured against Claude Code 2.1.228 rather than taken from its documentation, which calls the field flat. Symlinks (files and directories) MUST be skipped. Dotfile-prefixed entries MUST also be skipped, which is a deliberate divergence: Claude Code registers `commands/.hidden/secret.md` as `acme:.hidden:secret`.      |
...
| **AG-11** | `convertAgent` MUST throw when an explicitly declared `tools:` maps to an empty list (pi-subagents reads an empty allowlist as "no tools", which has no safe representation). The error message MUST list the source `tools:` and `disallowedTools:`. An omitted `tools:` MUST NOT throw -- see the AG-7 `tools:` detail (#179).                                                                                                 |
| **AG-12** | Within a plugin, the first discovered exact source name MUST win; later exact duplicates MUST be skipped with a warning. The warning MUST identify both full source paths and the retained source. Discovery follows resolved directory order and sorts filenames with the existing comparator. Distinct complete source names MUST remain distinct; this directory adapter does not implement custom-file replacement ordering. |

Update and reinstall MUST migrate previous indexed agent names to the complete-source names without changing schemaVersion. They MUST preserve unrelated content and other owners at newly claimed targets. Commit MUST check occupied new targets before removing previous owned files. Reinstall retains its existing permission to overwrite previous indexed plugin targets and restores their bytes if replacement rolls back.
...
| **CMP-2** | Plugin lifecycle operations MUST distinguish target scope from source marketplace scope. The target scope is where plugin state and generated artifacts are written; the source marketplace scope is only where the marketplace manifest/source is read from.                                                                                                                  |
...
| ID       | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **PR-1** | The resolver MUST return a three-way discriminated state -- `installable` / `partially-available` / `unavailable` (RSTATE-01) -- replacing the original binary `installable: true \| false`. The `installable` and `partially-available` arms carry `pluginRoot`; the `unavailable` arm MUST NOT expose `pluginRoot` (RSTATE-03, NFR-7 refined not weakened).                                                                                                                 |
| **PR-2** | The resolver MUST yield `unavailable` for structural defects: non-`path` source kind, source path escaping marketplace root, source directory missing on disk, malformed plugin manifest, malformed `mcpServers`, a component path that's not a non-empty string, a component path escaping the plugin root, an array-form supported-component path. Declared or convention-detected UNSUPPORTED component kinds instead yield the `partially-available` arm (RSTATE-01..02). |
| **PR-3** | An unsupported component name (e.g., hooks) MUST produce a note `contains <name>` and place the plugin in the `partially-available` arm; it disqualifies a default install but is partially installable via `--partial` (RSTATE-04 / FORCE-01).                                                                                                                                                                                                                               |
...
| **PR-7** | A structural defect (unreadable/invalid manifest, malformed `hooks.json`, path / NFR-10 containment violation) MUST yield `unavailable` and take PRECEDENCE over unsupported component kinds -- a plugin that is both broken and partial resolves `unavailable` (RSTATE-02).                                                                                                                                                                                                  |
| **PR-8** | Two narrowing gates MUST exist: `requireInstallable` (→ `installable` only; default path, PR-6) and `requirePartialInstallable` (→ `installable \| partially-available`; the `--partial` path) (RSTATE-04).                                                                                                                                                                                                                                                                   |
| **PR-9** | Unsupported-component reasons MUST be derived per-kind from the component list as a marker family distinct from structural reasons, and MUST be identical across `list`, `info`, and every partial state (RSTATE-05).                                                                                                                                                                                                                                                         |

### 6.5 Resource Naming, Generation & Conflicts

| ID       | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **RN-1** | Generated names MUST be deterministic functions of `(plugin, source-name)`. Skill: `<plugin>-<skill>` (prefix elided). Command: `<plugin>:<command>` (prefix elided). Agent: `pi-claude-marketplace-<plugin>-<agent>` (the complete validated source name is preserved, including any `<plugin>-` prefix).                                                                                                                                                                                                                                                                                                                                                         |
...
| **RN-6** | Within a single plugin, two distinct skill or command source names that elide to the same generated name (e.g., a plugin `foo` with sources `foo-bar` and `bar`) MUST NOT throw. The second occurrence is the D-07 first-wins skip and a warning. That holds for skills (D-141-04) and for commands (D-141-01), where the collision is reachable between two files of one directory as well as between two declared entries. Both warnings name the WINNING source. A source name equal to the plugin name becomes skill `foo` and command `foo:foo`. (For agents, AG-12 skips only exact source-name duplicates; distinct complete source names do not collapse.) |
...
| **AP-5** | Every command MUST reject unknown long flags and surplus positional arguments before dispatch.                                                                                                                                                              |
| **AP-6** | Flag extraction MUST preserve quoted positional text, including explicitly empty arguments, and retain a scope value even when that value spells `--local`. A supplied empty or whitespace-only optional argument MUST be rejected, not treated as omitted. |
...
| **AS-6** | Post-commit cleanup leaks (e.g., uninstall data-dir rm failure, marketplace-update completion-cache refresh failure) are intentionally NOT surfaced. Because the cleanup runs only AFTER the durable atomic state save, a leak cannot corrupt state; the warning surface was dropped to reduce notification noise. The underlying cleanup still runs. |
...
| **ES-6** | The partial-feature severities (SEV-01..05) stamp the partial surfaces. A direct `install --partial` / `update --partial` degrade renders at `info` with no `Warning:` summary (FORCE-04). Installing a `partially-available` plugin WITHOUT `--partial` renders at `error` WITH the frozen hint trailer `Re-run with --partial to install the supported components.` (SEV-02 / D-70-01); installing a structurally `unavailable` plugin renders at `error` with NO `--partial` hint (SEV-02 / D-70-02). The hint references only the user's own flag, with no plugin/marketplace interpolation. |
...
| **NFR-4**  | The extension MUST work with Node >= 20.19.0.                                                                                                                                                                                                                                                                                         |
...
| Agent conversion   | "convertAgent: missing tools omits the allowlist"; "convertAgent: all tools disallowed throws"; "convertAgent: invalid thinking + valid effort falls back to effort with warning"; "stagePluginAgents: refuses to overwrite a foreign plugin's content"                                                                                                                                                                                                                                                                  |
...
```text
/claude:plugin
├── install      <plugin>@<marketplace>            [--scope user|project] [--map-model] [--partial]
├── uninstall    <plugin>@<marketplace>            [--scope user|project]
├── update       [<plugin>@<marketplace> | @<marketplace>]
│                                                  [--scope user|project] [--map-model] [--partial]
├── list         [<marketplace>] [--installed] [--available] [--unavailable] [--partial]
│                                                  [--scope user|project]
└── marketplace
...
| Command    | `<plugin>:<command>`, one colon per path segment for a nested source (`build/web` -> `acme:build:web`) | drop leading `<plugin>-` from the head of the source -- the first path segment when the source is nested (D-141-01) -- unless the drop would leave the head empty (D-141-02) |
| Agent      | `pi-claude-marketplace-<plugin>-<agent>`                                                               | none; preserve the complete source name                                                                                                                                      |
