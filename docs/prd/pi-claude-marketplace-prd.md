...
| **RN-6** | Within a single plugin, two distinct source names that elide to the same generated name (e.g., a plugin `foo` with sources `foo-bar` and `bar`) MUST NOT throw. The second occurrence is the D-07 first-wins skip and a warning. That holds for skills (D-141-04) and for commands, where D-141-01 makes the same collision reachable between two files of one directory and the warning names the winner. A source name equal to the plugin name becomes skill `foo` and command `foo:foo`. (For agents, see AG-12.) |
