...
//
// This module is the SINGLE source of that policy. Every list-rendering
// surface (mp list, plugin list, import / update / reinstall cascades)
// consumes this helper directly. Single-purpose pure-helper file
// mirroring the existing `reload-hint.ts` and `soft-dep.ts` modules.
//
// MSG-GR-3 lock notes:
//   - The comparator accepts a STRUCTURAL minimum
//     `{ readonly name: string; readonly scope: "user" | "project" }`
//     so it can sort `MarketplaceRow`, `PluginListRow`,
//     `PluginCascadeRow`, or any future row type that carries these
//     two fields without requiring an adapter.
//   - `sensitivity: 'base'` treats "Alpha", "alpha", and "ALPHA" as
//     equal -- accent differences are folded as well (matching the
//     style guide's "case-insensitive" wording, which under the JS
//     spec maps to base sensitivity).
//   - The scope tie-breaker uses a strict ternary -- mapping project
//     to -1 and user to +1 -- so the canonical "project before user"
...
//   - No imports from anywhere else in the codebase (pure helper;
//     closed under MSG-GR-3 only). The comparator never throws.
