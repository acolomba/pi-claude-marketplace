 ...
 * `hooks` is admitted here even though component-path validation uses a
 * narrower private set: hook discovery owns the convention file
 * `hooks/hooks.json` instead. `workflows` belongs to both sets: its
 * entry/manifest field is path-bearing, and its convention directory name is
 * the kind name itself.
 */
