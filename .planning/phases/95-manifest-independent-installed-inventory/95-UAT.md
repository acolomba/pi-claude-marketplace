...
number: 1
name: INV-05 concurrency backstop (atomic state.json read under concurrent write)
expected: |
  The list read never observes a partially-written `state.json` (no JSON parse
  error, no half-updated record) even without acquiring the lock.
