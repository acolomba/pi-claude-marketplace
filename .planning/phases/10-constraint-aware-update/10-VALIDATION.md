...
| 10-04-T1 | 10-04 | 4 | UPDT-02 | T-10-18 | The prose is held to the token the real composer stamps, so it cannot drift back | architecture | `node --test tests/architecture/dependency-doc-agreement.test.ts tests/architecture/partial-vocabulary-guard.test.ts` | ✅ | ⬜ pending |
| 10-04-T2 | 10-04 | 4 | UPDT-01, UPDT-02 | T-10-16, T-10-17, T-10-19 | SC3 proven non-vacuously on both cascades; network policy unchanged; no gate disarmed and no coverage exemption added | unit + architecture + full gate | `node --test tests/orchestrators/plugin/update-cascade.test.ts tests/orchestrators/marketplace/update.messaging.test.ts tests/orchestrators/plugin/update-flow.test.ts tests/architecture/no-orchestrator-network.test.ts` then `npm run check` | ✅ | ⬜ pending |
...
- [ ] `tests/orchestrators/plugin/update-constraint-gate.test.ts` — the new leaf module's
...
- [ ] A held-row fixture in `tests/architecture/catalog-uat/fixtures/plugin-update.ts`,
