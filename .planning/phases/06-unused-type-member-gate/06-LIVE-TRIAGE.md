...
This is an inventory, not a clean-gate verdict. It records 3348
candidates, of which 6 are still unresolved and
353 need a recorded disposition. Closure is
...
| Measurement | Value |
| --- | --- |
| Recorded | 2026-09-17T03:41:21.906Z |
| Revision | 241f5faa9381c22d08fb857352c59c3494847197 |
| Source digest | `92dab8239cc6c3866d0d2561eb086fd0dde91ebd74351bf6380a37cc40b600b0` |
...
| Transfer steps | 3035304 |
| Transfer walk milliseconds | 51151 |
...
| edge | 120 | 110 | 3 | 7 | 0 | 0 |
| orchestrators | 1766 | 1618 | 94 | 50 | 4 | 0 |
...
| --- | --- | --- | --- | --- | --- |
| extensions/pi-claude-marketplace/orchestrators/edge-deps.ts:92:5 | LocationsResolverLike.loadStateForScope | marketplaces | unread | - | Left standing with a measured reason, after the sibling half of the same mirror was collapsed and cleared. `MarketplaceStateRecordLike` is now the single declaration of the state-record shape and `edge/completions/data.ts` republishes it as `MarketplaceStateRecord`, which credited the sibling row and removed it from this population. The resolver half was deliberately NOT collapsed the same way, and the blocker is a different gate rather than this analyzer. Measured: aliasing `edge/completions/data.ts::LocationsResolver` to this declaration removes the only reference to `data.ts::MarketplaceStateRecord` inside the extension, and the production-scoped `fallow dead-code` run then reports `extensions/pi-claude-marketplace/edge/completions/data.ts:142 MarketplaceStateRecord` under 'Unused type exports', so `npm run fallow` exits 1. Both published forms were tried and both produced the identical finding: `export type MarketplaceStateRecord = MarketplaceStateRecordLike;`, and the re-export form `export type { MarketplaceStateRecordLike as MarketplaceStateRecord } from "../../orchestrators/edge-deps.ts";` which fallow reports with a `(re-export)` marker. The finding is true rather than a tool artifact: after the alias the name's only remaining consumers are `tests/edge/completions/data.test.ts`, `tests/edge/completions/provider.test.ts` and `tests/architecture/flag-catalog-drift.test.ts`, and the first `fallow dead-code` run is scoped to the production entry graph, which excludes `tests/`. The clean resolution points those three imports at `orchestrators/edge-deps.ts` and drops the republish, which edits test files the owning plan was forbidden to touch; adding a `fallow-ignore` instead is forbidden by the same plan. Returning the persistence state unprojected would also clear the row, but it changes what `loadStateForScope` returns and `tests/orchestrators/edge-deps.test.ts` deep-compares that exact value, so it is behaviour rather than a type repair. Next owner: a plan that holds the three consuming test files. |
...
{
  "schemaVersion": 1,
  "generated": "2026-09-17T03:41:21.906Z",
  "revision": "241f5faa9381c22d08fb857352c59c3494847197",
  "fingerprint": {
    "algorithm": "sha256",
    "files": 605,
    "digest": "92dab8239cc6c3866d0d2561eb086fd0dde91ebd74351bf6380a37cc40b600b0"
  },
  "counts": {
    "productionFiles": 236,
    "candidates": 3348,
  ...
  "dispositions": [
    ...
    {
      "id": "extensions/pi-claude-marketplace/orchestrators/edge-deps.ts:92:5",
      "path": "extensions/pi-claude-marketplace/orchestrators/edge-deps.ts",
      "owner": "LocationsResolverLike.loadStateForScope",
      "key": "marketplaces",
      "status": "unread",
      "disposition": "explained",
      "note": "Left standing with a measured reason, after the sibling half of the same mirror was collapsed and cleared. `MarketplaceStateRecordLike` is now the single declaration of the state-record shape and `edge/completions/data.ts` republishes it as `MarketplaceStateRecord`, which credited the sibling row and removed it from this population. The resolver half was deliberately NOT collapsed the same way, and the blocker is a different gate rather than this analyzer. Measured: aliasing `edge/completions/data.ts::LocationsResolver` to this declaration removes the only reference to `data.ts::MarketplaceStateRecord` inside the extension, and the production-scoped `fallow dead-code` run then reports `extensions/pi-claude-marketplace/edge/completions/data.ts:142 MarketplaceStateRecord` under 'Unused type exports', so `npm run fallow` exits 1. Both published forms were tried and both produced the identical finding: `export type MarketplaceStateRecord = MarketplaceStateRecordLike;`, and the re-export form `export type { MarketplaceStateRecordLike as MarketplaceStateRecord } from \"../../orchestrators/edge-deps.ts\";` which fallow reports with a `(re-export)` marker. The finding is true rather than a tool artifact: after the alias the name's only remaining consumers are `tests/edge/completions/data.test.ts`, `tests/edge/completions/provider.test.ts` and `tests/architecture/flag-catalog-drift.test.ts`, and the first `fallow dead-code` run is scoped to the production entry graph, which excludes `tests/`. The clean resolution points those three imports at `orchestrators/edge-deps.ts` and drops the republish, which edits test files the owning plan was forbidden to touch; adding a `fallow-ignore` instead is forbidden by the same plan. Returning the persistence state unprojected would also clear the row, but it changes what `loadStateForScope` returns and `tests/orchestrators/edge-deps.test.ts` deep-compares that exact value, so it is behaviour rather than a type repair. Next owner: a plan that holds the three consuming test files."
    },
    {
