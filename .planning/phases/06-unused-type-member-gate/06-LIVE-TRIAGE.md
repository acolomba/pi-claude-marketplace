...
candidates, of which 33 are still unresolved and
...
| Measurement | Value |
| --- | --- |
| Recorded | 2026-09-16T16:07:59.101Z |
| Revision | e4b680370b8de41178049dcbd0947db6ad720b0f |
| Source digest | `feb875b464a59c2e60ca98a681d50fd2ab8e58ac24b6ccf69c54f1af7a6090d2` |
| Source files hashed | 605 |
| Production files analysed | 236 |
| Candidates | 3352 |
| Runtime-observed | 2995 |
| Test-only-observed | 234 |
| Explicit-contract | 90 |
| Unread | 33 |
| Unsupported analysis | 0 |
| Transfer steps | 3002713 |
| Transfer walk milliseconds | 50168 |
...
| domain | 266 | 227 | 22 | 10 | 7 | 0 |
| edge | 121 | 111 | 3 | 0 | 7 | 0 |
| orchestrators | 1766 | 1617 | 92 | 47 | 10 | 0 |
| persistence | 77 | 58 | 15 | 1 | 3 | 0 |
...
| extensions/pi-claude-marketplace/domain/components/hook-tool-names.ts:52:21 | PiToolName | toolName | unread | - | Load-bearing compile-time proof; NOT removed and NOT rewritten. The member is the filter literal inside a conditional type `extends` clause (`Event extends { toolName: infer Name }`), doing the type-system work a two-argument selection does, in the form 06-06 recorded the engine does not prove. Measured: renaming the key to `piToolNameProbe` collapses `PiToolName` to `never` and `npm run typecheck` fails at `extensions/pi-claude-marketplace/domain/components/hook-if-targets.ts(88,19): error TS2769: No overload matches this call.` -- `new Set<PiToolName>(["bash"])` can no longer take a string. The drift was restored. A `type-selection` draft is refused by `selectionOf`, which requires the filter literal parent to be a `TypeReferenceNode` with exactly two type arguments; the parent chain here is `TypeLiteral -> ConditionalType`. Next owner: the SAME bounded engine plan 06-11 named for `ParsedCommandArgs.required` (`extensions/pi-claude-marketplace/edge/args-schema.ts:60:70`) -- one conditional-clause proof with its own drift controls clears both rows together. Neither may be cleared by rewriting the product type. |
...
| Declaration | Owner | Member | Status | Reasons | Disposition |
| --- | --- | --- | --- | --- | --- |
| extensions/pi-claude-marketplace/edge/args-schema.ts:60:70 | ParsedCommandArgs | required | unread | - | Filter literal in a conditional type's `extends` clause, left standing deliberately. `ParsedCommandArgs` maps each `Entry` through a conditional type whose `extends` clause is the literal `{ required: false }`, selecting a possibly-undefined string for a matching entry and a plain string otherwise, so this member is what makes a tail-optional positional come back possibly-undefined; deleting it would change a public generic every command handler's argument parsing flows through. A `type-selection` entry naming the conditional type at extensions/pi-claude-marketplace/edge/args-schema.ts:60:54 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/args-schema.ts:60:70 filter extensions/pi-claude-marketplace/edge/args-schema.ts:60:54 is not a two-argument type selection". `selectionOf` requires the literal's parent to be a `TypeReferenceNode` carrying exactly two type arguments; the parent chain here is TypeLiteral -> ConditionalType, so no category fits. The rewrite that would restate the filter as an `Extract`-based two-argument selection is refused on purpose: 06-CONTEXT's implementation discretion corrects an incomplete analysis in preference to changing working product code to satisfy it. The twin is `PiToolName.toolName` in `domain/components/hook-tool-names.ts` (owner plan 06-13); one bounded engine plan adding a conditional-clause proof with its own drift controls clears both together. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |
| extensions/pi-claude-marketplace/edge/handlers/tools.ts:100:9 | marketplaces | name | unread | - | Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:29 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:100:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |
| extensions/pi-claude-marketplace/edge/handlers/tools.ts:101:9 | marketplaces | scope | unread | - | Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:48 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:101:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |
| extensions/pi-claude-marketplace/edge/handlers/tools.ts:102:9 | marketplaces | pluginCount | unread | - | Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:55 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:102:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |
| extensions/pi-claude-marketplace/edge/handlers/tools.ts:103:9 | marketplaces | source | unread | - | Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:68 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:103:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |
| extensions/pi-claude-marketplace/edge/handlers/tools.ts:140:3 | PluginRow | marketplace | unread | - | Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:437:9 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:140:3 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 is not a return to an external declaration". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |
| extensions/pi-claude-marketplace/edge/handlers/tools.ts:141:3 | PluginRow | scope | unread | - | Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:438:9 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 was submitted to the real engine against this tree and REFUSED: "Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:141:3 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 is not a return to an external declaration". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge). |

### orchestrators

| Declaration | Owner | Member | Status | Reasons | Disposition |
| --- | --- | --- | --- | --- | --- |
| extensions/pi-claude-marketplace/orchestrators/edge-deps.ts:58:3 | MarketplaceStateRecordLike | plugins | unread | - | Kept as an outstanding finding by the orchestrators repair, with the reason measured rather than inherited. This is the orchestrator half of the completions-seam mirror: the architecture boundary forbids `orchestrators/` from importing `edge/`, so the shape is re-declared here and the file's own comment records that it must stay in sync with `edge/completions/data.ts`. The edge side reads `record.plugins` through its OWN `MarketplaceStateRecord.plugins`, so this declaration has no reader by construction. The collapse that would resolve it runs the legal direction -- `edge/` referencing the orchestrator declaration -- and lands in an edge-owned file this plan is forbidden to edit. The sibling `manifestPath` slot WAS removed here, because it is dead on both sides at once. Next owner: one bounded plan that lands the single shared declaration across both files, or a standing decision that the sync-comment mirror is the price of the boundary. |
| extensions/pi-claude-marketplace/orchestrators/edge-deps.ts:64:5 | LocationsResolverLike.loadStateForScope | marketplaces | unread | - | Kept as an outstanding finding by the orchestrators repair, for the same mirror reason as the sibling at `:58:3`. The seam declares the projection shape the completions cache rebuilds from; the edge reads `state.marketplaces` through its own `LocationsResolver.loadStateForScope` return annotation. The duplicate HALF of this row was repaired: `makeLocationsResolver`'s implementation used to restate the same return annotation, and now takes it contextually from the seam, so two declarations became one. What remains is the cross-boundary mirror itself. Returning the persistence state unprojected would resolve it but changes what the resolver returns, which `tests/orchestrators/edge-deps.test.ts` pins exactly -- that is behaviour, not a type repair. |
| extensions/pi-claude-marketplace/orchestrators/marketplace/remove.ts:114:35 | RemoveMarketplaceOutcome | name | unread | - | Kept as an outstanding finding by the orchestrators repair; the inherited disposition's claim that nothing reads it 'in production or in tests' is wrong about the tree. Production is clean: `foldRemoveOutcome` (`reconcile/apply.ts:251`) takes `marketplace` as its own parameter and never reads `result.name`. Tests are not: `tests/orchestrators/marketplace/remove.test.ts:563` and `:596` deep-compare the whole outcome, and at `:596` the compared `name: "user-only"` IS the claim the case makes -- 'selects the user marketplace in orchestrated bare form when project is absent'. Removing the slot would delete the discriminating field from that assertion. The sibling-symmetry question was answered in writing: `AddMarketplaceOutcome.name` is read at `reconcile/apply.ts:332` for a documented CR-01 reason (the add outcome's name is MANIFEST-derived and need not equal the declared key), which remove has no analogue for -- so the production symmetry is NOT load-bearing and is not the reason this slot stays. It stays because three assertions observe it. Analyzer under-credit: a whole-object `deepStrictEqual` is not modelled as a read of the member. |
| extensions/pi-claude-marketplace/orchestrators/marketplace/remove.ts:129:7 | RemoveMarketplaceOutcome | name | unread | - | Kept as an outstanding finding by the orchestrators repair, for the same measured reason as the `removed` arm at `:114:35`: `foldRemoveOutcome` reads `unstaged`, `failed` and `reason` and takes the marketplace name from its own parameter, while `tests/orchestrators/marketplace/remove.test.ts:1440` deep-compares the partial arm as a whole. The sibling-symmetry question is answered on the `:114:35` row. Removing half of a discriminated outcome's subject while its tests observe it is a weakening, not a repair. |
...
| extensions/pi-claude-marketplace/orchestrators/plugin/update-preflight.ts:315:37 | StaticPreflightRowOptions | fromVersion | unread | - | Kept as an outstanding finding by the orchestrators repair, after the restatement the taxonomy prescribes was built, submitted and measured. Hoisting `fromVersion?: string` onto the base and distributing the intersection over both arms DID get the marker past the 'adds a key the rest of the intersection does not declare' refusal, and the engine then refused it at the next check: `Invalid contract: ...:318:63 refines ...:318:6 does not narrow fromVersion`. Mechanism, read out of the prover: `narrows` (`scripts/check-unused-type-members.contracts.mjs:825`) requires `chosen.length < allowed.length` over union constituents, and `constituentsOf` returns `[type]` for a non-union, so `never` counts as ONE constituent against `string`'s one. A `never` pin can only be proved over a wider slot that IS a union (`true` over `boolean` passes, which is why the sibling `partialable` pin was accepted) or over a slot the wider shape lets a value omit. The restatement was REVERTED rather than shipped: reshaping production source to satisfy a proof the engine refused is exactly what this phase forbids. Next owner: a bounded engine plan teaching `narrows` that `never` admits strictly fewer values than any inhabited type. |
| extensions/pi-claude-marketplace/orchestrators/plugin/update-swap.ts:188:3 | DirectRenderableFailedOutcome | cause | unread | - | Kept as an outstanding finding by the orchestrators repair, measured the same way as `update-preflight.ts:315:37`. The R4 restatement was built: `PluginUpdateFailedOutcome & { ... }` in place of `extends Omit<..., five keys>`, which is sound because all five omitted slots are OPTIONAL on the failed outcome, and a compiler probe confirmed all five refusals survive it and no producer gains a new obligation. It still cannot be proved: `narrows` compares union-constituent counts and `never` counts as one against `Error`'s one. The sibling `reasons` requirement and the `fromVersion` / `phaseFailures` pins are unaffected and stay credited. The restatement was reverted; the vestigial `Omit` stays until a prover that can read a `never` pin exists. |
| extensions/pi-claude-marketplace/orchestrators/plugin/update-swap.ts:191:3 | DirectRenderableFailedOutcome | toVersion | unread | - | Kept as an outstanding finding by the orchestrators repair, for the identical mechanism recorded on `:188:3`: `never` pinned over `string` gives `chosen.length == allowed.length` in `narrows`, so the refinement prover cannot distinguish it from a same-width restatement. Measured, not inferred. |
| extensions/pi-claude-marketplace/orchestrators/types.ts:155:3 | UpdatePhaseFailure | msg | unread | - | Kept as an outstanding finding by the orchestrators repair, and reclassified from the inherited disposition. The reader survey agrees with it -- the only syntax naming this declaration is the write at `update-swap.ts:929`, and the `f.msg` reads at `:921` and `:929` land on `Phase3Failure.msg` through `UpdatePhase3Failure`, a different declaration -- but the removal is still refused. `phaseFailures` is populated by exactly one path, the phase-3 rollback aggregation, and its own header states the slot's contract: surface the failures structurally 'so the cascade renderer can build the rollback-partial parent plus indented children block', with `notes[]` retained only for outcome-level text. A slot a rollback path populates is behaviour even when nothing reads it back today, so this is recorded rather than deleted. Deleting it would force a future consumer of the structured surface to re-parse the per-phase text out of `notes` prose. |

### persistence

| Declaration | Owner | Member | Status | Reasons | Disposition |
| --- | --- | --- | --- | --- | --- |
| extensions/pi-claude-marketplace/persistence/migrate-config.ts:71:7 | MigrateFirstRunResult | reason | unread | - | Measured analyzer under-credit, NOT a surplus slot; no source touched and no test reshaped. The inherited disposition reads this row as an unread slot to confirm surplus and remove, which is wrong about the tree: `tests/persistence/migrate-config.test.ts:232 (and the replay assertion at :301)` asserts `reason: \"existing-valid\"` inside a whole-object `assert.deepStrictEqual` of the result, so the read is real. Measured from `node scripts/check-unused-type-members.mjs --json` on this tree, derived from the report rather than transcribed: the operations model credits `deep-comparison` 22950 times tree-wide, so it is live; the suite carries five whole-object deep comparisons, at `tests/persistence/migrate-config.test.ts` 230:26, 250:26, 269:26, 293:26 and 299:26, and the report credits ALL FIVE to every sibling member of these same arms -- `migrated` 20 witnesses, `filePath` 20, `entryCount` 5, `error` 5 -- while all three `reason` rows carry an EMPTY witness list. The difference is how many arms spell the key: `migrated` is on all four arms, so the checker answers the union directly; `entryCount` and `error` are each on exactly one arm, which 06-06 single-arm-union rule resolves; `reason` is on three arms, and 06-06 deliberately leaves a key two or more arms spell unsettled rather than excusing a member by its neighbour. Under-crediting is the designed direction, so this is a finding about the analyzer, never a defect in the product. The discriminant is load-bearing on its own terms too: cutting `error` along it is what makes `tests/persistence/migrate-config.test.ts:34` (`@ts-expect-error `error` exists only on the existing-invalid arm`, over the read at :35) hold. Next owner: a bounded analyzer plan attributing a whole-object comparison by the discriminant VALUE where the key alone is ambiguous, with its own drift controls -- not a widening of the existing rule. |
| extensions/pi-claude-marketplace/persistence/migrate-config.ts:76:7 | MigrateFirstRunResult | reason | unread | - | Measured analyzer under-credit, NOT a surplus slot; no source touched and no test reshaped. The inherited disposition reads this row as an unread slot to confirm surplus and remove, which is wrong about the tree: `tests/persistence/migrate-config.test.ts:252` asserts `reason: \"existing-invalid\"` inside a whole-object `assert.deepStrictEqual` of the result, so the read is real. Measured from `node scripts/check-unused-type-members.mjs --json` on this tree, derived from the report rather than transcribed: the operations model credits `deep-comparison` 22950 times tree-wide, so it is live; the suite carries five whole-object deep comparisons, at `tests/persistence/migrate-config.test.ts` 230:26, 250:26, 269:26, 293:26 and 299:26, and the report credits ALL FIVE to every sibling member of these same arms -- `migrated` 20 witnesses, `filePath` 20, `entryCount` 5, `error` 5 -- while all three `reason` rows carry an EMPTY witness list. The difference is how many arms spell the key: `migrated` is on all four arms, so the checker answers the union directly; `entryCount` and `error` are each on exactly one arm, which 06-06 single-arm-union rule resolves; `reason` is on three arms, and 06-06 deliberately leaves a key two or more arms spell unsettled rather than excusing a member by its neighbour. Under-crediting is the designed direction, so this is a finding about the analyzer, never a defect in the product. The discriminant is load-bearing on its own terms too: cutting `error` along it is what makes `tests/persistence/migrate-config.test.ts:34` (`@ts-expect-error `error` exists only on the existing-invalid arm`, over the read at :35) hold. Next owner: a bounded analyzer plan attributing a whole-object comparison by the discriminant VALUE where the key alone is ambiguous, with its own drift controls -- not a widening of the existing rule. |
| extensions/pi-claude-marketplace/persistence/migrate-config.ts:82:7 | MigrateFirstRunResult | reason | unread | - | Measured analyzer under-credit, NOT a surplus slot; no source touched and no test reshaped. The inherited disposition reads this row as an unread slot to confirm surplus and remove, which is wrong about the tree: `tests/persistence/migrate-config.test.ts:271` asserts `reason: \"empty-state\"` inside a whole-object `assert.deepStrictEqual` of the result, so the read is real. Measured from `node scripts/check-unused-type-members.mjs --json` on this tree, derived from the report rather than transcribed: the operations model credits `deep-comparison` 22950 times tree-wide, so it is live; the suite carries five whole-object deep comparisons, at `tests/persistence/migrate-config.test.ts` 230:26, 250:26, 269:26, 293:26 and 299:26, and the report credits ALL FIVE to every sibling member of these same arms -- `migrated` 20 witnesses, `filePath` 20, `entryCount` 5, `error` 5 -- while all three `reason` rows carry an EMPTY witness list. The difference is how many arms spell the key: `migrated` is on all four arms, so the checker answers the union directly; `entryCount` and `error` are each on exactly one arm, which 06-06 single-arm-union rule resolves; `reason` is on three arms, and 06-06 deliberately leaves a key two or more arms spell unsettled rather than excusing a member by its neighbour. Under-crediting is the designed direction, so this is a finding about the analyzer, never a defect in the product. The discriminant is load-bearing on its own terms too: cutting `error` along it is what makes `tests/persistence/migrate-config.test.ts:34` (`@ts-expect-error `error` exists only on the existing-invalid arm`, over the read at :35) hold. Next owner: a bounded analyzer plan attributing a whole-object comparison by the discriminant VALUE where the key alone is ambiguous, with its own drift controls -- not a widening of the existing rule. |
...
{
  "schemaVersion": 1,
  "generated": "2026-09-16T16:07:59.101Z",
  "revision": "e4b680370b8de41178049dcbd0947db6ad720b0f",
  "fingerprint": {
    "algorithm": "sha256",
    "files": 605,
    "digest": "feb875b464a59c2e60ca98a681d50fd2ab8e58ac24b6ccf69c54f1af7a6090d2"
  },
  "counts": {
    "productionFiles": 236,
    "candidates": 3352,
    "runtimeObserved": 2995,
    "testOnlyObserved": 234,
    "explicitContract": 90,
    "unread": 33,
    "unsupportedAnalysis": 0
  },
  "work": {
    "transferSteps": 3002713,
    "transferEdges": 217093,
    "transferReads": 82941,
  ...
  "dispositions": [
    ...
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Load-bearing compile-time proof; NOT removed and NOT rewritten. The member is the filter literal inside a conditional type `extends` clause (`Event extends { toolName: infer Name }`), doing the type-system work a two-argument selection does, in the form 06-06 recorded the engine does not prove. Measured: renaming the key to `piToolNameProbe` collapses `PiToolName` to `never` and `npm run typecheck` fails at `extensions/pi-claude-marketplace/domain/components/hook-if-targets.ts(88,19): error TS2769: No overload matches this call.` -- `new Set<PiToolName>([\"bash\"])` can no longer take a string. The drift was restored. A `type-selection` draft is refused by `selectionOf`, which requires the filter literal parent to be a `TypeReferenceNode` with exactly two type arguments; the parent chain here is `TypeLiteral -> ConditionalType`. Next owner: the SAME bounded engine plan 06-11 named for `ParsedCommandArgs.required` (`extensions/pi-claude-marketplace/edge/args-schema.ts:60:70`) -- one conditional-clause proof with its own drift controls clears both rows together. Neither may be cleared by rewriting the product type."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Analyzer under-credit plus a live compiler obligation; no source touched here. The anchor context is duplicated in `domain` on purpose so the parser does not depend on the bridge surface (D-11 import direction); the value flows through whole and every real read lands on the bridge-side twin `CompileIfPredicateContext`, which is why the third slot (`cwd`) is credited and these two are not. The member is NOT dead: deleting it makes `npm run typecheck` fail at `extensions/pi-claude-marketplace/bridges/hooks/event-router.ts(149,47): error TS2345 ... Property homedir is missing in type ResolveHookIfContext but required in type CompileIfPredicateContext` (and again at 670:47). The deletion was restored. The repair that settles the row is to make the bridge-side declaration an alias of this one -- the legal import direction, and the shape 06-06 named for the async-rewake duplicate -- which edits `extensions/pi-claude-marketplace/bridges/hooks/if-field/index.ts`. That file belongs to owner plan 06-09, which has already completed, so this plan does not reach into it. Next owner: a follow-up that holds the `bridges/hooks/if-field` files."
    },
    {
    ...
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:29 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: \"Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:100:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration\". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge)."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:48 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: \"Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:101:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration\". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge)."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:55 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: \"Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:102:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration\". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge)."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:114:68 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 was submitted to the real engine against this tree and REFUSED: \"Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:103:9 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:117:7 is not a return to an external declaration\". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge)."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:437:9 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 was submitted to the real engine against this tree and REFUSED: \"Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:140:3 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 is not a return to an external declaration\". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge)."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Payload slot crossing the Pi tool boundary: proved in one half, refused in the other, and left standing rather than deleted. The slot is built at extensions/pi-claude-marketplace/edge/handlers/tools.ts:438:9 and returned under `details` from the tool's `execute`, whose enclosing object literal contextually types to `ToolDefinition<...>` -- a declaration in node_modules/@earendil-works/pi-coding-agent/dist/core/extensions/types.d.ts -- so the value really does leave this tree and removing the slot would silently change a shipped payload. An `external-output` entry naming that origin and the return at extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 was submitted to the real engine against this tree and REFUSED: \"Invalid contract: extensions/pi-claude-marketplace/edge/handlers/tools.ts:141:3 boundary extensions/pi-claude-marketplace/edge/handlers/tools.ts:524:7 is not a return to an external declaration\". The origin half passed -- `proveExternalOutput` checks `originCandidates` first and agreed the named site builds this exact member -- so only the boundary half is unproven. Two measured engine limits stand in the way, neither a defect in this code. First, `execute` is a method shorthand, so `checker.getContextualType` on the `MethodDeclaration` answers `undefined` and `externalCallSignatures` finds no signature, even though the object literal one level out resolves to `ToolDefinition` and its `execute` property is declared in the installed package. Second, `reaches` follows only an identifier chain or an origin textually inside the returned expression; both returns here are `ObjectLiteralExpression`s carrying the slot nested under `details`, built in a loop elsewhere. Owner decision: accept the finding, or a bounded engine plan teaching `external-output` to read a method-shorthand handler's contextual signature from its enclosing object literal and to follow a value into a nested property of the returned literal. Converting `execute` to an arrow-function property, or flattening the return, to suit the prover is refused on purpose. Recorded outstanding by the edge repair; owner plan 06-11 (edge)."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair, with the reason measured rather than inherited. This is the orchestrator half of the completions-seam mirror: the architecture boundary forbids `orchestrators/` from importing `edge/`, so the shape is re-declared here and the file's own comment records that it must stay in sync with `edge/completions/data.ts`. The edge side reads `record.plugins` through its OWN `MarketplaceStateRecord.plugins`, so this declaration has no reader by construction. The collapse that would resolve it runs the legal direction -- `edge/` referencing the orchestrator declaration -- and lands in an edge-owned file this plan is forbidden to edit. The sibling `manifestPath` slot WAS removed here, because it is dead on both sides at once. Next owner: one bounded plan that lands the single shared declaration across both files, or a standing decision that the sync-comment mirror is the price of the boundary."
    },
    {
      "id": "extensions/pi-claude-marketplace/orchestrators/edge-deps.ts:64:5",
      "path": "extensions/pi-claude-marketplace/orchestrators/edge-deps.ts",
      "owner": "LocationsResolverLike.loadStateForScope",
      "key": "marketplaces",
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair, for the same mirror reason as the sibling at `:58:3`. The seam declares the projection shape the completions cache rebuilds from; the edge reads `state.marketplaces` through its own `LocationsResolver.loadStateForScope` return annotation. The duplicate HALF of this row was repaired: `makeLocationsResolver`'s implementation used to restate the same return annotation, and now takes it contextually from the seam, so two declarations became one. What remains is the cross-boundary mirror itself. Returning the persistence state unprojected would resolve it but changes what the resolver returns, which `tests/orchestrators/edge-deps.test.ts` pins exactly -- that is behaviour, not a type repair."
    },
    {
    ...
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair; the inherited disposition's claim that nothing reads it 'in production or in tests' is wrong about the tree. Production is clean: `foldRemoveOutcome` (`reconcile/apply.ts:251`) takes `marketplace` as its own parameter and never reads `result.name`. Tests are not: `tests/orchestrators/marketplace/remove.test.ts:563` and `:596` deep-compare the whole outcome, and at `:596` the compared `name: \"user-only\"` IS the claim the case makes -- 'selects the user marketplace in orchestrated bare form when project is absent'. Removing the slot would delete the discriminating field from that assertion. The sibling-symmetry question was answered in writing: `AddMarketplaceOutcome.name` is read at `reconcile/apply.ts:332` for a documented CR-01 reason (the add outcome's name is MANIFEST-derived and need not equal the declared key), which remove has no analogue for -- so the production symmetry is NOT load-bearing and is not the reason this slot stays. It stays because three assertions observe it. Analyzer under-credit: a whole-object `deepStrictEqual` is not modelled as a read of the member."
    },
    {
    ...
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair, for the same measured reason as the `removed` arm at `:114:35`: `foldRemoveOutcome` reads `unstaged`, `failed` and `reason` and takes the marketplace name from its own parameter, while `tests/orchestrators/marketplace/remove.test.ts:1440` deep-compares the partial arm as a whole. The sibling-symmetry question is answered on the `:114:35` row. Removing half of a discriminated outcome's subject while its tests observe it is a weakening, not a repair."
    },
    {
    ...
    {
      "id": "extensions/pi-claude-marketplace/orchestrators/plugin/update-preflight.ts:315:37",
      "path": "extensions/pi-claude-marketplace/orchestrators/plugin/update-preflight.ts",
      "owner": "StaticPreflightRowOptions",
      "key": "fromVersion",
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair, after the restatement the taxonomy prescribes was built, submitted and measured. Hoisting `fromVersion?: string` onto the base and distributing the intersection over both arms DID get the marker past the 'adds a key the rest of the intersection does not declare' refusal, and the engine then refused it at the next check: `Invalid contract: ...:318:63 refines ...:318:6 does not narrow fromVersion`. Mechanism, read out of the prover: `narrows` (`scripts/check-unused-type-members.contracts.mjs:825`) requires `chosen.length < allowed.length` over union constituents, and `constituentsOf` returns `[type]` for a non-union, so `never` counts as ONE constituent against `string`'s one. A `never` pin can only be proved over a wider slot that IS a union (`true` over `boolean` passes, which is why the sibling `partialable` pin was accepted) or over a slot the wider shape lets a value omit. The restatement was REVERTED rather than shipped: reshaping production source to satisfy a proof the engine refused is exactly what this phase forbids. Next owner: a bounded engine plan teaching `narrows` that `never` admits strictly fewer values than any inhabited type."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair, measured the same way as `update-preflight.ts:315:37`. The R4 restatement was built: `PluginUpdateFailedOutcome & { ... }` in place of `extends Omit<..., five keys>`, which is sound because all five omitted slots are OPTIONAL on the failed outcome, and a compiler probe confirmed all five refusals survive it and no producer gains a new obligation. It still cannot be proved: `narrows` compares union-constituent counts and `never` counts as one against `Error`'s one. The sibling `reasons` requirement and the `fromVersion` / `phaseFailures` pins are unaffected and stay credited. The restatement was reverted; the vestigial `Omit` stays until a prover that can read a `never` pin exists."
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Kept as an outstanding finding by the orchestrators repair, for the identical mechanism recorded on `:188:3`: `never` pinned over `string` gives `chosen.length == allowed.length` in `narrows`, so the refinement prover cannot distinguish it from a same-width restatement. Measured, not inferred."
    },
    {
    ...
    },
    {
      ...
      "status": "unread",
      "disposition": "explained",
      "note": "Measured analyzer under-credit, NOT a surplus slot; no source touched and no test reshaped. The inherited disposition reads this row as an unread slot to confirm surplus and remove, which is wrong about the tree: `tests/persistence/migrate-config.test.ts:232 (and the replay assertion at :301)` asserts `reason: \\\"existing-valid\\\"` inside a whole-object `assert.deepStrictEqual` of the result, so the read is real. Measured from `node scripts/check-unused-type-members.mjs --json` on this tree, derived from the report rather than transcribed: the operations model credits `deep-comparison` 22950 times tree-wide, so it is live; the suite carries five whole-object deep comparisons, at `tests/persistence/migrate-config.test.ts` 230:26, 250:26, 269:26, 293:26 and 299:26, and the report credits ALL FIVE to every sibling member of these same arms -- `migrated` 20 witnesses, `filePath` 20, `entryCount` 5, `error` 5 -- while all three `reason` rows carry an EMPTY witness list. The difference is how many arms spell the key: `migrated` is on all four arms, so the checker answers the union directly; `entryCount` and `error` are each on exactly one arm, which 06-06 single-arm-union rule resolves; `reason` is on three arms, and 06-06 deliberately leaves a key two or more arms spell unsettled rather than excusing a member by its neighbour. Under-crediting is the designed direction, so this is a finding about the analyzer, never a defect in the product. The discriminant is load-bearing on its own terms too: cutting `error` along it is what makes `tests/persistence/migrate-config.test.ts:34` (`@ts-expect-error `error` exists only on the existing-invalid arm`, over the read at :35) hold. Next owner: a bounded analyzer plan attributing a whole-object comparison by the discriminant VALUE where the key alone is ambiguous, with its own drift controls -- not a widening of the existing rule."
    },
    {
