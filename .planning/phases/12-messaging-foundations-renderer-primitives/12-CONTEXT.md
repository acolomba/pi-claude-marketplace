...
- `.planning/REQUIREMENTS.md` — CMC-08, CMC-11, CMC-14, CMC-19, CMC-36, CMC-37 are Phase 12's six requirements. **Open inconsistency:** CMC-08 text says the closed set is the frontmatter "plus the `reinstalled` token used in reinstall cascades" — but the frontmatter has 14 tokens, no `reinstalled`, and roadmap criterion #1 says "exactly the frontmatter set" (14). Planner / researcher must reconcile during Phase 12 plan creation: either drop the "plus `reinstalled`" clause from REQUIREMENTS.md CMC-08 (most likely correct — `reinstalled` is an internal partition kind in `orchestrators/plugin/reinstall.ts`, not a user-visible status token), or add `reinstalled` to the frontmatter (drives Phase 13 icon-predicate work). User declined to lock this in discussion; flagged here for downstream resolution.
...
- **REQUIREMENTS.md CMC-08 vs frontmatter `reinstalled` reconciliation** — Surfaced during discussion as a doc inconsistency (CMC-08 says "+ `reinstalled`" = 15 tokens; frontmatter has 14; roadmap criterion says "exactly the frontmatter set"). User declined to lock the resolution path. Planner / researcher MUST address during Phase 12 plan creation — either update REQUIREMENTS.md CMC-08 to drop the `reinstalled` clause (most likely correct given `reinstalled` appears to be an internal partition kind), or add `reinstalled` to the frontmatter (drives additional Phase 13 icon-predicate work for the `(reinstalled)` token). The choice affects the drift-test (D-CMC-04) constants membership and Phase 13's icon predicate.
