...
### `(reinstalled)` Status Token Reconciliation (resolved 2026-05-23)

- **D-13-20:** The `(reinstalled)` token contradiction surfaced by Phase 13 research (catalog renders `(reinstalled)` on reinstall cascade rows at `docs/output-catalog.md` lines 147, 351-357 but Phase 12's `STATUS_TOKENS` closed set has 14 entries and excludes it; style guide §3 status-tokens table also lists only 14 tokens) is resolved by **extending the closed set to 15 entries**. Wave 1 must add `"reinstalled"` to (a) `extensions/pi-claude-marketplace/shared/grammar/status-tokens.ts` (`STATUS_TOKENS` literal-union array), (b) `docs/messaging-style-guide.md` YAML frontmatter `status_tokens:` list, (c) `docs/messaging-style-guide.md` §3 status-tokens table with the operator-mental-model row ("operation just ran: reinstall partition"). This keeps `tests/architecture/grammar-frontmatter.test.ts` green (Phase 12 D-CMC-04 byte-equality assertion stays intact). The catalog stays unchanged; the renderer emits `(reinstalled)` on reinstall cascade rows. The `RowSpec` variant for reinstall cascade rows narrows `status` to `Extract<StatusToken, 'reinstalled' | 'skipped' | 'failed'>`. REQUIREMENTS.md CMC-08 is not edited (Phase 12's reconciliation said "the cascade-kind discriminant `ReinstallPluginPartition` is internal" -- still true; the *token* is rendered, the *partition kind* remains internal). Rejected: amending the catalog to `(installed)` -- loses observability of which rows the reinstall partition actually processed; operators cannot distinguish a reinstall no-op from a successful reinstall on the cascade summary.
