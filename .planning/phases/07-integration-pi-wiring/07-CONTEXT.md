...
- **D-11 (Disk walk over state.json):** For each scope, list subdirectories of `<scopeRoot>/claude-marketplace/resources/skills/` (each subdirectory is a `<plugin>-<skill>/` with a `SKILL.md` inside; the returned path is the directory or `SKILL.md` per Pi's `resources_discover` event contract -- confirmed during research). List `*.md` files in `<scopeRoot>/claude-marketplace/resources/prompts/`. Source-of-truth is disk -- if state.json is stale or a user manually deletes a file, discovery reflects reality.
...
- **D-04 ESLint rule exact pattern + exception list:** Planning picks the precise glob; default exempts `platform/pi-api.ts` itself + `tests/**/*` + `extensions/claude-marketplace/index.ts` (if it imports for the `ExtensionAPI` parameter type).
...
- `extensions/claude-marketplace/shared/notify.ts` -- every Phase 7 user-visible message routes through `notifyError(ctx, msg, cause?)`; the lock-acquisition failure surfaces here
- `extensions/claude-marketplace/shared/markers.ts` -- Phase 7 ADDS `STATE_LOCK_HELD_PREFIX` here
- `extensions/claude-marketplace/shared/errors.ts` -- Phase 7 ADDS `StateLockHeldError` and `AggregateResourcesDiscoverError`
- `extensions/claude-marketplace/shared/atomic-json.ts` -- `atomicWriteJson` is the durability layer under the new lock layer
...
- `extensions/claude-marketplace/transaction/with-state-guard.ts` -- Phase 7 D-06 EXTENDS to acquire/release `proper-lockfile` around the existing load/save sequence
- `extensions/claude-marketplace/persistence/locations.ts` -- `ScopedLocations` brand. Phase 7 ADDS a `stateLockFile(loc)` helper returning `<scopeRoot>/claude-marketplace/.state-lock` (containment via `assertPathInside`)
- `extensions/claude-marketplace/persistence/state-io.ts` -- `loadState`, `saveState`; Phase 7's lock layer wraps these in `withStateGuard` without changing their signatures
- `extensions/claude-marketplace/domain/manifest.ts` -- the NFR-8 caching seam; Phase 7's architecture test asserts this file is the sole `marketplace.json` reader
...
- `extensions/claude-marketplace/orchestrators/marketplace/shared.ts` -- `DEFAULT_GIT_OPS` import target for `index.ts`
- `extensions/claude-marketplace/orchestrators/types.ts` -- `PluginUpdateFn`, `PluginUpdateOutcome`
- `extensions/claude-marketplace/orchestrators/edge-deps.ts` -- `makeLocationsResolver`; Phase 6 D-04 created this; `index.ts` consumes via `registerClaudePluginCommand`
...
- `extensions/claude-marketplace/orchestrators/plugin/update.ts` -- exports `updateSinglePlugin: PluginUpdateFn` for `index.ts` injection
...
- `extensions/claude-marketplace/edge/register.ts` -- `registerClaudePluginCommand`, `registerClaudeMarketplaceTools`; Phase 7's `index.ts` calls both
- `extensions/claude-marketplace/edge/types.ts` -- `EdgeDeps` interface; Phase 7 builds an instance
...
- `git show features/initial:extensions/claude-marketplace.ts` -- V1 entrypoint; the registration shape Phase 7's `index.ts` matches (modulo Phase 6's two-helper split via `registerClaudePluginCommand` + `registerClaudeMarketplaceTools`)
...
- **`extensions/claude-marketplace/index.ts`** -- the Phase 1 stub returns empty `resources_discover` arrays and a "not implemented yet" warning for the slash command. Phase 7 REPLACES the stub with a 3-call entrypoint (D-10 + D-13 + Phase 6 D-04 registration).
- **`extensions/claude-marketplace/edge/register.ts`** -- already exports `registerClaudePluginCommand(pi, deps)` + `registerClaudeMarketplaceTools(pi)`. Phase 7's `index.ts` calls both. No changes to `register.ts`.
- **`extensions/claude-marketplace/edge/types.ts`** -- `EdgeDeps` shape locked. Phase 7's `index.ts` builds the instance from `DEFAULT_GIT_OPS` + `updateSinglePlugin`.
- **`extensions/claude-marketplace/orchestrators/marketplace/shared.ts`** -- `DEFAULT_GIT_OPS` already exported. Phase 7 imports.
- **`extensions/claude-marketplace/orchestrators/plugin/update.ts`** -- `updateSinglePlugin: PluginUpdateFn` already exported. Phase 7 imports.
- **`extensions/claude-marketplace/orchestrators/edge-deps.ts`** -- `makeLocationsResolver` already exists; `register.ts` already calls it. No changes for Phase 7 unless the resolver needs a tweak for `resources_discover`'s per-call cwd capture (D-13).
- **`extensions/claude-marketplace/transaction/with-state-guard.ts`** -- Phase 7 D-06 EXTENDS to acquire `proper-lockfile` lock before `loadState`, release after `saveState`. Caller signatures unchanged.
- **`extensions/claude-marketplace/persistence/state-io.ts`** -- `loadState`, `saveState` consumed unchanged. The lock layer wraps these.
- **`extensions/claude-marketplace/persistence/locations.ts`** -- Phase 7 ADDS `stateLockFile(loc: ScopedLocations): string` returning `<scopeRoot>/claude-marketplace/.state-lock` (containment via `assertPathInside`).
- **`extensions/claude-marketplace/domain/manifest.ts`** -- the NFR-8 seam. No code change in Phase 7; the architecture test verifies the seam.
- **`extensions/claude-marketplace/presentation/soft-dep.ts`** -- Phase 7 D-03 MOVES `softDepStatus`, `hasLoadedPiSubagents`, `hasLoadedPiMcpAdapter` to `platform/pi-api.ts`. This file becomes a one-line re-export shim so Phase 4/5 callsites stay green.
- **`extensions/claude-marketplace/shared/markers.ts`** -- Phase 7 ADDS `STATE_LOCK_HELD_PREFIX = "Another claude-marketplace operation is in progress for"`. Markers-snapshot test gains one case.
- **`extensions/claude-marketplace/shared/errors.ts`** -- Phase 7 ADDS `StateLockHeldError extends Error` (carries the scope name) and `AggregateResourcesDiscoverError extends Error` (carries the per-scope error array via `Error.cause`).
...
- **E2E Layer A (programmatic) consumes `index.ts` directly:** No new entrypoint needed; tests `import claudeMarketplaceExtension from "../../extensions/claude-marketplace/index.ts"` and instantiate it with a tiny `mockPi: ExtensionAPI` record-and-replay shim.
...
- **`tests/architecture/manifest-read-seam.test.ts`:** Walk every `.ts` file under `extensions/claude-marketplace/` (excluding `domain/manifest.ts` and `tests/`). Assert no source-text occurrence of `marketplace.json` in a `readFile`-call context (regex-based check matching the precedent of `no-orchestrator-network.test.ts`).
