...
No user decision remains open. The only executor-level choice is whether P109-15 can cover the exceptional filesystem arms without a production edit; if not, the locked discretion explicitly permits the smallest internal behavior-preserving dependency-access refactor, with no public export or test mode. [VERIFIED: D-04 and the agent's Discretion; path-safety.ts:9-147]
