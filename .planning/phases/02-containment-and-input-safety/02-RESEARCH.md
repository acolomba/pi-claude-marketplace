...
The implementation order should be three independently reviewable waves: MCP classification, shared path containment plus affected consumers, then root lifecycle recovery. The first wave must make the raw boundary truthful and route stage and unstage through one classifier. The second must reject raw child `..` segments before normalization, then use normalized operands for containment and the existing-component walk so no physical component is erased before its safety decision. The third must catch only aggregate discovery at the root callback after prior lifecycle work has completed, return empty paths, and leave the next invocation clean. [VERIFIED: `.planning/phases/02-containment-and-input-safety/02-CONTEXT.md:15-94`; Phase 1 `SHC-F001`/`SHC-F051` evidence]
...
The internal classification names are discretionary, but the implementation preserves the three JSON-realizable states: absent field, present object map, and present malformed null/string/array/boolean/number. The selected public outcome is `MalformedMcpServersError`, the smallest caller-compatible typed shape because existing stage and unstage callers already propagate typed failures; a new result arm would require unrelated lifecycle churn. [RESOLVED: caller evidence in `extensions/pi-claude-marketplace/bridges/mcp/index.ts`, `extensions/pi-claude-marketplace/orchestrators/plugin/install.ts`, and `extensions/pi-claude-marketplace/orchestrators/marketplace/shared.ts`; locked contract in `.planning/phases/02-containment-and-input-safety/02-CONTEXT.md:41-54`]
...
1. Add a production-used `LexicalTraversalError extends PathContainmentError`, with normalized parent/child fields and an exact message stating that lexical traversal is forbidden. Split the raw child spelling into components and decide refusal for any segment exactly equal to `..` before normalization or filesystem inspection; then normalize refused operands only to construct the diagnostic and throw without containment/walking. Safe redundant `.` spellings remain accepted. [RESOLVED: existing subclass precedent in `SymlinkRefusedError`; real `root/a` symlink plus raw `root/a/../b` regression for `SHC-F001`/`SHC-F051`]
2. Compute `normalizedParent = path.resolve(parent)` and `normalizedChild = path.resolve(child)` only after that lexical gate, then apply the existing `path.relative` containment predicate. Preserve equality and absolute child paths that remain inside the normalized root. [VERIFIED: `extensions/pi-claude-marketplace/shared/path-safety.ts:42-51`; `.planning/phases/02-containment-and-input-safety/02-CONTEXT.md:29-36`]
3. Walk normalized relative segments from the normalized parent. `lstat` each existing component in order, including intermediates and an existing leaf. Refuse the first symbolic link using the existing `SymlinkRefusedError`; stop cleanly at the first `ENOENT` because descendants cannot yet exist. This walk protects accepted lexical paths; it does not detect a symlink component erased by normalization. [RESOLVED: `path-safety.ts:87-137`; `SHC-F001`/`SHC-F051`; locked PDEF-02 behavior]
4. Report normalized `parent` and `child` on every error-family member. Preserve ordinary PathContainmentError and SymlinkRefusedError messages; the lexical subclass must override the factually false inherited escape message with `${label} contains forbidden lexical traversal (parent: ${normalizedParent}, target: ${normalizedChild}).`. [RESOLVED: current constructors and public fields in `extensions/pi-claude-marketplace/shared/path-safety.ts:9-40`]
...
The shared classifier covers parsed JSON states only: absent field, object map, and malformed null/string/array/boolean/number. JavaScript `undefined` is not representable in stored JSON, so it receives no public-flow regression and no special production branch. The classifier remains an ordinary stage export solely because unstage imports it; the type owner uses type-only assertions and no direct classifier-only runtime test is required. [RESOLVED: JSON.parse data model and the stage/unstage ownership selected in the Phase 2 plan]
...
**Avoidance:** Reject raw child components exactly equal to `..` with LexicalTraversalError before normalization, because resolving `root/a/../b` erases a even when a is a physical symlink. Keep normalized fields but use a traversal-specific message rather than claiming the normalized child escapes. After that gate, use only normalized operands for comparison, walking, and typed error fields. [RESOLVED: SHC-F001/SHC-F051 regression design and locked PDEF-02 contract]
...
| ----------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PDEF-03     | `tests/bridges/mcp/types.test.ts` for truthful type assertions; `stage.test.ts`; `unstage.test.ts` | Focused install/reinstall/update or marketplace-shared suites only where the typed outcome changes caller behavior                                                                                  | Sibling `null`, string, array, boolean, and number cases; exact typed failure; unchanged bytes and metadata; missing remains frozen no-op; object path still works. The shared classifier needs no direct classifier-only runtime case. [RESOLVED: current test owners and locked contract] |
...
| ------------------------------------------------ | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Raw `..` traversal and normalization erasure     | Tampering                          | Throw LexicalTraversalError with normalized fields and an honest traversal-specific message from the pre-normalization gate; then enforce normalized relative containment. [RESOLVED: `SHC-F001`, `SHC-F051`] |
...
| A2  | Stored JSON cannot produce an own `mcpServers` property whose value is `undefined`; no public-flow case or special production branch is planned. | Classify once pattern  | None; this is resolved by the JSON data model and keeps tests on realizable disk input. |

## Open Questions (RESOLVED)

1. **What exact public typed failure shape should stage and unstage expose? — RESOLVED**
   - Selected decision: use `MalformedMcpServersError` with stable source-path and classified-value-kind fields, propagated by both public flows before enumeration or mutation.
   - Evidence: immediate bridge, install, update/reinstall, and marketplace-shared callers already propagate typed preparation/removal failures. A rejection class is the smallest caller-compatible public shape; adding result arms would force unrelated lifecycle changes.

2. **Should explicit `mcpServers: undefined` count as missing? — RESOLVED**
   - Selected decision: it is outside the stored-document public-flow matrix because JSON cannot represent `undefined`; no special production branch and no classifier-only runtime case is added.
   - Evidence: `JSON.parse` can produce absence, object, null, string, array, boolean, and number for this field, but never an own property with an `undefined` value. Type-only RawMcpDoc checks remain honest without inventing impossible disk input.
