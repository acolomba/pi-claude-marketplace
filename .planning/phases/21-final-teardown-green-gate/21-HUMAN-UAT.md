...
[gap closure pending — `/gsd-plan-phase 21 --gaps` to schedule the fix]
...
result: FAIL — every `plugin list` call that finds at least one currently-installed plugin emits the `/reload to pick up changes` trailer.
