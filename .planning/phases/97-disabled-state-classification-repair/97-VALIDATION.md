...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 97-01 T1 (tracer) | 97-01 | 1 | ENBL-05, ENBL-06 | T-97-02, T-97-06 | Predicate stays narrow (soft-degraded ENABLED records keep materializing); no local twin can re-appear | unit + integration (byte-exact) | `node --test tests/orchestrators/reconcile/plan.test.ts tests/orchestrators/plugin/info-manifest-absent.test.ts tests/orchestrators/reconcile/plan-convergence.test.ts tests/orchestrators/edge-deps.test.ts && npm run typecheck` | ✅ | ⬜ pending |
| 97-01 T2 | 97-01 | 1 | ENBL-05 | T-97-02 | Classifier precedence pinned in both directions | unit | `node --test tests/orchestrators/plugin/plugin-state-classifier.test.ts tests/orchestrators/edge-deps.test.ts` | ✅ | ⬜ pending |
| 97-02 T1 | 97-02 | 2 | ENBL-06 | T-97-05 | Disabled row is bare; no reason brace can leak a manifest-absence claim | integration (byte-exact) | `node --test tests/orchestrators/plugin/list.test.ts` | ✅ | ⬜ pending |
| 97-02 T2 | 97-02 | 2 | ENBL-06 | T-97-09 | Skip note reports the true cause | integration (byte-exact) | `node --test tests/orchestrators/plugin/info-manifest-absent.test.ts` | ✅ | ⬜ pending |
| 97-02 T3 | 97-02 | 2 | ENBL-06 | T-97-08 | Binding contract prose matches shipped trigger | architecture (byte gate) + lint | `node --test tests/architecture/catalog-uat.test.ts && npm run lint && npm run format:check` | ✅ | ⬜ pending |
| 97-03 T1 | 97-03 | 2 | ENBL-07 | T-97-04, T-97-10, T-97-11 | Partial gate widens to the partially-available arm only; unavailable still rejected; fail-clean before any ledger phase | integration (byte-exact) | `node --test tests/orchestrators/plugin/enable-disable.test.ts && npm run typecheck && npm run lint` | ✅ | ⬜ pending |
| 97-03 T2 | 97-03 | 2 | ENBL-07 | T-97-11 | Idempotent disable runs no cascade (proven by unchanged state bytes) | integration (byte-exact) | `node --test tests/orchestrators/plugin/enable-disable.test.ts` | ✅ | ⬜ pending |
| 97-04 T1 | 97-04 | 2 | ENBL-08 | T-97-01 (high), T-97-13 | Load-time backfill cannot re-materialize or re-enable a user-disabled plugin | unit (seam) | `node --test tests/orchestrators/reconcile/backfill.test.ts && npm run typecheck && npm run lint` | ✅ | ⬜ pending |
| 97-04 T2 | 97-04 | 2 | ENBL-08 | T-97-12 | Planner is a fixed point; no repeated unstage cascade | unit (pure planner) | `node --test tests/orchestrators/reconcile/plan.test.ts tests/orchestrators/reconcile/plan-convergence.test.ts` | ✅ | ⬜ pending |
| 97-05 T1 | 97-05 | 2 | ENBL-09 | T-97-03 | Persisted availability discriminant agrees with the unsupported list | integration (state assertion) | `node --test tests/orchestrators/plugin/update.test.ts && npm run typecheck && npm run lint` | ✅ | ⬜ pending |
| 97-05 T2 | 97-05 | 2 | ENBL-09 | T-97-14, T-97-15 | No artifact re-staged on disk; operation is a fixed point | integration | `node --test tests/orchestrators/plugin/update.test.ts` | ✅ | ⬜ pending |
