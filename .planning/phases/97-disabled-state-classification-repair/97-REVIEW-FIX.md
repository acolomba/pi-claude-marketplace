...
**Fixed at:** 2026-08-10T01:05:00Z
**Source review:** `.planning/phases/97-disabled-state-classification-repair/97-REVIEW.md`
**Iteration:** 3 — FINAL (cumulative disposition across all three iterations)

**Summary — cumulative across the fix loop:**

- Findings actioned in scope: 5 (WR-07, WR-08, WR-09 in iteration 2; WR-10
  fixed and IN-07 carried in iteration 3), on top of iteration 1's 8
- Fixed: 4 (WR-07, WR-08, WR-09, WR-10)
- Carried with a todo: 1 (IN-07)
- Skipped: 0
- Info findings (IN-01..IN-06): recorded deferrals — see below

Iteration 3 closed the two residuals the final re-review raised after verifying
all three iteration-2 fixes RESOLVED. Nothing is outstanding.
...
integration tests, 0 failures). Log at `97-fix4-check.log` (iteration 2's run is
`97-fix3-check.log`). `pre-commit run --files <every touched path>` also clean
(exit 0) for every commit in this iteration.
...
| WR-07 | WARNING | fixed (iteration 2), verified RESOLVED | `8367bb10` |
| WR-08 | WARNING | fixed (iteration 2), verified RESOLVED | `97b600b7` |
| WR-09 | WARNING | fixed (iteration 2), verified RESOLVED | `72316b7e` |
| **WR-10** | **WARNING** | **fixed** | `7ccf5e51` |
| **IN-07** | **INFO** | **carried + carrier todo** | `aed50530` |
| IN-01..IN-06 | INFO | recorded deferrals (not attempted) | — |
...
### WR-10: the enable-arm dispatch comment claimed a severity rule that no longer held

**Files modified:** `orchestrators/reconcile/notify.ts`
**Commit:** `7ccf5e51` — comment only, no behavior change.

The `case "plugin-enabled":` dispatch comment still read "SEV-03: both arms stay
`info` — the degradation predates the enable". That was true when CR-01 wrote it
and stopped being true at `8367bb10`, which made `enabledRowFromOutcome` raise
to `warning` on a malformed-frontmatter degrade. A stale comment at the dispatch
site is worse than none: it asserts a rule the function three lines below
contradicts.

Restated to the rule that actually holds — severity splits on WHICH degradation
the ledger reported, not on which projection arm was picked:

- dropped-kind shortfall → `info` (SEV-03 parity; it predates the enable, so the
  request was fully carried out — the operator decision, unchanged)
- malformed-frontmatter degrade → `warning` (WARN-01 parity with the install row
  for the same ledger run)

`npx eslint` clean on the file and on
`tests/orchestrators/reconcile/notify.test.ts`; the D-75-01 vocabulary guard
passes (no bare backtick verdict token introduced — the earlier iteration-2 near
miss was watched for here).

## Carried (iteration 3)

### IN-07: the install arm drops `orphanRewake`, the inverse of WR-07

**Carrier:** `.planning/todos/pending/2026-08-09-install-arm-orphan-rewake-asymmetry.md`
(`resolves_phase: "98"`, `source: 97-REVIEW.md IN-07`)
**Commit:** `aed50530` — carrier only, no code change.

Verified on the source before writing the carrier: `installPlugin`'s standalone
row reads `installCtx.resolved.orphanRewake` directly (`install.ts:1768`), but
the orchestrated `InstallPluginOutcome` it returns carries `degradedKinds` only
(`:1856-1865`) — no `orphanRewake` field exists on it, so `apply.ts` has nothing
to thread and `installedRowFromOutcome` composes from `degradedKinds` alone.

Net effect after WR-07: the load-time reconcile cascade reports
`{orphan rewake}` for a **re-enable** but not for a **fresh install** of the
same plugin. Two sibling arms of one cascade disagree about whether the signal
is worth reporting.

**Why carried, not fixed:** pre-existing on the install arm — it predates the
enable-arm work entirely; WR-07 only made it visible by closing the same gap on
the other side. The fix widens the install outcome contract and its projection,
which is install-surface scope rather than the disabled-state classification
repair this loop was scoped and capped to. Severity is low: the token is
advisory (a plugin-authoring bug, not a failed operation) and the surface a user
actually invokes already reports it correctly.

The carrier records the four threading sites, notes that severity is unaffected
(`orphan rewake` moves no severity channel on any surface), and raises the
design question worth deciding in Phase 98: whether `EnableDegradationSignals`
should be widened into a shared shape BOTH the install and enable outcomes
inherit — which is what would make this class of asymmetry a compile error
instead of a review finding. No new catalog state is needed;
`success-with-orphan-rewake` already pins the rendered form.
...
All three land in Phase 98; none was touched in iterations 2 or 3.
...
Iteration 3 added ONE carrier — IN-07, documented above
(`2026-08-09-install-arm-orphan-rewake-asymmetry.md`, `resolves_phase: "98"`).
Every warning-severity finding raised across all three iterations is either
fixed or carried; none is silently dropped.

### Info findings — recorded deferrals (not attempted)

Out of fix scope by declared policy. Recorded here so they are visible to the
next reader rather than lost with the review artifact.
...
| `7ccf5e51` | `fix(97): correct the stale enable-arm severity comment` |
| `aed50530` | `docs(97): carry the install-arm orphan-rewake asymmetry forward` |

---

_Fixed: 2026-08-10T01:05:00Z_
