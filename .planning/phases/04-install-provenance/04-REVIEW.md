...
## Operator Decisions (2026-09-16, before the fix pass)

The fixer applies THESE resolutions, which override the "Fix" prose above where they differ.

- **CR-01 -> shape (a).** The planner respects provenance on the marketplace-removal path: in `diffMarketplaces`, a recorded-but-undeclared marketplace is retained while any record under it carries `provenance: "dependency"`; `buildUninstallBucket`'s retained set is widened by the same predicate so that marketplace's `"explicit"` orphans are still swept. `claude-plugins.json` stays pure (D-04-02); `plan.ts` stays pure and network-free. Add the `install-flow.test.ts` case with the dependency seeded in a user-scope marketplace and a project-scope install (`marketplacesToRemove` empty, dependency survives `applyReconcile`), plus `plan.test.ts` cases for the retention and for the explicit-orphan sweep under a retained-by-dependency marketplace. Reword `docs/dependency-resolution.md:116` to match.
- **WR-01 -> import promotes.** In `executeScopedPlan`, a `"dependency"` record falls through to `installOnePlannedPlugin` so the orchestrated promotion runs; the imported config naming the plugin IS the user asking for it by name. Add the `execute.test.ts` case (pre-seeded `"dependency"` record named by the imported settings reads `"explicit"` afterwards).
- **WR-02 -> promotion ENABLES a disabled record.** Operator rule: a plugin asked for by name is enabled. The promotion arm flips `provenance` AND, when the record is disabled (`isRecordedButDisabled`), enables it through the existing enable branch (materialize, drop the `enabled: false` marker, config entry becomes the plain declaration) so the `(installed) {already installed, dependency promoted}` row is true as rendered. No new status token, no new reason, no new catalog row. Because materialization now happens on that arm, the row carries the reload-hint trailer in the disabled->enabled case only if the existing enable path already emits one -- follow the enable precedent, do not invent one. The `--local` arm must not clobber a base `{ enabled: false }` into an implicit enable by omission: the promotion writes what the enable path writes. Add the `install-flow.test.ts` case that disables the seeded dependency before promoting it.
- **WR-03 -> partial consent, pin refusal, map-model irrelevant.** (1) A version pin (`pinVersionOverride`) on the promotion arm refuses promotion -- the existing `(failed) {already installed}` refusal stands and `update`/`reinstall` are the remedies. (2) `--partial` is the consent gate for a partially-installed record (`compatibility.installable === false`): promoting such a record by name WITHOUT `--partial` refuses; WITH it, promotes. On a fully-supported record `--partial` is harmless. (3) `--map-model` has no bearing on promotion: it is neither a refusal trigger nor consumed. Add cases for the pin refusal and for the partial gate both ways. Document (1)-(3) in the catalog paragraph for the promoted row and in `docs/dependency-resolution.md`.
- **IN-01, IN-02, IN-03 -> fix all three** as written above (fix scope: all).

Recorded to `.planning/BACKLOG.md`, NOT in this phase's scope: (i) the cascade case -- a disabled, already-installed dependency of a plugin being installed should become enabled (upstream parity), retiring the RESV-05 `{already installed, dependency disabled}` warning skip; (ii) dependency status propagation -- a plugin whose dependency is partially-installed is itself partial, with reasons, on its rows.
