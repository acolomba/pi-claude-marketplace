...
- **RESOLVED 2026-09-13, verified at milestone close.** The condition no longer holds. `shared/README.md` now names `shared/notification-dispatch.ts` as "the sole Pi notification boundary" and states it is "the SOLE sanctioned `ctx.ui.notify(` call site"; `shared/notify.ts` does not exist in the tree. The cleanup this entry waited on has landed, so nothing is carried forward. Kept as a record, not as debt.
  status: acknowledged
...
- **PROMOTED TO BACKLOG 2026-09-13** at milestone close: tracked as `NEGCTL-01` in `.planning/BACKLOG.md`. Still open; CI pins Node 24, so it is latent rather than red.
  status: acknowledged
