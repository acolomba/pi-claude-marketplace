...
| **112-25** `routing-state.ts` | FAIL: `88.96/100/20`. | Directly prove all exported state verbs: epoch current/bump/reset; pending empty skip/append/order/read/clear; parsed cache set/overwrite/delete/missing/read; bucket default/set/replace/read/all; composite reset. Quote private cell inventory as `parsedConfigCache`, `routingTable`, `liveEpoch`, `pendingSessionStartContext`; do not expose cells. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/routing-state.ts:141-164,172-326] | Leaf after 13’s predicate shape; Wave 2. Replace global `beforeEach` with case-owned lifecycle cleanup. Absorb routing-state cases from `session-start-additional-context`. [VERIFIED: tests/bridges/hooks/routing-state.test.ts:1-18] [VERIFIED: .planning/phases/112-hook-runtime/112-CONTEXT.md:87-101] |
| **112-26** `settle.ts` | FAIL: only `"pending"` branch uncovered, but current tests depend on hidden readers. | Prove epoch/cache one-shot behavior, every exact stop reason (`"stop"`, `"error"`, `"length"`, `"pending"`, `"aborted"`, `"toolUse"`, `"deferred"`), Stop aggregate precedence, StopFailure all-observer discard, post-await epoch guard, message rendering, bounded shared block/context re-entry, exact eighth suppression, one-shot notify, genuine-input reset, send failure, and no bucket paths through outputs and lifecycle. Exact custom type is `"claude-hook-stop-block"`; cap is `8`. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/settle.ts:38-119,141-220,238-433] | Depends on 05, 22-23, 25. Rewrite proof through `sendMessage`, executor events, next payloads, and existing reset lifecycle instead of `settleCacheSnapshot`/`loopProtectionState`; do not add another reader. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/settle.ts:435-457] [VERIFIED: .planning/phases/112-hook-runtime/112-CONTEXT.md:87-101] |
| **112-27** `spawn-helpers.ts` | MISSING owner. | Prove args-present exec form including `[]`, mixed argument stringification, args-absent shell form, string shell, missing command, serialization below/exact cap/one byte over/multibyte, marker overwrite, object no-mutation, oversized primitive/array wrapping, permitted overshoot. Exact cap is `256 * 1024` bytes and marker is `"_truncated": true`. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/spawn-helpers.ts:16-45,48-85] | Leaf with 25 type input; Wave 1. Absorb serialization/spawn-plan assertions from `dispatch-exec`, `hooks-exec`, and `hooks-async-rewake`. [VERIFIED: supplemental-suite audit, 2026-08-30] |
| **112-28** `stage.ts` | FAIL: `76.47/61.54/71.43`. | Real-FS cases for missing/non-directory subtrees, regular/nested dirs, in-tree symlink boundary, escaping direct/buried symlink, macOS realpath normalization, exact typed containment error fields, unexpected I/O propagation, unreadable readlink fallback `"<unreadable>"`, safe-name rejection, atomic write bytes/idempotence, containment, remove existing/missing. Remove the provably unreachable `stack.pop()` guard rather than manufacturing an impossible state. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/stage.ts:40-174,176-237] | Leaf, Wave 1. Absorb and remove `tests/bridges/hooks/symlink-escape.test.ts`; no `skip`-gated duplicate suite. [VERIFIED: supplemental-suite audit, 2026-08-30] [VERIFIED: .claude/rules/typescript-unit-testing.md:36-42,221-224] |
...
| 2 — matcher and state composition | 11 after 12; 13 after 11/12; 25 after 13 | The Bash owner names `CompiledBashGlob`; the index owner composes Bash/glob; routing entries store `IfPredicate`. Let 13 be the sole carrier that prunes `hooks-if-field.test.ts` after leaf evidence exists. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/if-field/bash.ts:444-458] [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/if-field/index.ts:55-112] [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/routing-state.ts:75-87] |
| 3 — adapters and async engine | 06 after 08/25; 02 after 01/03/09/10/15-24/25/27/29/30 | Adapters append routing state; registry composes nearly every process leaf. Give 06 sole ownership of `hooks-adapters.test.ts` and 02 sole ownership of pruning `hooks-async-rewake.test.ts`. [VERIFIED: source import graph and supplemental audit, 2026-08-30] |
| 4 — blocking execution and reduction | 04 after 02/03/09/10/15-24/27/29-31; 05 after 04/06/08/13/25 | Blocking execution shares types and process primitives with registry; reduction composes execution, predicates, adapters, and routing state. Give 04 sole ownership of `hooks-exec.test.ts` and 05 sole ownership of `hooks-reducer.test.ts`. [VERIFIED: source import graph via CodeGraph, 2026-08-30] |
| 5 — settlement and bridge registration | 26 after 05/22/23/25; 07 after 02/05/25/26 | Settlement collects dispatch outcomes and payloads; router owns final lifecycle wiring. Give 07 sole ownership of deleting `session-start-additional-context.test.ts` and pruning runtime cases from `hooks-dispatch.test.ts`. [VERIFIED: source import graph and supplemental audit, 2026-08-30] |
| 6 — barrel and phase gates | 14 after 07/28 | Binding identity should be proven only after final public sources settle; then run all-pair correspondence/direct coverage and `npm run check`. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/index.ts:13-24] [VERIFIED: package.json:75-95] |
...
| `tests/architecture/hooks-translators.test.ts` | Retain narrowly | Keep only cross-module completeness: every dispatchable event maps to a translator and tool translators share the name mapping; remove byte-equal per-translator duplicates. [VERIFIED: tests/architecture/hooks-translators.test.ts:172-270] |
...
1. **Should already-exported test-oriented readers be removed or merely stop receiving new callers?**
   - What we know: `asyncRewakeEntries`, `awaitPidTablePersist`, `settleCacheSnapshot`, and `loopProtectionState` are exported from concrete modules, their comments name test observation/cleanup, and production call-site search found no consumers. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/async-rewake/registry.ts:178-203] [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/settle.ts:435-457] [VERIFIED: CodeGraph/`rg` call-site audit, 2026-08-30]
   - What is unclear: D-03/D-05 require internal metadata to stay private and forbid test-only state readers, while the phase also preserves current public surfaces; removing an existing module export narrows a source-level surface. [VERIFIED: .planning/phases/112-hook-runtime/112-CONTEXT.md:29-37] [VERIFIED: .planning/ROADMAP.md:319-325]
   - Recommendation: owner tests must stop depending on hidden readers first; preserve the exports unless the owning plan proves no production caller, records the narrowing as removal of an internal-only surface, and passes typecheck/fallow. Do not make this question block the test rewrite. [VERIFIED: .planning/REQUIREMENTS.md:79-106]

2. **Which existing boundary should stage PID-table write/unlink failures?**
   - What we know: the missing branches catch any write failure and any non-`ENOENT` unlink failure, while D-21 permits a narrow existing double only for hard-to-stage I/O. [VERIFIED: extensions/pi-claude-marketplace/bridges/hooks/async-rewake/pid-table.ts:142-174] [VERIFIED: .planning/phases/112-hook-runtime/112-CONTEXT.md:99-101]
   - What is unclear: the most portable failure across Linux/macOS may be a case-owned directory/permission shape rather than permissions alone. [VERIFIED: environment probe reports Linux, 2026-08-30]
   - Recommendation: prefer real deterministic directory-at-target failures; if one branch cannot be staged portably, inject or mock only the already-imported filesystem/atomic-write boundary through the current test context without adding a production parameter. [VERIFIED: .planning/phases/112-hook-runtime/112-CONTEXT.md:127-128] [VERIFIED: .claude/rules/typescript-unit-testing.md:151,221-224]
