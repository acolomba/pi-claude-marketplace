...
**parseCommandArgs callback contract -- PINNED CASE A (verified at planning time from edge/args-schema.ts:71-84):** The `parseCommandArgs` function takes an `notifyError: (message: string) => void` callback (args-schema.ts line 71). The callback receives a PLAIN STRING with NO trailing USAGE concatenation. Specifically:
- args-schema.ts line 33: `parseArgsOrNotify` passes `errorMessage(err)` to the callback (the parsing-error text, no USAGE).
- args-schema.ts line 84: when a required positional is missing, the callback receives `schema.usage` (the usage string ALONE, not concatenated with an error reason).
- Conclusion: the callback's `message` argument is EITHER a plain parser-error string OR the usage string standing alone. Neither case includes a `\n` followed by USAGE -- there is no trailing-usage suffix appended downstream.

Per Case A, Task 3 (marketplace handlers) wraps the existing callback to call `notifyUsageError(ctx, message, USAGE)` -- passing the received `message` as arg 2 and the closure-scope `USAGE` / `usage` literal as arg 3. **args-schema.ts is NOT modified by this plan** -- the callback contract is correct as-is; only the handlers' callback closure is updated.
...
From extensions/pi-claude-marketplace/edge/args-schema.ts (Case A confirmed at planning time -- file lines 26-99):
- `parseCommandArgs<Spec>(args: string, schema: { positional: Spec; usage: string }, notifyError: (message: string) => void): ParsedCommandArgs<Spec> | undefined` (line 68-72).
- The `notifyError` callback receives a plain string -- EITHER `errorMessage(err)` from `parseArgsOrNotify` (line 33) OR `schema.usage` alone (line 84). NEVER `${reason}\n${usage}`; NEVER concatenated.
- args-schema.ts is unchanged by this plan.

From extensions/pi-claude-marketplace/edge/router.ts:
- Router uses `notifyUsageError(ctx, message, usageBlock)` directly (lines 125, 148, 161, 181) where `usageBlock` is a separate parameter. This confirms the project pattern: `notifyUsageError` always takes a (message, usageBlock) pair -- the migration in this plan follows the same shape at every callsite.
...
- `notifyError(ctx, message)` at line 28 (inside parseCommandArgs callback)  → `notifyUsageError(ctx, message, USAGE)`. Case A confirmed: parseCommandArgs hands the callback a plain `message` (either an error reason or `schema.usage` alone) -- the callback closure simply re-routes to `notifyUsageError(ctx, message, USAGE)` with the closure-scope USAGE literal as arg 3.
...
<task type="auto">
  <name>Task 1: Audit existing router test byte-shape (parseCommandArgs callback contract is Case A -- pinned at planning time)</name>
  ...
  <read_first>
    ...
    - extensions/pi-claude-marketplace/edge/router.ts (the router itself -- confirm it doesn't pre-compose the usage block before delegating to handlers; the planner already confirmed router.ts uses `notifyUsageError(ctx, message, usageBlock)` at lines 125/148/161/181)
    - extensions/pi-claude-marketplace/edge/args-schema.ts (READ-ONLY in this plan -- Case A is confirmed; no edits to this file)
    - extensions/pi-claude-marketplace/shared/notify.ts:95-97 (canonical notifyUsageError contract)
  </read_first>
  <action>
    **parseCommandArgs callback contract is Case A (PINNED at planning time -- args-schema.ts:71 `notifyError: (message: string) => void` is called with `errorMessage(err)` or `schema.usage` alone; never concatenated).** This task only audits the existing router-level byte-shape assertions; no parseCommandArgs change is required.

    Two audits to run before editing the 6 handler files:

    1. Open `tests/edge/router.test.ts` lines 70-86. Identify whether the test asserts:
    ...
    2. Sweep adjacent edge tests for the same pattern: `tests/edge/handlers/**/*.test.ts` -- any handler test that captures a notify body for an argument-validation failure may pin the wrong byte-shape. Use the grep `grep -rn 'Usage:' tests/edge/ | head -30`. Inspect any `assert.equal` / `assert.deepEqual` that includes a string literal containing `\nUsage:` (single backslash-n). Flip to `\n\nUsage:` if found.
  ...
  <verify>
    <automated>
      ...
      # (a result here = a test that needed flipping to \\n\\nUsage: -- check it was flipped)
      # Confirm Case A pin is documented in the SUMMARY:
      grep -c 'Case A' .planning/phases/14-drift-guard-test-alignment/14-02-SUMMARY.md 2>/dev/null || echo "SUMMARY not yet created"
      # Expect: a Case A reference once SUMMARY exists
    </automated>
  </verify>
  <done>
    1. Existing router test at `tests/edge/router.test.ts:70-86` reviewed; any wrong-shape pin has been flipped to the `\n\n` shape OR confirmed presence-only.
    2. Adjacent handler tests under `tests/edge/handlers/**/*.test.ts` swept for the same wrong-shape pattern.
    3. Findings recorded in this task's SUMMARY block for downstream tasks (including the Case A pin).
    4. args-schema.ts UNCHANGED (Case A means no refactor needed).
  </done>
</task>

<task type="auto">
  <name>Task 2: Migrate 13 plugin-handler callsites to notifyUsageError</name>
  <files>
    extensions/pi-claude-marketplace/edge/handlers/plugin/list.ts,
    extensions/pi-claude-marketplace/edge/handlers/plugin/reinstall.ts,
    extensions/pi-claude-marketplace/edge/handlers/plugin/update.ts,
...
<task type="auto">
  <name>Task 3: Migrate 2 marketplace-handler callsites to notifyUsageError (Case A migration) + run full check</name>
  <files>
    extensions/pi-claude-marketplace/edge/handlers/marketplace/list.ts,
    extensions/pi-claude-marketplace/edge/handlers/marketplace/autoupdate.ts
  </files>
  <read_first>
    ...
    - extensions/pi-claude-marketplace/edge/args-schema.ts:71-84 (the `parseCommandArgs` callback contract -- Case A pinned: callback receives plain string, no USAGE concatenation)
    - extensions/pi-claude-marketplace/shared/notify.ts:95-97 (notifyUsageError signature)
  </read_first>
  <action>
    Migrate the 2 marketplace-handler callsites listed in CONTEXT.md. Per the **Case A** pin (planner-verified at args-schema.ts:71-84 -- the `notifyError` callback receives a plain string with no USAGE concatenation), the migration is identical in shape across both handlers: wrap the existing callback so it forwards the `message` to `notifyUsageError(ctx, message, <usage-block>)`. **args-schema.ts is NOT modified by this plan.**

    **list.ts:28**:
    - Current: `(message) => { notifyError(ctx, message); }` (callback passed to parseCommandArgs)
    - Updated: `(message) => { notifyUsageError(ctx, message, USAGE); }`
    - Update imports: drop `notifyError` from the `shared/notify.ts` import line (if no other callsite uses it in this file); add `notifyUsageError`.
    - The closure-scope `USAGE` literal is the existing constant at the top of list.ts -- the marketplace-list usage string.

    **autoupdate.ts:37**:
    - Current: `(message) => { notifyError(ctx, message); }` (callback passed to parseCommandArgs)
    - Updated: `(message) => { notifyUsageError(ctx, message, usage); }` -- note **`usage`** (lowercase), not `USAGE`: this is the closure variable holding the result of `usageFor(enable)` at line 28 (since the autoupdate handler builds the usage string dynamically based on the `enable` parameter).
    - Update imports: drop `notifyError`; add `notifyUsageError`.

    After both edits, the 2 marketplace-handler callsites produce on-the-wire bytes `${message}\n\n${usageBlock}` (notifyUsageError contract) instead of today's `${message}` (which had no usage block at all, because the parseCommandArgs callback's `message` parameter is the usage string alone -- not message-then-USAGE).

    NOTE: do not modify args-schema.ts. Case A is confirmed -- the callback contract is correct.
  ...
  <verify>
    <automated>
      cd /home/acolomba/pi-claude-marketplace
      # args-schema.ts must NOT be modified (Case A means no refactor needed):
      git diff --stat extensions/pi-claude-marketplace/edge/args-schema.ts 2>&1 | tail -3
      # Expect: empty (file unchanged)
      ...
      # Expect: ≥1
      grep -c 'notifyError(' extensions/pi-claude-marketplace/edge/handlers/marketplace/list.ts extensions/pi-claude-marketplace/edge/handlers/marketplace/autoupdate.ts
  ...
  <done>
    1. `list.ts:28` and `autoupdate.ts:37` callback closures route through `notifyUsageError` with the USAGE / usage block passed as arg 3 (per the on-wire `\n\n` separator contract).
    2. `args-schema.ts` is UNCHANGED (Case A means no refactor needed).
...
| T-14-04 | Tampering | parseCommandArgs callback contract | accept | Case A pinned at planning time (args-schema.ts:71-84) -- callback receives plain string. No refactor of args-schema.ts; mitigation is the planner-confirmed contract. |
</threat_model>
...
- `args-schema.ts` is UNCHANGED by this plan (Case A pin means no refactor needed).
...
5. args-schema.ts callback contract (Case A) preserved without modification.
</success_criteria>
