...
must_haves:
  truths:
    - "19 meta-assertion rule files exist under tests/lint-rules/ -- one per MSG-* ID in the meta-assertion set listed in RESEARCH.md §5 (MSG-GR-1..5, MSG-IC-1..3, MSG-SD-3, MSG-PL-1..6, MSG-ER-1)."
    ...
    - "`tests/lint-rules/index.js` `RULE_NAMES` array includes all 19 names; `rules:` object exports all 19 rule modules under the same names."
  ...
  artifacts:
    ...
    - path: tests/lint-rules/index.js
      provides: "Plugin entry; RULE_NAMES + rules object populated with the 19 meta-assertion rules"
      contains: "RULE_NAMES"
  key_links:
    - from: tests/lint-rules/index.js
      to: "tests/lint-rules/msg-*.js (19 files)"
      via: "ESM named imports + plugin rules dict"
      pattern: "import.*from.*msg-"
---

<objective>
Land 19 of the 34 MSG-* drift-guard rules -- the meta-assertion subset. Per D-14-09 (LOCKED), rules whose semantic content is already structurally enforced (by the `RowSpec` discriminated union in `presentation/compact-line.ts`, by the `Reason` literal-union in `shared/grammar/reasons.ts`, or by the byte-equality assertions in `tests/architecture/catalog-uat.test.ts`) become thin "structural meta-assertion" rules. Each meta-assertion rule cites the structural mechanism in its `meta.docs.description`, contains a no-op `Program: () => {}` visitor (RESEARCH.md Pitfall 8 mitigation), and is paired with a companion `*.test.js` file containing a SMOKE-level `valid:` RuleTester case.

Per RESEARCH.md §5 (verbatim split -- copy-edit):

**Meta-assertion (19 rules; this plan):** MSG-GR-1, MSG-GR-2, MSG-GR-3, MSG-GR-4, MSG-GR-5, MSG-IC-1, MSG-IC-2, MSG-IC-3, MSG-SD-3, MSG-PL-1, MSG-PL-2, MSG-PL-3, MSG-PL-4, MSG-PL-5, MSG-PL-6, MSG-ER-1.

(That is 16 explicit IDs. Per CONTEXT.md the meta-assertion split is exactly 19. The additional 3 are: MSG-NC-1's structural cousin... read RESEARCH.md §5 again. The actual count per CONTEXT.md: meta-assertion family includes 19 entries -- the corrected split per RESEARCH.md §5 "Which MSG-* IDs are meta-assertion?" enumerates: MSG-GR-1..5 (5) + MSG-IC-1..3 (3) + MSG-SD-3 (1) + MSG-PL-1..6 (6) + MSG-ER-1 (1) = 16. RESEARCH.md §10 states 19/15 split. **EXECUTOR ACTION**: re-read RESEARCH.md §5 "Which MSG-* IDs are meta-assertion?" and confirm -- if the count is 16 in the section but 19 in the plan-decomposition section, that is a research-internal contradiction; resolve by following RESEARCH.md §5 verbatim (count 16) and adjust this plan to ship 16 meta-assertion rules. Plan 05 then ships 18 full-impl rules to reach 34 total. The frontmatter `files_modified:` and `truths:` enumerate 16 rules here based on the §5 enumeration. If the executor finds the 19 number authoritative elsewhere, append the additional rules accordingly.)

**Full-impl (15 rules per RESEARCH.md §10; covered in Plan 05):** MSG-SR-1..7 (7) + MSG-MR-1..2 (2) + MSG-RP-1 (1) + MSG-CC-1 (1) + MSG-NC-1..2 (2) + MSG-RH-1 (1) + MSG-LC-1..2 (2) + MSG-SD-1..2 (2) = 18. (Or 15 depending on the count source. The total of 34 holds: 16 meta + 18 full = 34.)

Purpose: Ships 16 (or 19) of 34 MSG-* rules as structural meta-assertions; satisfies SC #2 ("failure includes MSG-* rule ID") for these rules via the rule-name-embedded message; satisfies the registry parity test (Plan 05) for these IDs. Per D-14-09 (LOCKED meta vs. full split).
Output: 16-19 rule files + 16-19 RuleTester companion tests + updated tests/lint-rules/index.js; `npm run check` green.
</objective>
...
From RESEARCH.md §5 "Which MSG-* IDs are meta-assertion?" -- citation map for `meta.docs.description`:
...
- Currently empty `RULE_NAMES` + `rules: {}`. This plan populates both with the meta-assertion rules.
...
<task type="auto">
  <name>Task 1: Author 16 (or 19) meta-assertion rule files under tests/lint-rules/</name>
  ...
  <read_first>
    ...
    - .planning/phases/14-drift-guard-test-alignment/14-RESEARCH.md §5 "Which MSG-* IDs are meta-assertion?" (citation map per rule)
    ...
    - extensions/pi-claude-marketplace/presentation/compact-line.ts (load lines 62-260 -- covers the icon constants, the RowSpec union variants, the `renderRow` switch -- the structural enforcement points each rule cites)
  </read_first>
  <action>
    For each of the 16 (or 19 -- confirm count from RESEARCH.md §5 before starting) meta-assertion MSG-* IDs, create a `.js` file under `tests/lint-rules/` following the Pattern 2 template verbatim:

    File naming: lowercase `msg-<family>-<n>-<descriptive-slug>.js`. The `<descriptive-slug>` is a short hyphenated description (e.g., `msg-gr-1-line-grammar.js`, `msg-ic-3-blocked-icon.js`). The registry test in Plan 05 uses `name.startsWith("msg-gr-1-")` (slug-prefix matching), so the slug after the numeric ID is free-form but must be present.

    Per-file requirements (uniform across all meta-assertion rules):

    1. **Imports**: `import { ESLintUtils } from "@typescript-eslint/utils";`
    2. **createRule factory**: `const createRule = ESLintUtils.RuleCreator((name) => \`https://github.com/acolomba/pi-claude-marketplace/blob/main/docs/messaging-style-guide.md#${name.replace(/^msg-/, "msg-")}\`);` -- anchor URL points to the style guide's section. For the citation slug, prefer the MSG-ID lowercased (e.g., `#msg-gr-1`) -- the anchor is a documentation hint; the registry test does NOT verify the anchor URL, only the rule existence.
    3. **default export**:
       ```
       export default createRule({
         name: "msg-<family>-<n>-<slug>",
         meta: {
           type: "problem",
           docs: { description: "<MSG-X-N>: <one-sentence summary>. Structurally enforced by <enforcement point>; this rule cites the enforcement and exists for registry parity (D-14-09).", },
           messages: { structurallyEnforced: "<MSG-X-N> is structurally enforced; see <citation>.", },
           schema: [],
         },
         defaultOptions: [],
         create() {
    ...
    4. **Top-of-file comment** citing:
       ...
       - The specific structural enforcement mechanism (one short paragraph; copy from RESEARCH.md §5).
    ...
    No file should contain a real AST visitor -- all meta-assertion rules return `{ Program: () => {} }`.
  ...
  <verify>
    <automated>
      ...
      # Expect: 16 (or 19 if research is read as the higher count)
      # Each contains the no-op Program visitor:
      grep -l 'Program: () => {}' tests/lint-rules/msg-*.js | grep -v '.test.js' | wc -l
      # Expect: same count as above
      # Each has a messageId 'structurallyEnforced' or equivalent:
  ...
  <done>
    1. 16 (or 19) rule files exist under `tests/lint-rules/` matching the family/number split per RESEARCH.md §5.
    ...
    4. `npm run lint` green (rules exist as JS source but are not yet registered).
  </done>
</task>

<task type="auto">
  <name>Task 2: Author 16 (or 19) RuleTester companion test files</name>
  ...
  <action>
    For each rule file created in Task 1, create a sibling `*.test.js` file under `tests/lint-rules/` with the same base name + `.test.js` suffix. Files share the same structure (uniform across all 16+ tests):
    ...
    2. **node:test shim (REQUIRED per Pitfall 1)** -- exactly 4 lines, placed immediately after imports:
       ...
       RuleTester.itOnly = test.it.only;
       ```
    3. **RuleTester instance + run**:
       ```
       const ruleTester = new RuleTester();
       ruleTester.run("<rule-name-matching-the-rule-file-export>", rule, {
         valid: [
           { code: "/* meta-assertion smoke: any code is valid because the rule does no AST work. */" },
         ],
         invalid: [],
       });
       ```
       For some meta-assertion rules where a meaningful smoke case is appropriate (e.g., MSG-GR-1: include a tiny snippet showing a RowSpec usage), the `valid:` case can include something more substantive -- but it MUST be valid (no `invalid:` entries for meta-assertion rules; they can't fail).
  ...
  <verify>
    <automated>
      cd /home/acolomba/pi-claude-marketplace
      # File count match (every rule has a test):
  ...
  <done>
    1. Companion `*.test.js` count matches the rule-file count from Task 1 (1:1 pairing).
...
<task type="auto">
  <name>Task 3: Wire 16 (or 19) meta-assertion rules into tests/lint-rules/index.js + run full check</name>
  ...
  <action>
    Extend `tests/lint-rules/index.js` to import all 16 (or 19) meta-assertion rule files from Task 1 and register them in BOTH `RULE_NAMES` (frozen ordered array) AND `rules:` (plugin shape dict).
    ...
    2. `RULE_NAMES` becomes `Object.freeze(["msg-gr-1-line-grammar", "msg-gr-2-marketplace-token", ..., "msg-er-1-empty-token"])` -- listed in a stable order (family-then-numeric). Plan 05 will add its 15 full-impl rules to this same list.
    3. `default.rules` object: each rule's `name` field (which matches the rule-file slug) maps to the imported default. Plan 05 adds 15 more entries.
  ...
  <verify>
    <automated>
      cd /home/acolomba/pi-claude-marketplace
      # RULE_NAMES count matches the meta-assertion rule count:
      node -e "import('./tests/lint-rules/index.js').then(m => { console.log('RULE_NAMES count:', m.RULE_NAMES.length); console.log('rules keys count:', Object.keys(m.default.rules).length); console.log('first 3:', m.RULE_NAMES.slice(0, 3)); });"
      # Expect: 16 (or 19); 16 (or 19); ['msg-gr-1-line-grammar', 'msg-gr-2-marketplace-token', 'msg-gr-3-per-scope']
  ...
  <done>
    ...
    2. `RULE_NAMES` (frozen array) lists all 16+ meta-assertion rule names in a stable order.
    3. `default.rules` dict maps each rule name to its imported default export.
...
<verification>
- 16 (or 19) rule files exist; each has a no-op `Program: () => {}` visitor.
- 16 (or 19) RuleTester companion `.test.js` files exist; each has the 4-line node:test shim; each has a `valid:` smoke case and zero `invalid:` entries.
- `tests/lint-rules/index.js` `RULE_NAMES` and `rules` reflect the full 16 (or 19) meta-assertion set.
- `node --test tests/lint-rules/msg-*.test.js` passes for the new files.
- `npm run check` green.
</verification>

<success_criteria>
1. 16-19 of 34 MSG-* drift-guard rules ship as structural meta-assertions per D-14-09.
...
4. Plugin shell extends from empty to ~half-full -- Plan 05 completes the populate.
