...
1. **P108-18 resolver carrier:** land the schema/type/owner contract before later tests create more resolver fixtures. Trace callers with CodeGraph; do not silently edit later production owners. [VERIFIED: `.planning/phases/108-domain-and-platform/108-CONTEXT.md`:94-95; `.planning/REQUIREMENTS.md`:76-85]
2. **P108-21 credential carrier:** establish supplemental-evidence classification, relocate credential support, add the first registrar/negative-control pattern, and update all credential-fake imports. [VERIFIED: `.planning/phases/108-domain-and-platform/108-CONTEXT.md`:26-45,96-97]
3. **P108-12 device-flow carrier:** run after the credential support path is stable because the device-flow owner and integration tests use credential support. [VERIFIED: helper import inventory and CodeGraph call graph inspected 2026-08-28]
4. **P108-22 Git carrier:** run after credential relocation because current Git auth tests import the credential fake; relocate Git support and eliminate the loopback server. [VERIFIED: `tests/platform/git-remote-refs.test.ts`:13-19; helper import inventory inspected 2026-08-28]
...
**What goes wrong:** P108-18 edits many later orchestrator production files so each lexically checks `installable` first, violating one-source/one-owner delivery; or it edits none and claims the locked caller integration point without evidence. [VERIFIED: `.planning/REQUIREMENTS.md`:76-85; `.planning/phases/108-domain-and-platform/108-CONTEXT.md`:94-95]
...
**How to avoid:** P108-18 must include a complete caller trace, implement the boolean schema/constructors/narrowers, and use compile-time tests to prove `pluginRoot` is impossible on the false arm. Treat lexical caller rewrites as a planner policy decision: either explicitly classify them as carrier support or schedule them with their later owning pairs; never silently spread them across the P108-18 commit. [VERIFIED: `.planning/REQUIREMENTS.md`:72-85]

**Warning signs:** A plan lists production orchestrator files under P108-18 without explaining DEL-01/DEL-03, or reports success using only `npm run typecheck` while no caller inventory is recorded.
...
| Uppercase/implicit/mixed case comments in touched tests | Exact lowercase `// arrange`, `// act`, `// assert`, or single-expression `// act & assert` | v1.19 locked decision | Every touched runtime case is structurally reviewable; type-only cases remain compile-only. [VERIFIED: `.planning/phases/108-domain-and-platform/108-CONTEXT.md`:18-22] |
