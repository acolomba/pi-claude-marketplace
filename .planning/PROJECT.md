...
*Last updated: 2026-05-25 -- Phase 14.2 complete (Address tech debt: CR-01 + retroactive Phase 12 / 14.1 gates). All 5 plans landed across 4 waves; 1249/1249 tests green; lint+format+types clean. CR-01 closed: zero `function scopeOrder` declarations remain in `extensions/pi-claude-marketplace/orchestrators/`; `compareByNameThenScope` is now the single project-first comparator across `reinstall.ts`, `update.ts`, and `import/execute.ts`; `autoupdate.ts` iteration flipped to `["project", "user"]`; cross-scope `same-mp-both-scopes` byte-equal fixtures in `tests/architecture/catalog-uat.test.ts` lock the ordering at the catalog-UAT layer; CMC-03/25/26/27 traced and remain Complete. MSG-GR-3 lint rule promoted from no-op meta-assertion (D-14-09) to active two-axis AST detector (D-14-2-08) at `tests/lint-rules/msg-gr-3-per-scope.js` with `FunctionDeclaration` + `ArrayExpression` visitors scoped to `orchestrators/**/*.ts`; activation surfaced 7 additional `["user", "project"]` literals across 6 orchestrator files that were also flipped. Retroactive gates landed for both pending phases: `12-SECURITY.md` NEW (status: passed; 14/14 plan-time threats closed), `12-VALIDATION.md` advanced draft → validated (nyquist true; 8/8 rows covered), `14.1-SECURITY.md` NEW (passed; 14/14 closed), `14.1-VALIDATION.md` NEW (validated; nyquist true; 6/6 CMC-13 dimensions covered). Cosmetic ROADMAP drift closed: 29 Phase-13 CMC checkboxes in `## Coverage (v1.3)` flipped Pending → Complete; Phase 14.1 Phase Details row updated to `2/2 plans | Complete`; investigation note recorded that `/gsd:execute-phase` should auto-flip Coverage checkboxes (GSD-plugin ticket, out of scope here). Code review (14.2-REVIEW.md) flagged 1 Critical CR-01 follow-up (`edge/handlers/plugin/import.ts:45` still hardcodes `["user", "project"]` outside the lint rule's `orchestrators/**` glob -- orphan-diagnostic path not exercised by catalog UAT), 7 Warnings, 3 Info. Verification: passed (7/7 must-haves). Phase 14.2 is the final phase of milestone v1.3 -- STATE.md advances to `milestone_complete`; ready for `/gsd:complete-milestone v1.3`.*
