...
**Driver:** An installed plugin should stay visible, inspectable and uninstallable after its entry disappears from a marketplace manifest the extension read successfully. It should also be recognized as disabled when the user disables it, which a partially-installed plugin had silently stopped being. Both are read-surface truths about a record the extension already holds — not new persisted state.

**Key accomplishments:**

- **Manifest-independent inventory (Phase 95):** `list` states `{not in manifest}` on installed and degraded rows whose marketplace no longer declares them, and never states it about a manifest it failed to read. The claim authority is a `ManifestLookup` discriminated value that keeps "read failed" distinguishable from "read fine, entry missing", and the LLM tool surface forwards reasons on all four installed-family arms (INV-01..05, BOUND-03).
- **Record-backed info (Phase 96):** `info` falls through to the installation record when a valid manifest lacks the entry, reconstructing the component inventory locally and reading hooks from a new additive `hookEntries` record key written by install / update / reinstall — the materialized-file read surviving as the legacy fallback. When it cannot read them the `hooks:` line disappears in favor of a closed-set reason, so silence never passes for a verified absence. D-96-02 settles that a cross-scope folded row reads its own record's manifest (INFO-09..12, BOUND-01/02).
- **Disabled-state repair (Phase 97):** four independently-drifting copies of `installable && !enabled` collapsed into one `persistence/state-io.ts` predicate reading only `enabled`, under a whole-tree drift gate that also catches the destructured, bracket-access and `Boolean()`-coercion spellings. The conjunct had become load-bearing in the wrong direction — once a partial install always persisted `installable: false`, the guard excluded exactly one reachable shape and reported no error, so five surfaces took a silently wrong branch. `enable` now re-materializes a partial through the partially-available ledger arm, backfill is a fixed point for disabled records, and the update short-circuit derives its persisted availability discriminant (ENBL-05..09).
- **Lifecycle and contract (Phase 98):** uninstall pinned across all five resource kinds after manifest removal, `(skipped) {not in manifest}` pinned on all three update enumeration targets plus both halves of the autoupdate cascade, and one architecture test now holds every structural clause of the no-expansion promise — four closed sets by enumeration equality, seven glyph code points with an eighth-glyph tripwire, the record's key set, the schema-version union, and the network clause. Catalog, PRD and design doc reconciled against what shipped (LIFE-04..06, COMPAT-01, DOC-08).
- **Post-audit debt closure (Phase 99):** the question "does this manifest declare this plugin" is now asked in one place and answered identically on every rendering surface, with a source walk standing between the tree and a fourth copy. A disabled record repairs its source and availability discriminant when they move under an unchanged version. The coverage residual was re-measured against the tree the milestone actually left rather than the year-old table, and the seven arms the measurement named as reachable now assert what the failure left behind, not that the line ran.
- **Disabled-plugin information retention (Phase 100):** added 2026-08-11 by operator decision rather than opening v1.19. Disable keeps the record's inventory and drops only artifacts, so a disabled plugin keeps describing itself on both surfaces. That retired ENBL-04's empty-resources marker as a disabled signal, which forced the two guards that had read emptiness as intent — hook-routing suppression and the enable-path self-conflict exclusion — onto the explicit `enabled` boolean in the same change. Closed on live human UAT against Pi 0.84.1, 3/3, zero issues (ENBL-18).

**Closeout:** verified_closeout — all six phases complete with `verification_status: passed`; milestone audit passed 32/32 requirements, 6/6 phases, 5/5 integration seams, 3/3 flows, `threats_open: 0`. The eight items the pre-close artifact audit flagged were **resolved rather than acknowledged**: four were scanner-unclassifiable bookkeeping (the debug knowledge base counted as an open session; a UAT reading `passed` instead of the terminal `complete`; two quick-task summaries missing `status:`), two were promoted into `BACKLOG.md` (SEED-001 → the RSTA/FTCH remote-status + fetch-verb set; the coverage todo → COV-01), and the Phase 95 deferred pair was genuinely fixed — upgrading the global `pi-subagents` peer 0.24.3 → 0.47.1 took `npm run test:integration` from 16/18 to 18/18 with no source change, confirming the environment diagnosis. No deferred items carried.
