...
- [ ] **STOP-03**: A hook returning `{"decision": "block", "reason": "..."}` re-enters the agent loop via `pi.sendMessage(..., { deliverAs: "followUp", triggerTurn: true })` with the reason as model-visible, display-suppressed content.
...
- [ ] **DOC-04**: `docs/hooks-compatibility.md` reconciled with shipped behavior — the `Stop`/`StopFailure` event rows flip to supported with the turn-boundary timing-shift caveat documented, the matcher table gains the StopFailure error-type row, and the stale v1.13 hard-trip "Install-time disposition" section is rewritten for the force-install partial-partition model (`(partially-available)` + per-entry drops).
