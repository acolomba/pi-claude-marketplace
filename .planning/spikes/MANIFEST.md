...
| 021  | workflow-demand-signal              | standard   | Given real Claude Code plugins and marketplaces in the wild, when scanned for a `workflows/` directory or a `workflows` manifest field, then determine whether anyone actually ships the component kind yet | ✓ VALIDATED       | claude-code, workflows, demand-signal, research |
| 022a | script-trust-boundary-quintinshaw   | comparison | Given a Claude-shaped workflow script, when executed by @quintinshaw/pi-dynamic-workflows, then measure the privilege gap vs Claude's sandbox and the DSL semantic drift | ✓ WINNER          | pi-extension, workflows, trust-boundary, dsl, comparison |
| 022b | script-trust-boundary-nicknisi      | comparison | Given the same script, when executed by @nicknisi/pi-workflows, then measure the same privilege gap and DSL drift head-to-head against 009a | ⚠ PARTIAL (loses) | pi-extension, workflows, trust-boundary, dsl, comparison |
| 023  | landing-zone-quintinshaw            | standard   | Given @quintinshaw/pi-dynamic-workflows as the target settled on trust grounds (009a), when its saved-workflow format, path model, and name rules are exercised with a hand-planted bridged artifact, then determine what a bridge must write, where, and what our NFR-10 containment boundary must become | ⚠ PARTIAL         | pi-extension, workflows, landing-zone, scope-model, nfr-10 |
| 024  | workflows-bridge-shape              | standard   | Given our stage/commit/unstage + locationsFor + soft-dep machinery, when a workflows bridge is mapped onto it, then produce the reuse-vs-delta table including the NFR-10 containment amendment, namespacing, and collision handling | ⚠ PARTIAL         | architecture, bridges, nfr-10, design |
| 025  | canonical-path-mechanics            | standard   | Given the decision to write @quintinshaw's canonical paths for both scopes, when the project-key derivation is reimplemented and the staging/rename path is exercised, then confirm key parity across edge cases and establish where the staging tree must live for NFR-1 atomicity | ✓ VALIDATED       | pi-extension, workflows, paths, nfr-1, nfr-10, prototype |
| 026  | meta-name-extraction                | standard   | Given a Claude workflow script, when meta.name is extracted statically with acorn rather than by regex or filename, then confirm it survives comment/string decoys, reports its failure modes precisely, and matches upstream command naming on real plugin scripts | ✓ VALIDATED       | pi-extension, workflows, naming, parser, prototype |
| 027  | workflow-engine-3-10-1-recheck      | standard   | Given the workflows-milestone claims measured against @quintinshaw/pi-dynamic-workflows 3.5.1, when the same probes are re-driven against 3.10.1, then determine which claims survive the six-minor engine drift and which need regrading | ✓ VALIDATED       | pi-extension, workflows, engine-drift, re-verification |
