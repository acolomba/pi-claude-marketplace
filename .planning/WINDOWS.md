...
fixed_count: 19
total_count: 33
last_updated: 2026-09-08T07:41:37.490Z
---
...
|----|-------|------|------|------|-------------|--------|--------|-------------|-------------|
| 1 | 86 | unrun-verify | extensions/pi-claude-marketplace/bridges/skills/stage.ts |  | SKILL-01 backstop: after /reload a degraded skill's /skill:<name> resolves and the model never auto-invokes it (disable-model-invocation) — needs a live Pi session, not exercised in unit tests | waived | Accepted as unverifiable by automation, not undone work. The half this repository owns is pinned byte-for-byte: bridges/skills/frontmatter-degrade.ts synthesizes the block, and tests/bridges/skills/frontmatter-degrade.test.ts plus tests/bridges/skills/stage.test.ts assert the exact disable-model-invocation: true bytes. What stays unverified is Pi's own loader honouring that flag and resolving /skill:<name> after a reload - upstream behaviour no unit, integration or e2e test here can exercise. Closing it needs a live Pi session UAT, not a code change. | 2026-07-26T13:18:03.001Z | 2026-09-07T11:29:32.141Z |
| 2 | 88 | stub | extensions/pi-claude-marketplace/bridges/hooks/settle.ts |  | stop_hook_active hardcoded false in synthetic Stop event; loop-protection flag + 8-block cap land in plan 03 (STOP-07) | fixed |  | 2026-07-30T12:26:37.974Z | 2026-09-07T11:28:18.464Z |
| 3 | 88 | stub | extensions/pi-claude-marketplace/bridges/hooks/payloads/stop-failure.ts |  | thin StopFailure translator; errorMessage classifier lands in plan 04 (SFAIL-03) | fixed |  | 2026-07-30T12:26:38.396Z | 2026-09-07T11:28:18.827Z |
...
| 7 | 115 | deviation | tests/orchestrators/reconcile/backfill.test.ts |  | The two runScopeIsolated cases own no temporary tree; that entrypoint touches no filesystem, HOME or agent directory, so the plan's per-case tree requirement is satisfied vacuously. | waived | Accepted, nothing outstanding. Re-verified on the current tree: runScopeIsolated (reconcile/backfill.ts:143) touches no filesystem, HOME or agent directory, so the per-case temporary-tree requirement is satisfied vacuously rather than skipped. No fix exists to make. | 2026-09-02T01:30:10.510Z | 2026-09-07T11:29:32.509Z |
| 8 | 115 | deviation | tests/orchestrators/reconcile/pending.test.ts |  | Two force-preview guards in pending.ts (record === undefined, manifestEntry === undefined) are behaviorally redundant with the per-install catch in resolvePendingForceInstalls: removing either leaves the owner suite green because the resulting throw is caught per install and degrades the row to the same plain (will install). The two cases still make the guards reachable, so they are not dead code, but no public behavior discriminates them. | waived | Accepted coverage shortfall, nothing outstanding. Re-verified on the current tree: both guards are present and reachable in reconcile/pending.ts (record === undefined at 290, manifestEntry === undefined at 296), so they are not dead code - they are simply not discriminated by public behaviour, because resolvePendingForceInstalls catches per install and degrades to the same row either way. Removing them would be a production change with no behavioural gain. | 2026-09-02T01:56:56.047Z | 2026-09-07T11:29:32.865Z |
| 9 | 115 | deviation | extensions/pi-claude-marketplace/orchestrators/reconcile/apply.ts |  | Three per-entry catch clauses (marketplace add, plugin install, plugin toggle) were removed under the unreachable-code rule because addMarketplace, installPlugin and setPluginEnabled each answer with a typed outcome for every throw they can meet. RECON-03 per-entry isolation for those three now rests on an internal contract with no compile-time enforcement. WR-02 corrects the blast radius recorded here: an escape is NOT confined to one entry losing its failed row. applyPlan is called bare inside applyReconcile's per-scope loop, unlike its neighbours applyBackfillForScopeIsolated and rebuildScopeRoutingTableIsolated (both wrapped in runScopeIsolated) and unlike the read pass (its own try). An escape from any of the three uncaught loops therefore aborts the remaining entries in that bucket, skips backfill and the routing-table rebuild for that scope, skips the sibling scope entirely (project runs first, so a project-scope throw means user scope never reconciles), and DISCARDS EVERY OUTCOME ACCUMULATED SO FAR because notifyReconcileAppliedWithContext is never reached. The user gets a raw error out of resources_discover instead of a cascade. WR-02 also records that no test can ever prove the loops safe: installPlugin, addMarketplace and setPluginEnabled are static imports and ApplyReconcileOptions exposes no injection seam, so apply.test.ts physically cannot plant a throwing collaborator (contrast the two loops that kept their catch, which ARE covered through raceStateFromRead). Two margins the header comment does not mention: installPlugin has one awaited statement outside its try (collectPostCommitWarnings) plus buildInstalledOutcome, both dereferencing a definitely-assigned installCtx and safe only because runInstallLedgerBody returns a two-armed union 500 lines away; and handleInstallThrow composes a full failure message BEFORE its orchestrated branch, so a future throw inside composeInstallFailureMessage would escape from inside the entrypoint's own catch handler. Neither remedy is in bounds for the fix pass: restoring the catches creates arms no input can reach, which breaks the 100 percent direct-branch-coverage requirement for this pair, and adding the three orchestrators as injected collaborators is the test seam D-115-03 and CONVENTIONS.md forbid. DECIDED 2026-09-02: the operator accepted the recorded exposure. No code change - both structural remedies collide with phase 115's own bounds (100% branch coverage; no test-only DI seam on apply.ts), and the removed arms are unreachable today, so there is no live defect. This entry stays `open` deliberately, as the durable record of the residual risk rather than as a pending action. Resolution recorded in 115-VERIFICATION.md human_verification item 2. | waived | Exposure reviewed and ACCEPTED by the operator on 2026-09-02; encoded as a waiver so it stops recurring at every progress check, with the substance kept here rather than in an open action. What the exposure is: applyPlan runs three per-entry loops (marketplace add, plugin install, plugin toggle) with no per-entry catch, and it is called BARE inside applyReconcile per-scope loop -- unlike its neighbours applyBackfillForScopeIsolated and rebuildScopeRoutingTableIsolated (both wrapped in runScopeIsolated) and unlike the read pass (its own try). So an escape from any of the three uncaught loops is NOT confined to one entry losing its failed row: it aborts the remaining entries in that bucket, skips backfill and the routing-table rebuild for that scope, skips the sibling scope entirely (project runs first, so a project-scope throw means user scope never reconciles), and DISCARDS every outcome accumulated so far because notifyReconcileAppliedWithContext is never reached -- the user gets a raw error out of resources_discover instead of a cascade. Why it is not a live defect: the removed catch arms are unreachable today, because addMarketplace, installPlugin and setPluginEnabled each answer with a typed outcome for every throw they can meet. Why neither remedy was applied: restoring the catches creates arms no input can reach, breaking the 100 percent direct-branch-coverage requirement for the apply.ts / apply.test.ts pair; and adding the three orchestrators as injected collaborators is the test seam D-115-03 and CONVENTIONS.md forbid. Two margins worth remembering: installPlugin has one awaited statement outside its try (collectPostCommitWarnings) plus buildInstalledOutcome, both dereferencing a definitely-assigned installCtx and safe only because runInstallLedgerBody returns a two-armed union 500 lines away; and handleInstallThrow composes a full failure message BEFORE its orchestrated branch, so a future throw inside composeInstallFailureMessage would escape from inside the entrypoint own catch handler. The actionable residue -- ApplyReconcileOptions exposes no injection seam, so apply.test.ts physically cannot plant a throwing collaborator and the loops cannot be proven safe by test -- is filed as RCSEAM-01 in .planning/BACKLOG.md. Resolution recorded in 115-VERIFICATION.md human_verification item 2. | 2026-09-02T04:33:39.238Z | 2026-09-07T12:05:13.282Z |
| 10 | 115 | deviation | extensions/pi-claude-marketplace/orchestrators/reconcile/apply.ts |  | applyPlan's documented remove-before-add ordering is not discriminated by any input: swapping the two leaves the owner suite green, because the planner makes the removal and add buckets disjoint by name. The add-before-install and project-before-user orderings ARE discriminated and are pinned. | waived | Accepted, nothing outstanding. The planner makes the removal and add buckets disjoint by name, so no input can discriminate the documented remove-before-add ordering. The two orderings that ARE discriminated - add-before-install and project-before-user - are pinned. Pinning the third would require a production change that makes the buckets overlap. | 2026-09-02T04:33:39.577Z | 2026-09-07T11:29:33.209Z |
...
| 33 | 114 | deviation | README.md |  | [workflows-replay] both README taglines still list five component kinds and omit workflows; the Features and Prerequisites lists were updated but the tagline was out of the plan's scope | open |  | 2026-09-08T07:41:37.490Z |  |
...
[
  {
    ...
    "status": "waived",
    "reason": "Accepted as unverifiable by automation, not undone work. The half this repository owns is pinned byte-for-byte: bridges/skills/frontmatter-degrade.ts synthesizes the block, and tests/bridges/skills/frontmatter-degrade.test.ts plus tests/bridges/skills/stage.test.ts assert the exact disable-model-invocation: true bytes. What stays unverified is Pi's own loader honouring that flag and resolving /skill:<name> after a reload - upstream behaviour no unit, integration or e2e test here can exercise. Closing it needs a live Pi session UAT, not a code change.",
    "recorded_at": "2026-07-26T13:18:03.001Z",
    "resolved_at": "2026-09-07T11:29:32.141Z"
  },
  {
    "id": 2,
    "kind": "stub",
    ...
    "resolved_at": "2026-09-07T11:28:18.464Z"
  },
  {
    "id": 3,
    "kind": "stub",
    ...
    "resolved_at": "2026-09-07T11:28:18.827Z"
  },
  {
    "id": 4,
    "kind": "deviation",
  ...
  },
  {
    ...
    "status": "waived",
    "reason": "Accepted, nothing outstanding. Re-verified on the current tree: runScopeIsolated (reconcile/backfill.ts:143) touches no filesystem, HOME or agent directory, so the per-case temporary-tree requirement is satisfied vacuously rather than skipped. No fix exists to make.",
    "recorded_at": "2026-09-02T01:30:10.510Z",
    "resolved_at": "2026-09-07T11:29:32.509Z"
  },
  {
    "id": 8,
    "kind": "deviation",
    ...
    "status": "waived",
    "reason": "Accepted coverage shortfall, nothing outstanding. Re-verified on the current tree: both guards are present and reachable in reconcile/pending.ts (record === undefined at 290, manifestEntry === undefined at 296), so they are not dead code - they are simply not discriminated by public behaviour, because resolvePendingForceInstalls catches per install and degrades to the same row either way. Removing them would be a production change with no behavioural gain.",
    "recorded_at": "2026-09-02T01:56:56.047Z",
    "resolved_at": "2026-09-07T11:29:32.865Z"
  },
  {
    "id": 9,
    "kind": "deviation",
    ...
    "status": "waived",
    "reason": "Exposure reviewed and ACCEPTED by the operator on 2026-09-02; encoded as a waiver so it stops recurring at every progress check, with the substance kept here rather than in an open action. What the exposure is: applyPlan runs three per-entry loops (marketplace add, plugin install, plugin toggle) with no per-entry catch, and it is called BARE inside applyReconcile per-scope loop -- unlike its neighbours applyBackfillForScopeIsolated and rebuildScopeRoutingTableIsolated (both wrapped in runScopeIsolated) and unlike the read pass (its own try). So an escape from any of the three uncaught loops is NOT confined to one entry losing its failed row: it aborts the remaining entries in that bucket, skips backfill and the routing-table rebuild for that scope, skips the sibling scope entirely (project runs first, so a project-scope throw means user scope never reconciles), and DISCARDS every outcome accumulated so far because notifyReconcileAppliedWithContext is never reached -- the user gets a raw error out of resources_discover instead of a cascade. Why it is not a live defect: the removed catch arms are unreachable today, because addMarketplace, installPlugin and setPluginEnabled each answer with a typed outcome for every throw they can meet. Why neither remedy was applied: restoring the catches creates arms no input can reach, breaking the 100 percent direct-branch-coverage requirement for the apply.ts / apply.test.ts pair; and adding the three orchestrators as injected collaborators is the test seam D-115-03 and CONVENTIONS.md forbid. Two margins worth remembering: installPlugin has one awaited statement outside its try (collectPostCommitWarnings) plus buildInstalledOutcome, both dereferencing a definitely-assigned installCtx and safe only because runInstallLedgerBody returns a two-armed union 500 lines away; and handleInstallThrow composes a full failure message BEFORE its orchestrated branch, so a future throw inside composeInstallFailureMessage would escape from inside the entrypoint own catch handler. The actionable residue -- ApplyReconcileOptions exposes no injection seam, so apply.test.ts physically cannot plant a throwing collaborator and the loops cannot be proven safe by test -- is filed as RCSEAM-01 in .planning/BACKLOG.md. Resolution recorded in 115-VERIFICATION.md human_verification item 2.",
    "recorded_at": "2026-09-02T04:33:39.238Z",
    "resolved_at": "2026-09-07T12:05:13.282Z"
  },
  {
    "id": 10,
    "kind": "deviation",
    ...
    "status": "waived",
    "reason": "Accepted, nothing outstanding. The planner makes the removal and add buckets disjoint by name, so no input can discriminate the documented remove-before-add ordering. The two orderings that ARE discriminated - add-before-install and project-before-user - are pinned. Pinning the third would require a production change that makes the buckets overlap.",
    "recorded_at": "2026-09-02T04:33:39.577Z",
    "resolved_at": "2026-09-07T11:29:33.209Z"
  },
  {
    "id": 11,
    "kind": "deviation",
  ...
  },
  {
    ...
    "resolved_at": "2026-09-07T12:05:26.945Z"
  },
  {
    "id": 14,
    "kind": "deviation",
    ...
    "status": "waived",
    "reason": "Accepted, nothing outstanding. The overstated doc comment was already corrected to describe the four-function invariant instead of implying the type enforces it (still present at import/execute.ts:291-292), and WR-06 removed the one concrete near-miss by giving the unknown-stored branch a status. A structural headerless-row gate is deliberately not added: it would be an arm no input can reach, which breaks the complete direct-branch-coverage requirement for this pair.",
    "recorded_at": "2026-09-02T06:22:47.283Z",
    "resolved_at": "2026-09-07T11:29:33.562Z"
  },
  {
    "id": 15,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, pinned by identity. NOT compiler-forced but structural: the shared parseCommandArgs contract passes the usage string only for a required positional, and this schema declares its sole positional optional, so the collapse arm is dead here while staying live for sibling handlers that declare a required one. Closes only by a production rewrite of the shared parser contract; do not re-litigate.",
    "recorded_at": "2026-09-02T22:03:02.310Z",
    "resolved_at": "2026-09-07T11:29:15.079Z"
  },
  {
    "id": 16,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, pinned by identity. Compiler-forced: Array.prototype.at() is typed T or undefined by the TypeScript standard library, so the nullish fallback must exist even though the array is non-empty on every path that reaches it, and narrowing it needs a non-null or type assertion - both barred throughout extensions/. Unreachability proved by construction, by a brute force over all 65536 BMP code points in five shapes, and by a plant that stayed green. Closes only by a production rewrite; do not re-litigate.",
    "recorded_at": "2026-09-02T22:04:43.869Z",
    "resolved_at": "2026-09-07T11:29:14.062Z"
  },
  {
    "id": 17,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, pinned by identity. NOT compiler-forced but structural: the declared element type keeps description optional while both producers of the flagCompletions list always supply one - a written-out literal, and completionFlagEntries deriving from a FlagEntry whose description field is required - so the guard must exist and nothing reachable can trip it. Proved by a plant that stayed green and by 406 long-flag cursor prefixes emitting 169 items with zero missing a description. Closes only by a production rewrite of the declared element type; do not re-litigate.",
    "recorded_at": "2026-09-03T00:18:38.629Z",
    "resolved_at": "2026-09-07T11:29:15.457Z"
  },
  {
    "id": 18,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, pinned by identity. Compiler-forced by useUnknownInCatchVariables: the catch binding is typed unknown, so the String-conversion arm must exist even though parseArgs constructs a new Error at both of its throw sites, and narrowing it needs a type assertion, barred throughout extensions/. Proved by a plant that stayed green across all 8 cases, a live-arm plant that turned the tokenizer case red, and a brute force over 3615 argument strings yielding 521 throws and zero non-Error values. Closes only by a production rewrite routing the catch through shared/errors.ts errorMessage; do not re-litigate.",
    "recorded_at": "2026-09-03T00:44:13.846Z",
    "resolved_at": "2026-09-07T11:29:14.398Z"
  },
  {
    "id": 19,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, pinned by identity. Compiler-forced by noUncheckedIndexedAccess (tsconfig.json:12): the index read is typed possibly-undefined even though the guard on the line above has already proven the positional list non-empty; removing the fallback raises TS18048 at its consumption site and narrowing it needs a non-null or type assertion, both barred throughout extensions/. Proved by two plants that stayed green across all 15 cases and by a brute force over 19530 argument strings with a working positive control. Closes only by a production rewrite; do not re-litigate.",
    "recorded_at": "2026-09-03T04:09:41.716Z",
    "resolved_at": "2026-09-07T11:29:14.771Z"
  },
  {
    "id": 20,
    "kind": "deviation",
    ...
    "resolved_at": "2026-09-07T12:05:27.307Z"
  },
  {
    "id": 21,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, original claimant, pinned by identity. Compiler-forced by noUncheckedIndexedAccess (tsconfig.json:12): every index read is typed T or undefined, so a loop whose bounds already guarantee the read must still carry a guard the loop can never enter. Removing it needs a non-null assertion, which is an error throughout extensions/ under strictTypeChecked. Not undone work and not re-litigable without a production rewrite.",
    "recorded_at": "2026-09-03T11:00:38.155Z",
    "resolved_at": "2026-09-07T11:28:57.174Z"
  },
  {
    "id": 22,
    "kind": "unmet-truth",
    ...
    "status": "waived",
    "reason": "Accepted permanent shortfall under D-116-01a, original claimant, pinned by identity. Same class as the args.ts claimant: noUncheckedIndexedAccess (tsconfig.json:12) forces the cross-cutting flag scanner to carry an index-read guard that its own bounds make unreachable, and removing it needs a non-null assertion, barred throughout extensions/ under strictTypeChecked. Not undone work and not re-litigable without a production rewrite.",
    "recorded_at": "2026-09-03T11:00:38.508Z",
    "resolved_at": "2026-09-07T11:29:13.703Z"
  },
  {
    "id": 23,
    "kind": "deviation",
    ...
    "resolved_at": "2026-09-07T11:28:19.171Z"
  },
  {
    "id": 24,
    "kind": "stub",
    ...
    "resolved_at": "2026-09-07T11:28:19.544Z"
  },
  {
    "id": 25,
    "kind": "deviation",
    ...
    "resolved_at": "2026-09-07T11:28:19.939Z"
  },
  {
    "id": 26,
    "kind": "deviation",
    ...
    "resolved_at": "2026-09-07T11:28:20.308Z"
  },
  {
    "id": 27,
    "kind": "unrun-verify",
  ...
  },
  {
    ...
    "resolved_at": "2026-09-07T11:28:20.636Z"
  },
  {
    "id": 30,
    "kind": "unrun-verify",
