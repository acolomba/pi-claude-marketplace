...
- `fallow` `^3.16.0` - whole-graph static analysis (`.fallowrc.json`). `npm run fallow` is three explicit invocations chained with `&&` -- `fallow dead-code --fail-on-issues`, `fallow health --fail-on-issues`, `fallow dupes --fail-on-issues` -- run over the WHOLE repository with no delta scoping. That identical command runs in two places: the `.pre-commit-config.yaml` `npm-fallow` hook (`always_run: true`, since health and duplication cover `tests/` too), and `npm run check` -- which is both the local gate and the CI gate, since `.github/workflows/ci.yml` runs `npm run check` on pull requests, on `main` and `features/**` pushes, and on the `v*` release path via `workflow_call`. There is deliberately no separate `fallow` job in `.github/workflows/lint.yml`: it would re-run the same whole-repo analysis a second time on every code pull request, and its only exclusive coverage would be docs-only pull requests that `ci.yml` skips via `paths-ignore` -- where fallow, which reads `.ts`, cannot produce a different answer. The blocking gate is whole-repo everywhere, so a green local run and a green pull request mean the same thing
  - A SECOND, additional job (`fallow-audit`) runs `fallow audit` on pull requests only. This is not a return of the old asymmetry: audit was previously the ONLY CI gate and was delta-scoped, so a dirty repository passed CI while local disagreed. The whole-repo `npm run fallow` is now the authority in all three surfaces and blocks; audit adds a different job on top -- new-vs-inherited attribution scoped to the diff, so a finding the pull request INTRODUCED is separated from one it inherited. It pins `--base` to the PR base branch, needs `fetch-depth: 0` for the merge base (the whole-repo job does not and stays shallow), and reports through `--format github-annotations`, which renders on fork pull requests without a write token
  - Audit's default `new-only` gate means introduced dead-code or complexity yields `fail` and exits 1, while introduced duplication yields `warn` and exits 0. Duplication therefore stays gated by the repo-wide `duplicates.threshold` in the whole-repo job, not by audit
