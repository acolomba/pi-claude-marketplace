...
**Barrel Files:** Barrels exist per bridge kind (`bridges/<kind>/index.ts`) and under `orchestrators/{import,marketplace,plugin}/`. There is no aggregate `bridges/index.ts`. The layer-level barrels (`domain/`, `edge/`, `orchestrators/`, `persistence/`, `transaction/`) were removed as unreachable from the extension entry point. Barrels are not universally used across every directory (check per-directory before assuming one exists).
