...
- **Sonar way on `extensions/` only.** A dedicated block spreads `sonarjs.configs.recommended.rules` over `extensions/pi-claude-marketplace/**/*.ts`: 217 rules at `error`, the other 62 off, verified by `eslint --print-config` (0 land at `warn`). 200 of the 217 are active in the TypeScript **Sonar way** profile SonarCloud runs against this project, so most Sonar findings surface locally instead of on the pull request. The scope mirrors `sonar.sources`; `sonar.test.exclusions` drops `tests/**`, which also carries the `void (x satisfies T)` assertion idiom that trips `sonarjs/void-use` 797 times. Two mechanics are load-bearing: the rules are **spread**, never extended as a config (`recommended` re-declares the `sonarjs` plugin, which ESLint 10 rejects, and it has no `files` key), and `cognitive-complexity` is **re-asserted** after the spread because `recommended` sets a bare `"error"` that would drop the threshold. All three ceilings agree at 15 — this rule, fallow's `health.maxCognitive`, and Sonar way's own `S3776` `threshold` parameter.
