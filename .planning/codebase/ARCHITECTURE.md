...
**Key Characteristics:**
- The install flow is a named 6-phase ledger (`transaction/phase-ledger.ts`'s `runPhases<C>`) with symmetric `do`/`undo` per phase (skills, commands, agents, hooks, mcp, state), guaranteeing all-or-nothing materialization. `runPhases` has TWO production consumers, at two tiers. `orchestrators/plugin/install-outcome.ts` owns the inner, per-plugin 6-phase ledger and invokes it through the injected `InstallLedgerTransaction.runPhases` seam (`orchestrators/plugin/install-flow.ts` binds the real implementation via `REAL_INSTALL_TRANSACTION`). `orchestrators/plugin/install-cascade.ts` owns an OUTER, per-plugin-in-the-dependency-closure ledger (RESV-01 / D-03-07): one phase per closure member, `do` = `runInstallLedger`, `undo` = `cascadeUnstagePlugin` plus dropping the member's in-memory record, so a failed member unwinds the whole cascade. Its array is DERIVED rather than literal; the module header carries the counter-rationale (the order is `domain/dependency-closure.ts`'s post-order, an explicit and separately tested contract, not the implicit ordering the literal-array discipline guards against). Its `undo` additionally gates on a context-set `materialized` sentinel, because TR-02 runs the failing phase's own undo and a member that threw may be one that was already installed. `orchestrators/plugin/enable-disable.ts` reaches the same materialization logic indirectly, by calling the guard-free `runInstallLedger` exported from `install-outcome.ts` (its enable branch, `runEnableBranch`), not `runPhases` directly. The update family (`orchestrators/plugin/update-flow.ts` + `update-swap.ts`) deliberately does NOT use `runPhases` — their shared header documents a hand-rolled heterogeneous-undo flow instead. `uninstall.ts` and the reinstall family (`reinstall-flow.ts` + its `reinstall-*` leaves) carry no ledger at all.
...
- **Circular imports:** Cycle detection is gated by **two** `fallow dead-code` runs inside `npm run fallow` (`.fallowrc.json` entry: `extensions/pi-claude-marketplace/index.ts`). `.fallowrc.json` sets `production: { deadCode: true, health: false, dupes: false }`, and production mode excludes the test and script trees, so the bare run covers only the production entry graph; the second run, `fallow dead-code --no-production --circular-deps --re-export-cycles --fail-on-issues`, carries the two cycle classes across `tests/` and `scripts/`. It stays filtered to those classes because a bare `--no-production` run also re-reads the production-mode suppressions as stale. `tests/architecture/import-boundaries.test.ts` ("D-11: npm run fallow gates cycles over the whole repository") asserts BOTH invocations are present, keeps the bare one unfiltered through an allow-list of its tokens (fallow's per-issue flags are only-report filters — naming one narrows the run and silently stops gating cycles), and pins the cycle run's required flags. The scope split itself is measured by a planted offender in `tests/architecture/fallow-production-mode.test.ts` ("D-05: the whole-tree cycle run reports a cycle the production-scoped run cannot see"): a two-file cycle outside the entry graph is invisible to the production-scoped run and exits 1 on the cycle run. The ESLint `import-x/no-cycle` rule that used to cover this was REMOVED after being measured to report nothing on a deliberate two-file cycle while `fallow dead-code` correctly exited 1 on the same case.
