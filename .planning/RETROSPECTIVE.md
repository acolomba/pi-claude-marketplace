...
**Shipped:** 2026-07-13
**Phases:** 4 (Phases 76-79) | **Plans:** 20 | **Tasks:** 49 | **Requirements:** 20/20 (MURL-02 dropped by design) | **Tests:** 2681 → 2739 unit + 16 integration

### What Was Built
- Arbitrary public HTTPS git URLs as first-class **marketplace** sources: `add` clones `source.url` verbatim (no github.com reconstruction), `update`/`remove`/`info` at full parity, config reconcile + Claude-settings import both map URL sources, and 401/403 clone challenges render the new `{authentication required}` closed-set reason.
- A **source-addressed refcounted plugin clone cache** (`plugin-clones/<12hex(sha256(url))>-<sha12>/`): one external-monorepo clone serves every referencing plugin (~180 `git-subdir` plugins in the official directory), warm-cache installs complete offline, recorded versions carry the resolved commit (`sha-<12hex>` rendering `#<7hex>`).
- The full **git-source plugin lifecycle**: sha-change atomic swaps on update, last-reference clone GC on uninstall AND update AND marketplace-remove (derive-not-persist live keys), offline reinstall from the recorded sha, and network-free list/info with install-completion parity via the shared `git-source-probe.ts` module.
- A **`GitAuthProvider` registry** generalizing the GitHub-only RFC-8628 Device Flow: public repos on any host clone unauthenticated (no provider involved), registered hosts run their flow host-keyed via `CredentialOps` with a once-per-host memo, no-provider hosts fail structurally clean (no `onAuth` callback registered — no isomorphic-git retry loop), and the no-credential-leak gate covers every provider file.

### What Worked
- The natural difficulty gradient held: marketplace-side widening (76, existing clone lifecycle) → genuinely-new plugin cache subsystem (77) → lifecycle generalization (78) → auth registry (79). Each phase froze a seam the next consumed.
- Upstream-absence-as-decisive: MURL-02 (marketplace-level `git-subdir`) was dropped in phase discussion after verifying upstream has no subdirectory-marketplace concept — scope shrank honestly instead of building an orphan feature.
- The byte-identity trio (three frozen auth test files, verified unmodified via git log) made "byte-identical github.com behavior" a checkable claim rather than an assertion.
- Round-2 UAT surfaced two real cross-surface gaps (install-completion parity, marketplace-remove clone GC) that notify-boundary fixtures could not see; both closed same-day with regression tests reproducing the exact defect shapes.

### What Was Inefficient
- Phase 78 took three verification rounds (initial `human_needed` → gap-closure re-verify → round-2 UAT gap closure). The two round-2 gaps were both same-class ("surface X doesn't know about git sources yet") — a completeness critic pass over every consumer of plugin-source classification after Phase 77 might have caught them pre-UAT.
- The classification logic initially lived inline in `list.ts` and had to be extracted to `git-source-probe.ts` when completion needed it — landing shared classification as a module from the start (the `narrowUnsupportedKinds` lesson from force-install) would have skipped the 78-09 extraction.

### Patterns Established
- **Presence probe vs materializing probe:** the same `ResolveContext.resolveGitPluginRoot` seam takes an fs-only presence probe on read surfaces (list/info/completion, network-free) and a clone-materializing probe on install — one resolver, two capability levels.
- **Derive-not-persist extended to GC:** live clone keys derive from surviving state records at collection time; no refcount field, no fetch registry, idempotent and self-healing.
- **Shared classification module + output-parity drift-guard:** when two surfaces must agree (list rows vs completion buckets), extract one module and lock the agreement with a two-independent-call-path byte comparison test.

### Key Lessons
1. **Consumers of a widened classification need enumerating, not discovering.** Both round-2 UAT gaps were surfaces (completion bucketizer, marketplace-remove cascade) that still held the pre-widening assumption. When a core classification widens, grep for every consumer of the old predicate before declaring the phase done.
2. **Upstream research before design debate is cheap and decisive.** The MURL-02 drop took one docs check; the alternative was building a marketplace-source concept with no upstream analog to stay aligned with.
3. **Cache-key structure is API.** Making the key `<urlhash12>-<sha12>` (url-hash prefix + sha suffix) is already paying forward: the next milestone's unpinned-source presence probe can prefix-scan on the url-hash half without persisting any fetch state.

### Cost Observations
- 133 commits over ~3 days (2026-07-11 → 2026-07-13), +20,370 / −613 LOC across 141 files.
- Two live-network UAT sessions (real public repos: awslabs/agent-plugins dedup trio, obra/superpowers sha-bump update) closed what mocked GitOps coverage structurally could not.
