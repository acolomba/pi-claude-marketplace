...
Use installed TypeScript 6.0.3 and Node's native test runner. Existing script tests live under `tests/scripts`; the normal unit command already discovers them. Reuse the documented .mjs import boundary pattern in `tests/scripts/check-unused-type-members.model.test.ts` when TypeScript declarations are needed, without loosening compiler/lint configuration. (This plan originally cited `tests/scripts/check-phase-06-hub-ledger.test.ts`, which belonged to a prior milestone's phase 06 and was retired as an unused file in phase 5; plan 06-01 re-established the pattern -- a runtime specifier plus an explicit test-facing interface, since the compiler cannot check a `.mjs` import.) No package installation is planned.
