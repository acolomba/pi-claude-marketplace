...
**Fixed at:** 2026-08-08T20:43:00Z
**Source review:** `.planning/phases/95-manifest-independent-installed-inventory/95-REVIEW.md`
(iteration 3)
**Iteration:** 3 (final -- cumulative; iterations 1 and 2 are summarized below)

**Summary (iteration 3):**

- Findings in scope (`critical_warning`): 1 (WR-08)
- Fixed: 1
- Skipped: 0

Three Info findings (IN-10, IN-11, IN-12) were left alone with a recorded
reason -- see Skipped Issues.

**Cumulative state across all three iterations:** every Critical and Warning
finding raised against this phase is fixed. Two items remain deliberately
open, both recorded and routed: WR-02 (Phase 98 / DOC-08 by `95-CONTEXT.md`
assignment) and WR-05(b) (Phase 96 / BOUND-01 / BOUND-02).

## Fixed Issues

### WR-08: the catalog paragraph documented the claim-authority rule the fix pass reverted

**Files modified:** `docs/output-catalog.md`
**Commit:** `b89aef7a`
**Applied fix:** The `manifest-absent-inventory` prose was written under WR-03
(`43d62143`) against the iteration-1 `ownsAbsenceClaim` behavior, then
invalidated by `06875fa4` in the same phase. Two clauses had become false: that
the claim is made only about a manifest **the block header names**, and that a
cross-scope folded row whose owning record names a different manifest **drops
the brace**. `list-manifest-absent.test.ts:642` asserts the opposite for exactly
that configuration, and no header-naming condition survives anywhere in
`list.ts`.

The sentence now states the shipped rule: the claim derives from the manifest
the plugin's OWN marketplace record names (INV-01), which on the cross-scope
orphan fold is the project-side record's manifest even though the row renders
under the user-scope header. BOUND-03 / D-95-05 are re-scoped to the arm they
actually cover -- the load failure -- and the previously undocumented folded
form of that failure is now written down: the row survives as a bare
`(installed)` with no brace (`list-manifest-absent.test.ts:568`). A closing
clause marks the residual "which manifest SHOULD a folded row describe"
question as open under BOUND-01 / BOUND-02, so the Phase 96 work is not
misdirected by reading this paragraph as a settled contract.

Prose only. The fenced example block, the `<!-- catalog-state: -->`
discriminator, the heading and the surrounding states are byte-unchanged, which
is what the catalog gate checks -- `git show --stat` reports one insertion and
one deletion on the single paragraph line.
...
**Reason:** Correct and inherited, not introduced -- the pre-rewrite shape only
ever tested `loadError !== undefined`, so the payload has never been read. Both
proposed remedies (drop the field, or route it to `debug-log.ts`) are
behavior-visible choices about diagnostics rather than defects: dropping it
forecloses the cause trailer a later boundary decision may want, and logging it
adds an output path the catalog does not describe. Below the fix threshold at
the final auto-loop iteration with working code and green tests.

### IN-11: `seedMarketplace`'s `scopeRoot` parameter is redundant with its hardcoded scope

**File:** `tests/orchestrators/plugin/list-manifest-absent.test.ts:100-110`, `:145-146`
**Reason:** Test-helper ergonomics, no production surface. The hazard described
is hypothetical (a future caller passing a divergent root); all 12 current call
sites pass the value the helper would compute. Cosmetic at this point in the
loop.

### IN-12: `loadManifestSoftly` is a pass-through whose doc comment overstates it

**File:** `extensions/pi-claude-marketplace/orchestrators/plugin/list.ts:262-269`
**Reason:** Pre-existing wrapper, untouched by this phase's commits. Inlining it
is a pure refactor across the enumerator's call sites with no behavior change
and no requirement attached; deferring it keeps the final iteration's diff to
the one doc/code contradiction this phase actually introduced.

## Requirements Held Green

Unchanged from iteration 2 -- this iteration touched documentation prose only:
...
Iteration 3: file re-read after the edit;
`pre-commit run --files docs/output-catalog.md` clean (mdformat and
markdownlint-cli2 both Passed, so the paragraph needed no reflow -- mdformat's
default wrap mode preserves the catalog's one-line-per-paragraph style);
`node --test tests/architecture/catalog-uat.test.ts` 6/6 pass -- the byte-equality
gate over every fenced catalog block, which is the suite that would catch an
accidental example edit. The three other catalog-derived architecture gates
(`partial-vocabulary-guard`, `notify-closed-set-locks`, `hooks-cap-notify`) also
run green, 59/59 combined. No `.ts` file changed, so typecheck / lint / format
carry over from iteration 2's full `npm run check` (exit 0; 3269 unit tests,
18 integration).

**Where verification ran:** in the working tree
`/home/acolomba/pi-claude-marketplace/.worktrees/manifest-independent-plugin-info`
(branch `features/manifest-independent-plugin-info`), the same tree the commit
...
(git-mode scan cannot read `.git/index`), so the commit was preceded by the
documented filesystem scan over the changed path
(`--results=verified,unknown --fail`, `verified_secrets: 0` /
`unverified_secrets: 0`) and committed with `SKIP=trufflehog`. No other hook was
skipped.

## Iteration 2 (superseded where noted, retained for history)

**Summary:** findings in scope 3 (WR-05, WR-06, WR-07); fixed 3; skipped 0. Two
Info findings were fixed opportunistically (IN-06, IN-08); two were left with a
recorded reason (IN-07, IN-09).
...
iteration 3's IN-11 revisits that call and defers it.

### Iteration-2 skipped items

- **IN-07** (`docs/output-catalog.md:412`, the "materialized on disk" wording):
  skipped by scope instruction, left for the phase's documentation pass so the
  catalog is edited once. Still open after iteration 3 -- the WR-08 edit
  replaced the following sentence and deliberately did not touch this one.
- **IN-09** (eight positional parameters on `installedRowMessage`): partially
  addressed by WR-07 (eight to seven) and otherwise skipped; the named-args
  conversion is a pure refactor with no requirement attached.
- **WR-05(b)**: deferred to Phase 96 (BOUND-01 / BOUND-02), rationale above.
...
WR-05 entry above. Iteration 3 in turn corrects the WR-03 catalog paragraph,
which described the reverted iteration-1 behavior. The CR-01 and WR-04 fixes are
untouched and were confirmed still green.

---

_Fixed: 2026-08-08T20:43:00Z_
