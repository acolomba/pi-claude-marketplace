...
| 03-04-01 | 03-04 | 1 | PDEF-08 | T-03-13 | `manual` maps to `manual`; threshold and overflow reasons map to `auto` in both compact payload owners | unit | `node --test tests/bridges/hooks/payloads/pre-compact.test.ts tests/bridges/hooks/payloads/post-compact.test.ts tests/domain/components/hook-events.test.ts` | ✅ | ⬜ pending |
| 03-05-01 | 03-05 | 1 | PDEF-07 | T-03-17 | Prototype keys are rejected and the trace-proven synchronous no-producer branch is removed while true guards remain | unit + structural | `node --test tests/domain/components/hook-tool-names.test.ts tests/domain/components/hooks/matcher.test.ts tests/bridges/hooks/dispatch-exec.test.ts` | ✅ | ⬜ pending |
...
| 03-07-03 | 03-07 | 3 | PDEF-06 | T-03-26 | Update retains the primary error, appends exact cleanup context, and exposes the expected residual artifact | unit + filesystem | `node --test tests/shared/errors.test.ts tests/orchestrators/plugin/update.test.ts` | ✅ | ⬜ pending |
...
| 03-10-01 | 03-10 | 3 | PDEF-01 | T-03-35 | Remaining plugin-surface producers declare structural cardinality from invocation shape | unit | `node --test tests/orchestrators/plugin/info.test.ts tests/orchestrators/plugin/list.test.ts tests/orchestrators/plugin/enable-disable.test.ts tests/orchestrators/plugin/shared.test.ts` | ✅ | ⬜ pending |
| 03-12-01 | 03-12 | 2 | PDEF-07 | T-03-41 | Async dispatch removes only the producer-proven empty complement while retaining registry and path guards | unit + structural | `node --test tests/bridges/hooks/async-rewake/registry.test.ts tests/domain/components/hook-events.test.ts tests/bridges/hooks/dispatch-exec.test.ts` | ✅ | ⬜ pending |
| 03-13-01 | 03-13 | 1 | PDEF-01 | T-03-44 | Marketplace add, remove, and update derive cardinality from named or plural invocation shape | unit + output | `node --test tests/orchestrators/marketplace/add.test.ts tests/orchestrators/marketplace/remove.test.ts tests/orchestrators/marketplace/update.test.ts` | ✅ | ⬜ pending |
| 03-14-02 | 03-14 | 2 | PDEF-01 / PDEF-08 | T-03-47 | Import and reconcile declare plural without mutating canonical alias state or triggering network work | unit + integration | `node --test tests/orchestrators/import/execute.test.ts tests/orchestrators/reconcile/pending.test.ts tests/integration/reconcile-plan-convergence.test.ts` | ✅ | ⬜ pending |
