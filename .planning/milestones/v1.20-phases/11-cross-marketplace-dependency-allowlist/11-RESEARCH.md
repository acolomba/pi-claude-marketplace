...
1. **RESOLVED — info values:** Plan 11-02 uses a JSON-array-style representation in source order after `description:`. It escapes JSON controls, Unicode line separators, and bidi controls. D-11-04 delegates this presentation choice; it is not an upstream requirement. [VERIFIED: .planning/phases/11-cross-marketplace-dependency-allowlist/11-CONTEXT.md:31-55] [VERIFIED: .planning/phases/11-cross-marketplace-dependency-allowlist/11-02-PLAN.md]
2. **RESOLVED — absent root marketplace:** Plan 11-04 preserves the existing `"marketplace-absent"` cascade outcome. The policy helper returns an empty set only when the root source record is truly absent; a present malformed manifest still throws the typed validation error. [VERIFIED: extensions/pi-claude-marketplace/orchestrators/plugin/install-cascade.ts:483-511] [VERIFIED: extensions/pi-claude-marketplace/domain/manifest.ts:123-133] [VERIFIED: .planning/phases/11-cross-marketplace-dependency-allowlist/11-04-PLAN.md]
3. **RESOLVED — reload policy root:** Plan 11-06 authorizes the original A→B edge when **any** eligible original declarer's marketplace permits B, before installing B. B then becomes root of its own cascade, and B's policy governs B→C. This follows Claude Code 2.1.267 and applies D-11-01 to each actual cascade. [VERIFIED: installed Claude Code 2.1.267 binary `/home/acolomba/.local/share/claude/versions/2.1.267`, JS payload near byte offset 191433570, inspected 2026-09-23] [VERIFIED: .planning/phases/09-reload-installs-missing-dependencies/09-CONTEXT.md:442-452] [VERIFIED: .planning/phases/11-cross-marketplace-dependency-allowlist/11-06-PLAN.md]
