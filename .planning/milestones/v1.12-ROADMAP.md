...
**Goal:** A Pi user can disable a plugin to keep its config entry and version pin while removing its Pi artifacts, and re-enable it from cache with no network -- with disabled status rendered as a distinct, deliberate fact separate from soft-degraded unavailability.
