...
- [x] **SPLIT-01**: Config file owns desired state + user settings (marketplace source/autoupdate, plugin enabled/version pin); machine bookkeeping (materialized artifact records, resolved versions) lives in an internal file under `pi-claude-marketplace/`
...
- [x] **ENBL-02**: A disabled plugin keeps its config entry and version pin, but its Pi artifacts are not materialized after reload
