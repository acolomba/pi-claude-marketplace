...
| T-117-02 | Tampering | the canary writing into the operator's real agent-state directory | high | mitigate | `assertSandboxContainment` (`workflow-agent-failure-canary.mjs:180-199`) called at `:229` — BEFORE `mkdir` (`:238`) and before the dynamic import (`:242`). Re-driven live during the audit against the real directory: refused at exit 1, nothing created. **Amended 2026-09-09 (code review):** the ordering half always held, but the guard was `agentDir.includes(path.join("tmp","pi-uat"))` — a substring test on the un-normalized value, which a `..` segment walked straight through (`$(pwd)/tmp/pi-uat/../../../tmp` passed, and would then have been created and handed to the engine). The audit's live re-drive used `$HOME/.pi/agent`, which the substring test refused, so it never reached the hole. The guard now resolves both sides and requires the sandbox root or a child of it; the traversal path and a `tmp/pi-uat-backup` sibling are both refused at exit 1 | closed |
| T-117-03 | Repudiation | a green canary on a credentialed machine that measured nothing | high | mitigate | A0 at `:250-260` precedes A1's first assert at `:267`; `nothingWasMeasured` (`:102-122`) exits 1. Re-proven firing during the audit with a stub returning the shape a SUCCESSFUL call produces. **Amended 2026-09-09 (code review):** the gate covered the first drive only. The fan-out drive is a separate call whose verdict depends on the same induced absence, and its logs were never read, so a credentialed machine got an assertion blaming the engine for a machine fact. The check now runs on every drive whose verdict depends on that absence (`:250-257`, `:259`, `:302`), and the exit message no longer states a cause it did not observe | closed |
...
| T-117-06 | Elevation of privilege | third-party JavaScript executed by the engine inside its own realm | low | accept | Verified: script text is literal-only (`FAN_OUT_SCRIPT` at `:222-225`, literal call arguments at `:259` and `:284`). No external input reaches the script body | closed |
