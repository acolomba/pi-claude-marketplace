...
| T-117-10 | Both latent tripwires were routed around and then checked: no table row in the file begins with a bare number in its first cell (the classification pin still scrapes exactly six and passes), and `grep -ci 'seven gates'` returns 0. Both pinned suites were run after the edit AND again after the plant was reverted. |
