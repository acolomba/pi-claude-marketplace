...
| T-116-11 | Repudiation | ungated prose counts in the notification modules | low | mitigate | Zero `45-member`/`45-entry` hits remain. Amended after code review WR-09: rather than keeping six prose sites in step with the length lock, the running count and its arithmetic changelog were removed from all of them. The only surviving `46` is the assertion `notify-closed-set-locks.test.ts` makes, plus its own test title three lines above it | closed |
