...
1. When `marketplace.json` omits a `plugins[].version` but the plugin's own `<pluginRoot>/.claude-plugin/plugin.json` declares a SemVer (e.g. `"version": "0.1.5"`), installing or updating that plugin records and renders that SemVer (e.g. `v0.1.5`), NOT a `vhash-…` token. A typo or non-SemVer string in plugin.json is rejected by shape validation and falls through to the PI-7 hash fallback.
