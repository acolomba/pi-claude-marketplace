...
- [x] 06-47-PLAN.md — Advance reinstall in the locked command sequence

**Wave 37** *(blocked on Wave 36 completion)*

- [x] 06-48-PLAN.md — Advance reinstall in the locked command sequence

**Wave 38** *(blocked on Wave 37 completion)*

- [x] 06-49-PLAN.md — Advance plugin list last in the locked command sequence
...
- [x] 06-51-PLAN.md — Advance plugin list last in the locked command sequence

**Wave 41** *(blocked on Wave 40 completion)*

- [x] 06-52-PLAN.md — Close Phase 6 with cross-family structural and full-suite proof
...
4. Every gate that addresses its targets by a path assembled from segments is
   discoverable without relying on a literal path appearing in its source, and
   proves it still resolves each target. A literal-match stale-path scan cannot
   see a composed reference: `tests/architecture/hooks-lifecycle.test.ts` and
   `tests/architecture/import-boundaries.test.ts` both kept addressing retired
   ledger modules through `path.join`, and one direction of the D-11 pair stayed
   green while matching import paths that could no longer occur. A gate whose
   target list silently stops resolving is worse than a missing gate, because it
   reports success.
...
- Gate targets live only in `tests/architecture/gate-targets.ts` as full literal repo-relative
  paths; every gate imports and composes nothing locally (`D-07-05`, `D-07-06`).
- Every gate this phase creates or modifies proves both visitation and firing: a visited-path
  report deep-compared against its declared group, a temp-root offender derived from the real
  target, and two benign controls (`D-07-01`, `D-07-03`, `D-07-04`).
- Tool-invoking gates resolve the effective configuration through the tool's own API and vary the
  config source through the real config plus exactly one appended block; no gate reads a config's
  raw source or mutates one (`D-07-02`, `D-07-09`, `D-07-10`, `D-07-20`).
- Both live `__`-prefixed offenders are removed before the gate that fires on them ships, and no
  allow-list forgives a named entry; the unowned-export census is a pinned set that fails in both
  directions (`D-07-18`, `D-07-19`).
- Superseded gates are deleted rather than left beside their replacements, and no comment survives
  the body it explained (`D-07-12`).
- Both complexity ceilings apply independently to every new gate helper, and `duplicates.threshold: 3`
  forces the temp-root and effective-config mechanics into shared modules.
...
- [x] 08-05-PLAN.md — the `cleanupStaging` removal port, migrated atomically across every production caller
...
- [x] 08-07-PLAN.md — close `install-outcome.ts`'s shortfall, with an escalation checkpoint and no pin row

**Wave 6** _(blocked on Wave 5 completion)_

- [x] 08-08-PLAN.md — the second sweep, the regenerated pin, and the coordinated record rewrite

**Wave 7** _(blocked on Wave 6 completion)_

- [x] 08-09-PLAN.md — wire the gate into a scoped pre-commit hook and a dedicated CI job
...
- [x] 09-01-PLAN.md — Thread one injected read port through both hooks call chains, end to end
- [x] 09-02-PLAN.md — Re-measure whole-tree coverage, regenerate and compare the pin, reconcile CONTRIBUTING.md
...
| 9. Final Quality and Backlog Closure | 2/6 | In Progress|  |
