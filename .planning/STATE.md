...
- [Phase 100]: info's disabled row routes through the shared block builder; the disabled status is injected at the two record-bearing arms, not inside the persisted derivation
- [Phase 100]: One optional InfoBlock.skipReason replaces the stateOnly boolean, so a scope with two skip causes cannot emit two rows; the disabled cause wins the fetch note, manifest absence stays on the inventory row
- [Phase 100]: ENBL-16's no-other-reason clause binds BOTH surfaces: info narrows the disabled row's reason brace exactly as list does, so one record cannot render two answers
- [Phase 100]: The disabled info row's reason narrowing is an allow-list against the single admitted member, folded into the same helper that injects the disabled status
- [Phase 100]: state-only-disabled-with-components carries no description line: the state-only arm reconstructs no description, so a fixture with one would be a falsified catalog example
...
| Phase 100 P04 | 55min | 2 tasks | 6 files |
| Phase 100 P05 | 50min | 3 tasks | 5 files |
